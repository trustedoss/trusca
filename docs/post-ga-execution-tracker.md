...
| W6-#42 | **자동 재매칭 — Celery beat 주기 재스캔**. DT가 못 하던(우리 코드 기준) "새 NVD 발견 시 기존 SBOM 재매칭"을 신규 기능으로 구현. 신규 task `tasks/vulnerability_rematch.py` — N시간(기본 6h)마다 succeeded 스캔들의 보존 SBOM을 `run_trivy_sbom`으로 재실행 → 새 finding만 추가/severity 변경 감지 → 알림 발행(기존 notification 인프라 재사용). Beat 시간대 분산 설계(다른 beat와 worker 큐 충돌 방지). 종료: 어제 깨끗했던 스캔이 새 NVD 데이터로 finding 추가되는 시나리오 e2e green. **dep: W6-#41 (#43a 전에 머지)**. | 🟦 PR 대기 — `feat/w6-42-vuln-rematch-beat`. **scope 확장**: source_preservation_service에 SBOM 폴드(`.trustedoss/cdxgen.cdx.json`)+`extract_preserved_sbom`/`preserved_tarball_has_sbom`/`PreservedSbomMissing` 추가(휘발 workspace 문제 해결). 마이그 0026 `scans.last_rematched_at` + 부분 인덱스 `ix_scans_rematch_due WHERE status='succeeded'`. 신규 task 2건(`enqueue_due_rematches` beat + `rematch_scan_findings` worker, `SELECT FOR UPDATE SKIP LOCKED` 단일 트랜잭션, prior snapshot → trivy → diff → persist → 알림 fan-out). 3 env: `VULN_REMATCH_INTERVAL_HOURS`(기본 6)·`VULN_REMATCH_BATCH_SIZE`(기본 50)·`VULN_REMATCH_LOCK_SKEW_SECONDS`(기본 30). Beat `crontab(minute=15, hour="*/6")` 다른 6h beat와 분산. 알림은 critical/high 신규/상승만 (스팸 가드). **테스트**: 단위 29(8 SBOM + 17 rematch + 4 기존 회귀 정합) + 통합 3(SBOM 보존→extract→trivy→persist→알림 chain). 게이트 ruff/mypy clean, 48 신규 모두 green. **rev: security-reviewer**(SBOM 추출 untrusted tarball + Trivy JSON 재파싱 + 행락 정확성). |
| W6-#43a | **백엔드 DT 제거 (비가역, #41+#42 머지 후)**. `apps/backend/integrations/dt/` 전체 삭제(`__init__`·`breaker`·`client`·`health`), `tasks/dt_{resync,orphan_cleaner,orphan_cleanup,health}.py` 4개 삭제, `api/v1/admin/dt.py` + admin service 삭제, `core/config.py:434-465` 8 getter 제거, `tasks/celery_app.py:39-81` import + beat schedule 정리, `scan_source.py`에서 `build_client`/`breaker`/`dt_executor`/`_dt_upload`/`_poll_dt_findings_with_retry` 잔재 제거(stage 이름은 #43f까지 유지). `.env.example` DT 섹션 제거. `docker-compose.dt.yml` → #43d로 이관(release notes와 함께 발표). 백엔드 DT 테스트 8개 삭제. `audit_log` 의 DT 액션 타입은 **보존**(역사 사실). **종료**: `grep -rIn "from .*dt\|DT_" apps/backend/ \| grep -v trivy \| grep -v audit_log` = 0. **회복**: 회귀 발생 시 본 PR git revert (외부 사용자 0). **dep: W6-#41 + W6-#42**. **rev: security-reviewer**(권한 우회/잔여 endpoint 검증). | ⏳ 백로그 |
