...
| W2 | **#33** | (정정) "조치신호(Exploitable/Solution)+CVSS 벡터"는 **이미 구현됨** — Exploitable=`status='exploitable'` 7-state enum + 드로어 status 배지, Solution=v2.2-a3 `upgrade_recommendation` + `DrawerUpgradeSection`, CVSS 벡터=`Vulnerability.cvss_vector` + 드로어 `cvss_vector_label`. **실제 남은 갭**: (a) 목록 License 리스크축 — ✅ 완료(2026-05-26): `list_project_vulnerabilities` lic_subq JOIN(per-cv worst rank, COALESCE→unknown) + `component_license_category` 필드 + `?license_category=` 필터(미지값 drop·unknown 버킷=LEFT JOIN miss), VulnerabilitiesTab 컬럼(CVE→Severity→License→Reachable…) + 툴바 MultiSelect, EN/KO 신규 2키(나머지 옵션은 기존 `license_category.*` 재사용). 게이트: ruff/mypy clean·pytest 144(+5)·vitest 913(+5)·openapi snapshot regen. (b) **Bulk actions** — ⬜ 다음 세션. | 🟦 진행중(a✅, b⬜) |
