...
| **W3 통합/발견성** | #32 | Reports 센터 탭 — **다운로드/익스포트 이력 통합 + 4영역 진입점 deeplink**(생성 UI는 도메인 탭 유지; 2026-05-26 사전 갭 분석으로 좁힘, [[feedback-tracker-text-may-overstate-gaps]]). BE: `report_downloads` 신규 테이블 + Notice/SBOM/Vuln-PDF/VEX-export 4 엔드포인트에 emit + `GET /v1/projects/{id}/reports/history`(팀 가시성 게이트, existence-hide 일관). VEX-import는 mutation이라 기존 audit_log로 흡수. FE: 신규 `Reports` 탭(좌·우 2단 — 좌 4 generate 카드 → 도메인 탭 deeplink, 우 이력 테이블 when/who/type/format/scan/size). 부속(분리 가능): Vuln PDF `scan_id` pin 미지원 → #32c 또는 #30과 묶음. | ⬜ 대기 |
