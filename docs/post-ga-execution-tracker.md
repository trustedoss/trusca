...
| W6-#43b | **프론트엔드 DT 제거**. `features/admin/dt/` 디렉토리 삭제(`AdminDTPage`·`adminDTApi`·`useAdminDT`). `router.tsx` `/admin/dt` 라우트 제거 → 404 처리. `AppShell.tsx` admin nav DT 항목 제거. EN/KO 번역 키 정리 + `npm run i18n:check` 통과. Playwright `admin/dt` 시나리오 → 404 redirect 시나리오로 교체. visual-regression DT 스크린샷 baseline 삭제. **dep: W6-#43a**. | ✅ 완료 (PR #206, 2026-05-27, squash merge `e812dac`). 8 파일 삭제 + 19 수정. `<Route path="*" element={<AdminNotFound />}>` 가 `/admin/dt` 자동 폴스루 — App.test.tsx + e2e #1 회귀 가드 추가. Disk/Health 도메인 타입을 W6-#43a BE trim(3-item disk · 6-component health · DT Problem extension 3 키 제거)과 정합. stage 이름(`dt_upload`/`dt_findings` + 관련 `step_dt_upload`/`alerts.dt_unavailable` i18n)은 #43f까지 의도 유지. 게이트: typecheck clean · lint 0 errors · i18n:check OK · vitest 947 pass(−25 DT 단위 케이스). 핸드오프 `docs/sessions/2026-05-27-w6-43b-fe-dt-removal.md`. |
...
| **W6-#44** | **Trivy DB 라이프사이클 관리 (필수로 승격)**. worker 부팅 시 `trivy --download-db-only` 1회 + weekly 주기 refresh(beat). **air-gapped 사용자용 `TRIVY_DB_REPOSITORY` 미러 설정 + 별도 PV 마운트 매뉴얼**(#43c air-gapped 절과 정합). `trivy image`(컨테이너 스캔)와 `trivy sbom`(SCA 매칭)이 동일 DB 공유 → 동시성 lock 테스트. 종료: 오프라인 부팅 시에도 마지막 캐시로 매칭 가능 + 동시 호출 안전. **dep: W6-#40 (Trivy 어댑터 패턴 재사용)**. | ✅ 완료 (PR #255, 2026-05-28). worker_ready 시그널 핸들러로 부팅 시 백그라운드 다운로드 (`tasks/trivy_db_bootstrap.py`) + 일요일 03:00 UTC weekly beat (`tasks/trivy_db_refresh.py` → `trustedoss.trivy_db_refresh`). `integrations/trivy.py::download_db_only` 신규 헬퍼가 두 경로 공유 (never-raises 계약, status=downloaded/skipped/timeout/failed). 실패/타임아웃 시 `TRIVY-DB-REFRESH-FAILED`/`-TIMEOUT` 알림을 기존 Slack/Teams 디스패처로 발사. air-gapped 매뉴얼 2-경로 추가 (사내 OCI 미러 / 완전 오프라인 캐시 볼륨 + `trivy-cache` named volume). 동시성: Trivy 자체 file lock에 위임 — 동일 캐시에 대한 두 subprocess 호출은 OS 레벨 직렬화되며 본 PR의 Python wrapper는 reentrant. 35 단위 테스트 / 91% line coverage. ruff/mypy clean. |
