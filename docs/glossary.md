...
| G7 AI SBOM Minimum Elements | G7 AI SBOM 최소요소 | The G7-agreed baseline element set an AI SBOM must carry (metadata, system-level properties, models, dataset properties, infrastructure, security properties, key performance indicators). Proper noun; keep "G7", spell out on first mention. | G7 합의로 정한 AI SBOM 필수 요소 집합(메타데이터·시스템 수준 속성·모델·데이터셋 속성·인프라·보안 속성·핵심성과지표). 고유명사로 "G7"을 유지하고 첫 등장에서 "G7 AI SBOM 최소요소(Minimum Elements)"로 풀어 씀. |

Updated 2026-07-02 — v2.3 (G7 AI SBOM minimum elements for the conformance panel).
