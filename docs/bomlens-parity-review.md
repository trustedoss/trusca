...
|---|---|
| BomLens 커밋 | `888f53f` |
| BomLens 태그 | v1.8.3 + 56커밋 (2026-07-23, #465) |
| 리뷰 일자 | 2026-07-24 (9차 — 8월 공개 준비 겸) |
...
| 21 | Deep-CVE: Maven CPE 유도 + grype CPE 보조 스캔 | — | **미채택 (9차, 2026-07-24 — 실증 기반)** | BomLens #464·#465 미러 후보였으나 **로컬 실증으로 기각**. ① BomLens 동기 사례(pdfbox 1.8.7·poi 3.6, "GHSA 부재 NVD-only CVE")는 현행 Trivy DB(2026-07 GHSA 백필)가 전부 탐지 — recall 14%→82% 격차가 TRUSCA 스택에서 미재현. ② 7-컴포넌트 probe(BomLens 픽스처 GAV 전수)에서 grype+CPE 추가분 72건을 GitHub Advisory API로 전수 대조 → **진성 CPE-only 미탐 0건**: 전부 (a) 제품 단위 CPE(`netty:netty`·`vmware:spring_framework`·`apache:poi`)가 형제 아티팩트 CVE를 오귀속(netty-common +48건은 netty-codec/handler/http2 것, spring-web +21건은 spring-messaging/beans 것, poi +2건은 poi-examples/scratchpad 것) 또는 (b) disputed CVE(CVE-2023-35116, GHSA 패키지 매핑 자체 없음). ③ BomLens의 NVD version filter는 버전 범위만 검증하고 아티팩트 오귀속은 못 거름(범위가 제품 단위라 통과). BomLens는 report-only+`†` unverified 각주로 수용 가능하나, TRUSCA는 findings를 트리아지 워크플로우에 영속하므로 FP 비용이 구조적으로 큼. **ADR-0001 개정 불필요.** 재평가 트리거: 실사용자 공급자 SBOM에서 Maven recall 격차 실증 시 |
| 22 | 일반 SBOM 적합성 CRA·NTIA 확장 (advisory 5종 + 규제 크로스워크 롤업) | P2 | **open — 채택, 구현 착수 (9차)** | BomLens #454·#455·#457·#462·#463 미러. TRUSCA `sbom_conformance.py`는 base 9체크+G7(AI 전용)만 보유 — CRA(BSI TR-03183-2)·NTIA 필드 체크 5종(`hash-algorithm`/`component-creator`/`component-filename`/`artifact-uri`/`file-properties`, 전부 advisory·verdict 불변) + `regulation-crosswalk.json` 벤더링(4 프레임워크, check-id 조인) + 프레임워크당 1행 롤업(present/gap/review) + purl empty-denominator 가드(패키지 0개 SBOM 부당 fail 수정) 미보유. 기존 g7 벤더링·정합 계약·패널 구조 재사용. 8차 #19(미채택)와 구분: 그건 정보성 결측→규제문서 매핑, 이건 적합성 채점 확장. X2(CRA 문서)와 시너지 |
| 23 | OS 컴포넌트 합성 (공급자 rpm SBOM의 operating-system 결측 보정) | P3 | open (9차) | BomLens #464(`enrich-os-context.py`) 미러. rpm PURL만 있고 `operating-system` 컴포넌트가 없는 공급자 SBOM은 Trivy가 distro advisory DB를 못 골라 **findings 0건**(BomLens 실측: 0→수만). TRUSCA ingest 경로도 동일 리스크 추정 — 채택 전 ingest 재현 확인 필요. 오프라인 후처리(neg. egress 0)라 이식 비용 소. distro 투표·major 축소(rocky 8.10→8)·멱등 로직 포팅 |
| 24 | G7 데이터셋 클러스터 v3 동기화 + NOTICE 데이터셋 구분 | P3 | open (9차) | BomLens #456 미러(부분). BomLens는 HF API로 데이터셋을 `data` 컴포넌트화하며 dp 클러스터 3요소(hash/dependency/content)를 auto/inferred로 승격(`4770bc2`)+ NOTICE `[dataset]` 태그·CC 라이선스 정규화(`b2112ec`)+ purl/name-version 커버리지를 packages 한정으로 교정. **채택 범위는 수용측만**: 레지스트리·술어 동기화(우리 g7_registry는 v2), NOTICE 확장, 커버리지 교정 — BomLens 산출 SBOM이 정당한 점수를 받게. **HF API 해석 자체는 역할경계**(생성기=BomLens 몫, TRUSCA는 ingest 수용. egress 불요) |
| 25 | G7 갭 해소 힌트 스니펫 표면화 | P3 | open (9차) | BomLens #435 미러. TRUSCA `g7Guidance.ts`는 #435 이전 벤더링 사본 — `docUrl` 링크만 렌더하고 CycloneDX 채움 조각(snippet)은 미표시. 결측 advisory 요소에 한해 조각 본문 노출(소규모 FE). BomLens의 백엔드 JSON 이관까지는 불요(우리는 FE 정적 맵 유지 가능) |
...
| ~~협의 대기~~ **전제 폐기(8차, 2026-07-18)** | 다언어 도달성 분석(Java/JS 등 call-graph 산출) | C4 원 전제("BomLens 산출→TRUSCA 소비")는 **성립 안 함**. BomLens 내부 로드맵(`improvement-roadmap.md:75`)이 도달성 분석을 정책 게이트·triage·VEX와 함께 **"포털(TRUSCA)의 영역"으로 명시 분류**하고 무상태 로컬 도구 정체성은 불변 선언 → 산출 주체가 될 의사 없음. 한편 **TRUSCA는 Go 도달성(govulncheck)을 이미 탑재**(`tasks/scan_reachability.py`, v2.3 r1). 따라서 C4는 "TRUSCA 자체 다언어 확장"으로 재정의. 보류하되 사유는 **데이터 해자**(ADR-0004): 콜그래프 도구는 존재(JS Jelly·Java WALA/Soot·Python PyCG)하나 비-Go 생태계의 오픈 함수 단위 취약-심볼 데이터가 부재(함수 단위 어드바이저리 전 생태계 ~1%, Go만 ~31%). Black Duck 도달성도 Java 전용+사람 큐레이션. 상세: `docs/decisions/0004-multilanguage-reachability-deferred.md` |

## 리뷰 이력

| 일자 | 기준점 이동 | 요약 |
|---|---|---|
| 2026-07-24 (9차) | `6b2ca03` → `888f53f` | v1.8.3 + 후속 64커밋 리뷰 (8월 공개 준비 트랙 P0). 신규 판정 5건: **#21 deep-CVE 미채택**(로컬 실증 — Trivy 현행 DB가 동기 사례 전부 탐지, grype+CPE 추가 72건 전수가 오귀속/disputed, ADR-0001 개정 불필요), **#22 CRA·NTIA 적합성 확장 채택·착수**(P2), #23 OS 컴포넌트 합성(P3)·#24 G7 dp v3 동기화+NOTICE(P3)·#25 갭 힌트 스니펫(P3) open. 격차 아님: SPDX 수출 UI(#439, 수출 4포맷 기보유), 적합성 리포트 KO(#447, i18n 기보유), Windows(#437, 역할경계), HF private 모델(#434, 역할경계 — 생성기 몫). 부수: 워커 cdxgen 핀 12.3.3 vs BomLens 12.8.0 — 범프는 골든 베이스라인 재채록 동반이라 별건 부채로(마스터플랜 §7). 실증 상세는 세션 핸드오프 참조 |
