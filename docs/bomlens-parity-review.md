...
| 11 | SCANOSS vendored OSS 식별 | P3 | in-progress (Phase J) | BomLens 기본 탑재(#271) ↔ TRUSCA. Phase J: `integrations/scanoss.py`(scancode 어댑터 패턴, full-file 매치만 승격) + 파이프라인 스테이지(scanoss 70, cdxgen→scancode→**scanoss**→trivy) + `ScanComponent(source="scanoss")`+detected 라이선스 + scanoss==1.53.1 워커 이미지. **판정: BomLens는 기본 ON이나 TRUSCA는 온프레미스라 `SCANOSS_ENABLED` opt-in·기본 OFF**(핑거프린트 외부 osskb.org 전송 프라이버시). security-reviewer(외부 egress) |
