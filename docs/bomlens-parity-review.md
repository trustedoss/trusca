...
| 9 | 의존성 그래프 뷰 | P3 | in-progress (Phase H-1) | BomLens cytoscape 그래프+트리 재설계(#243) ↔ TRUSCA Components는 테이블+드로어만. H-1: `GET /projects/{id}/dependency-graph`(edge 테이블 직렬화, 노드 상한) + cytoscape+dagre FE 포팅 + Components 탭 테이블↔그래프 토글 |
