...
>
> **2026-06-12 갱신 (TRUSCA 리브랜딩 W4)**: 레포가 `trustedoss/trusca`로 리네임되면서 포털 문서 경로가
> `/trustedoss-portal/` → **`/trusca/`** 로 바뀌었다. GitHub Pages 경로는 리다이렉트가 없으므로
> 구 경로로 들어오는 링크는 404가 된다 — 가이드 레포(`trustedoss.github.io` 루트, 불변)의 포털 링크는
> 새 경로로 갱신해야 한다(§7 핸드오프에 반영). 본문 경로 표기는 전부 새 경로 기준으로 치환했다.
...
| 항목 | 루트 가이드 (`/`) | 포털 문서 (`/trusca/`) |
|---|---|---|
| URL | `https://trustedoss.github.io/` | `https://trustedoss.github.io/trusca/` |
...
- **포털이 하위 경로(`/trusca/`)인 것은 버그가 아니라 의도된 설계다.** GitHub Pages 토폴로지상 `<org>.github.io` 루트는 org-pages 레포가, 프로젝트 레포는 하위 경로가 점유한다 (`docs-site/docusaurus.config.ts:16-17`).
...
- 가이드의 "도구 / DevSecOps" 섹션에서 포털 문서(`/trusca/`)로 연결. **퍼널상 더 가치 큰 방향**(why → how)이다.
...
> **주의 (2026-06-12, TRUSCA 리브랜딩)**: 포털 문서 경로가 `/trustedoss-portal/` → `/trusca/` 로 바뀌었다.
> GitHub Pages는 구 경로를 리다이렉트하지 않으므로, 가이드 사이트에 이미 있는 포털 링크가 있다면 전부 새 경로로 바꿔야 한다.

- **navbar**: `docusaurus.config.ts` 의 `themeConfig.navbar.items` 에 외부 링크 추가
  ```ts
  { href: "https://trustedoss.github.io/trusca/", label: "Portal", position: "right" },
  ```
...
- **콘텐츠 인라인 링크**: "DevSecOps / 도구 / SCA" 관련 문서 본문에서, 실제 스캔·SBOM·라이선스·CI 게이트를 실행하는 도구로 포털 문서를 연결한다. 이게 *why → how* 퍼널의 핵심 방향이다.
  - 제품 문서 진입점들:
    - 설치: `https://trustedoss.github.io/trusca/docs/quickstart`
    - CI 연동: `https://trustedoss.github.io/trusca/docs/ci-integration/github-actions`
    - API 레퍼런스: `https://trustedoss.github.io/trusca/reference/api`
