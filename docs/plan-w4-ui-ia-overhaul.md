...
## 4. W4-D — TYPE / USAGE 본격 픽스 (P2) ✅ PR 대기 (2026-05-27)
...
### 채택한 접근 — Option C (lockfile-direct 폴백)
3가지 옵션 평가 결과:
- **Option A (cdxgen 옵션 튜닝)**: cdxgen 12.3.3에 npm scope 강제 flag 없음. 12.5+ upgrade는 worker 이미지 변경 동반(별도 트랙). 패스.
- **Option B (ORT analyzer cross-merge)**: PR-A2가 ORT를 명시적으로 제거(`_persist_components` 도큐먼트). 부활 비용 큼. 본 PR scope 외.
- **Option C (npm lockfile 직접 파싱)** ✅ — 신규 모듈 1개 + 기존 2 함수 시그니처 확장. 외부 의존 없음, 기존 worker 이미지 그대로.

### 구현
- **신규** `apps/backend/integrations/npm_lockfile.py`: `package-lock.json` v3/v2/v1 파서. 산출 `NpmLockfileData(scope_by_purl, adjacency)`. 신뢰 경계(malformed JSON → None, `MAX_PACKAGES=200_000` 캡, 비-dict entry 무시, 스코프 precedence `required > peer > optional > dev`).
- **확장** `apps/backend/tasks/scan_source.py::_persist_components`:
  - `source_dir: Path | None = None` 인자 추가(기본 None → 기존 통합 테스트 호환).
  - 함수 진입 시 lockfile 1회 로드(`npm_lockfile_loaded` INFO). 컴포넌트 루프 내에서 **scope precedence**: cdxgen `scope` 비어있지 않으면 그대로 → 빈 string/None이고 npm purl이면 lockfile scope → 그 외 NULL. **non-npm purl은 lockfile 절대 미참조**(트러스트 경계, 스푸핑된 항목 무시).
- **확장** `_persist_dependency_graph`:
  - `npm_lock: NpmLockfileData | None = None` 인자 추가.
  - cdxgen `dependencies` 파싱 결과가 빈 adjacency 일 때만 `npm_lock.synthesize_cdxgen_dependencies()`를 동일 `parse_dependency_graph` 트러스트 경계로 재파싱 → fallback(`dependency_graph_lockfile_fallback` INFO). lockfile root `""`를 forced_root로 사용해 직접 dep = depth 1.
  - `dependency_graph_persisted source="cdxgen"|"npm_lockfile_fallback"` 로그 추적.

### 데이터 누락 graceful fallback
- npm 외 생태계(Maven/Gradle/Cargo/Go/.NET): cdxgen scope/graph 신뢰. lockfile 미참조.
- npm이지만 lockfile 부재 / malformed: `dependency_graph_missing` WARN + `npm_lockfile_available=false` 필드로 운영 가시화. scope/depth NULL 유지(graceful).
- pnpm-lock.yaml / yarn.lock: 본 PR scope 외(파서 미작성). cdxgen가 yarn.lock는 이미 처리, pnpm은 별도 트랙.

### 단위 테스트 (신규 53건)
- `tests/unit/integrations/test_npm_lockfile.py` (30 건) — happy path(prod/dev/optional/peer), 트랜지티브 inheritance, nested ancestor 해결, scoped `@scope/pkg` URL 인코딩, strongest-wins, v1 nested fallback, 5가지 adversarial payload, dataclass frozen, workspaces 레이아웃.
- `tests/unit/tasks/test_scan_source_dependency_graph.py` — lockfile 폴백 3 케이스(stamps depth when empty, ignored when cdxgen nonempty, skipped when None).
- `tests/unit/tasks/test_scan_source_scope_enrichment.py` (7건) — cdxgen wins / npm-only 적용 / non-npm 비참조 / no lockfile / source_dir None / load-once / empty string scope.
- 기존 `test_scan_source_parallel_dt_upload.py` lambda 시그니처 `source_dir=None` 추가.

### 게이트 결과
- `ruff check .` clean
- `mypy .` clean (테스트 포함, 422 source files)
- 신규 `npm_lockfile.py` line coverage 89%
- 전체 unit 2673 pass / 1 deselect (pre-existing `test_scan_reachability` REDIS_URL 환경 fluke, 본 PR 무관)
- OpenAPI 스냅샷 회귀 없음 (API 노출 미변경)
- Alembic 미동반 (스키마 무변경)

### 운영 검증 (orchestrator 머지 후 수행)
1. demo-org 재시드 또는 maven-node fixture로 새 스캔 트리거.
2. worker 로그에서 `npm_lockfile_loaded packages=… graph_nodes=…` + `dependency_graph_persisted source="cdxgen"|"npm_lockfile_fallback"` 확인.
3. `/projects/<id>?tab=components` UI에서 TYPE 컬럼(direct/transitive) + USAGE 컬럼(required/dev/optional/peer)이 npm 행 대부분에 채워지는지 시각 검증.
4. DB 쿼리 `SELECT direct, dependency_scope, COUNT(*) FROM scan_components WHERE scan_id=? GROUP BY ...` 로 NULL 비율 감소 측정.

### 알려진 한계 / 잔여 갭
- pnpm-lock.yaml / yarn.lock 직접 파싱 미구현 — pnpm 사용 프로젝트는 여전히 dash.
- npm 호이스팅 충돌 시 nearest-ancestor 해결로 95%+ 정확하지만 deliberate-conflict 그래프는 미세 오류 가능.
- cdxgen 12.3.3 자체 옵션으로 해결 못함이 확정. cdxgen 12.5+ upgrade는 별도 worker 이미지 트랙.

### 후속
- pnpm/yarn lockfile 파서 (별도 PR, npm 사용자 비율 관찰 후 우선순위 판단)
- cdxgen 12.5+ upgrade (worker 이미지 + e2e 재검증)
- UI banner "dependency graph 미수집" — diagnose §갭 2 권장 픽스 #4 (P3 본 PR scope 외)
...
| PR (예정) W4-B #16/#17/#19 | UX 정합성 단일 PR — Overview 차트 deep-link + Components/Vulnerabilities 컬럼 헤더 정렬·License 분리·Severity/License 토글 제거·`ActiveFilterChips`. **Component@Version 컬럼은 BE schema 갭(`VulnerabilityListItem`에 affected name/version 미존재)으로 보류 → BE 스키마 보강 후 follow-up.** 게이트 typecheck/lint/vitest 958/i18n:check 모두 green. | ✅ 머지 `edaee91` (PR #188) |
| PR (예정) W4-C #20/#21/#22 | IA 재정비 단일 PR — Licenses+Obligations→Compliance (sub-tab wrapper `?cview=`), SBOM→Reports (in-page section `reports-sbom-section`, `?rpt_section=sbom` auto-scroll), Remediation→Vulnerabilities (collapsible `VulnerabilitiesRemediationPanel`, `?vuln_section=remediation`). 탭 11→8. `ProjectDetailPage` `ALLOWED_TABS` 8개 + `redirectLegacyTab` 헬퍼로 `?tab=licenses\|obligations\|sbom\|remediation` URL 자동 rewrite. OverviewTab 차트 deeplink가 새 IA(`?tab=compliance&cview=licenses&license_category=…`)로 변경. 하네스 verb 3개(`selectLicensesTab`/`selectObligationsTab`/`selectSbomTab`) 새 IA로 리다이렉트. i18n EN/KO 동시(`tabs.compliance`/`compliance.subtab.*`/`reports.sbom.*`/`vulnerabilities.remediation.*`, legacy `tabs.{licenses,obligations,sbom,remediation}` 제거). 게이트 typecheck/lint 0 errors/vitest 967(+9)/i18n:check OK. | ✅ 머지 (PR #189) |
| PR (예정) W4-D | TYPE/USAGE 본격 픽스 — Option C `npm_lockfile.py` 파서. `_persist_components(source_dir)` + `_persist_dependency_graph(npm_lock)`. npm scope 폴백 + cdxgen `dependencies` 누락 시 lockfile adjacency 폴백(동일 trust boundary 재사용). 53 신규 unit + 89% line cov. 게이트 ruff/mypy clean. 운영 검증=머지 후. | 🟡 PR 생성 대기 (orchestrator) |
