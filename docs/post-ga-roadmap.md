...
| **v2.4** | 공급망 위협 탐지 & 우선순위 심화 | 악성/타이포스쿼팅 탐지, CISA KEV+통합 Risk Score, 바이너리 스캔, AI-BOM, (최하위)스니펫 | minor |

**비범위(Non-goal):** 자체 취약점 DB 구축은 비범위로 유지한다(DT 집계 + OSV/KEV 보조, §7). 스니펫/full-text 출처 검출(ScanOSS급)은 이전엔 명시적 제외였으나 2026-05-24 결정으로 **v2.4 P4(최하위 우선순위·RFC 선행)**로 편입한다.
...
- **작업:**
  - **OSSF Malicious Packages**(`github.com/ossf/malicious-packages`) + **OSV.dev `MAL-` 피드**를 Celery로 주기 동기화 → 탐지된 PURL과 대조(DT 캐시 패턴 재사용).
...
- **의존:** 기존 cdxgen PURL, DT 캐시/circuit-breaker 패턴.
...
- **작업:**
  - **CISA KEV** 무료 피드 동기화 → "known-exploited" 플래그(EPSS·DT 단일 의존 완화 — §9 컨틴전시 정합).
...
- **작업:** **Syft binary classifier** + Trivy `fs`/`rootfs` 모드를 새 스캔 종류 "binary"로 추가 → SBOM 생성 후 기존 DT 파이프라인 주입(배관 재사용).
...
- **접근:** **ScanOSS** — 클라이언트 `scanoss-py`(MIT) import + 엔진(GPL-2.0)은 **별도 컨테이너로 격리**(DT/Trivy와 동일 패턴 → Apache-2.0 오염 없음). KB는 osskb.org(PoC) / 자체 KB(온프렘, `minr`).
