...
**자동 재매칭 beat**(W6-#42 도입)은 DT가 우리 배포에서 제공하지 못했던 킬러 기능입니다 — 새 CVE가 갱신된 DB에 들어오면, beat 태스크가 모든 프로젝트의 최신 SBOM을 순회해 매칭되는 `vulnerability_findings` 행을 새로 씁니다. 사용자는 재스캔 없이 Vulnerabilities 탭에서 새 발견을 확인합니다.
...
| Maven (Java / Kotlin) | `pkg:maven/*` | OSV + GHSA + NVD | 1급. v2.4.0(W8-#46)부터 classifier 인식. |
...
- [ADR-0001 — Dependency-Track 제거](https://github.com/trustedoss/trustedoss-portal/blob/main/docs/decisions/0001-replace-dt-with-trivy.md) — Trivy가 v2.4.0부터 단일 엔진인 이유.
