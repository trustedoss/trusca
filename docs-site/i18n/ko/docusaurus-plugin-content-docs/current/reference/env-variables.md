...
| `LOG_MAX_SIZE` | `20m` | `docker-compose.yml` | 컨테이너 로그 파일이 회전하는 크기. |
| `LOG_MAX_FILE` | `5` | `docker-compose.yml` | 컨테이너마다 보관하는 회전 파일 수. 기본 크기와 합치면 서비스당 100 MB입니다. |
...
:::note 창을 좁히면 조용히 초과가 됩니다
이 값들은 호출 시점에 읽히므로 창을 바꾸려면 설정을 고치고 재시작해야 합니다. 새 창이 기한을 넘긴 것으로 만드는 finding은 전부 과거에 넘긴 것이고, SLA 스윕은 자기 창 안에서 넘은 것만 알리므로 어느 것도 통보되지 않습니다. 아무도 듣지 못하고, 그 변경은 나중에 `?sla=overdue` 목록이 길어진 것으로 드러납니다.

백엔드가 기동할 때마다 `vuln_sla.overdue_at_boot`에 적용 중인 창 값과 심각도별 초과 건수를 함께 남깁니다. 재시작 전후의 두 줄을 비교하면 그 변경이 무엇을 했는지 보입니다. `high`가 30에서 7로, 초과가 40에서 380으로 같은 식입니다. 건수는 규모와 무관하게 정확합니다. 행을 훑는 대신 집계 하나로 세기 때문입니다. 훑는 방식이면 어딘가에서 멈춰야 하고, 그 지점을 넘어선 배포는 변경 전후에 같은 숫자를 보고하게 됩니다.

로그가 전부입니다. 알림은 없으니 사람이 봐야 하고, 이것을 남기는 이유는 몇 달 뒤에 밀린 것이 왜 이렇게 늘었는지 묻는 사람입니다.
:::
...
| 키 | 기본값 | 읽는 위치 | 설명 |
|---|---|---|---|
| `GROUP_CASCADE_ENABLED` | `true` | `config.py` | 그룹에 부여된 멤버십이나 API 키가 그 그룹의 하위 그룹까지 함께 도달할지 정합니다. 기본값은 켜짐입니다. 관리자가 계층 안으로 실제로 옮겨 넣기 전에는 그룹이 하위 그룹으로 확장되지 않으므로, 아직 그룹을 중첩시키지 않은 배포는 이 값을 켜도 동작이 바뀌지 않습니다. `false`로 설정하면 계층 도입 전의 단일 그룹 범위로 되돌립니다. |
| `GROUP_SEARCH_RATE_LIMIT` | `20/minute` | `config.py` | `GET /v1/groups`의 검색 모드(`?q=`)에 적용되는 slowapi 제한이며, 인증된 사용자별로 키가 매겨집니다. 이 엔드포인트는 프로젝트 생성 화면의 그룹 선택 combobox가 입력할 때마다 호출하는 대상이라, 드릴다운 모드에서 가끔 읽는 것보다 키 입력 빈도에 맞춘 값입니다. |

캐스케이드의 사용자 관점 동작은 [중첩 그룹](../admin-guide/users-and-teams.md)을 참고합니다.
...
| `SCAN_SCOPE_FILTER_NON_DEPLOYABLE_ENABLED` | `true` | `config.py` | 스코프 필터의 경로 부분입니다. manifest가 모두 `test`·`tests`·`examples`·`demo`·`benchmark` 계열 디렉터리 아래에 있는 컴포넌트를 제거합니다(전체 목록은 사용자 가이드). 저장소가 이런 디렉터리의 코드를 배포한다면 끄십시오. 스캔 재사용 지문에 포함되므로 값을 바꾸면 이전 스캔을 재사용하지 않습니다. |
...
| `LICENSE_FETCH_SCAN_BUDGET_SECONDS` | `300` | `config.py` | 스캔 하나가 라이선스 보강을 위한 레지스트리 조회에 쓸 수 있는 시간(초)입니다. 캐시에서 답한 조회는 시간에 들어가지 않습니다. 시간을 다 쓰면 남은 컴포넌트는 라이선스 미상으로 두고, 스캔은 그대로 성공하며, 조회하지 않은 건수를 `scan_metadata.license_enrichment`에 기록합니다. 이 컴포넌트는 "없음"으로 캐시하지 않으므로 다음 스캔이 이어서 조회합니다. 검사는 조회와 조회 사이에 하므로 조회 한 건만큼 넘길 수 있습니다. 숫자가 아니거나 0 이하이면 기본값을 씁니다. |
| `LICENSE_FETCH_CONSECUTIVE_FAILURE_LIMIT` | `30` | `config.py` | 레지스트리가 답하지 않은 조회(연결 오류, 시간 초과, 재시도를 모두 거친 429 또는 5xx)가 이 횟수만큼 연달아 나오면 그 스캔은 라이선스 조회를 멈춥니다. 레지스트리가 "없음"(404)이라고 답한 것은 응답이므로 횟수를 0으로 되돌립니다. 건너뛴 조회는 `scan_metadata.license_enrichment`에 기록하고 캐시하지 않습니다. 정수가 아니거나 0 이하이면 기본값을 씁니다. |
| `LICENSE_FETCH_MAVEN_PARENT_MAX_DEPTH` | `3` | `config.py` | Maven 컴포넌트의 POM에 `<licenses>`가 없으면 `<parent>` POM에서, 그 POM에도 없으면 다시 그 상위 POM에서 라이선스를 가져옵니다. 이 값은 한 번의 조회가 따라가는 상위 POM의 최대 단계 수입니다. 이 단계에 닿으면 오류가 아니라 라이선스 미상으로 둡니다. `0`이면 상속을 끄고 컴포넌트 자신의 POM만 읽습니다. 여러 컴포넌트가 공유하는 상위 POM은 한 번만 조회하고 캐시합니다. 상속으로 얻은 라이선스 행에는 출처 POM이 `raw_data.inherited_from`에 기록됩니다. 정수가 아니거나 음수이면 기본값을 씁니다. |
...
| `LOGIN_THROTTLE_FAILURES` | `10` | `config.py` | 한 주소가 거절되기까지 허용하는 연속 실패 횟수입니다. IP 제한(분당 5회)보다 일부러 높게 둡니다. 5로 두면 한 대에서 로그인하는 사람이 이쪽에 먼저 걸려서, 문서에 적힌 IP 제한의 여섯 번째 429에 도달할 수 없고 비밀번호를 잘못 입력한 사람이 더 가혹한 쪽을 만나게 됩니다. 이 통제는 여러 출처에서 한 주소로 실패가 쌓이는 경우를 위한 것입니다. |
