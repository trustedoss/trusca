...
조직을 온보딩한다는 것은 저장소마다 프로젝트를 만든다는 뜻입니다. 그것이 HTTP 호출
300번이 되지 않도록 목록을 받는 엔드포인트가 둘 있습니다.

| 엔드포인트 | 본문 | 생성 대상 |
|---|---|---|
| `POST /v1/projects:batch` | `{"projects": [ … ]}`, 각 항목은 `POST /v1/projects`가 받는 본문 | 프로젝트 |
| `POST /v1/scans:batch` | `{"project_ids": [ … ], "ref": "main"}` | 프로젝트당 스캔 1건 |

한 요청에 최대 200행입니다. 행은 순서대로, 서로 독립적으로 처리됩니다. 실패한 행이
앞선 행을 되돌리지 않고 뒤 행도 그대로 실행됩니다. 의도한 동작이며, 재실행을 안전하게
만드는 것이 이 성질입니다.
...
행을 순회하지 말고 상태 코드나 `all_succeeded`를 보세요. 모든 행이 성공하면 `201`,
하나라도 실패하면 `207 Multi-Status`를 반환하므로 스크립트가 상태 줄만으로 분기할 수
있습니다. 부분 실패에 `200`을 쓰지 않은 것은 의도적입니다. 클라이언트 라이브러리가
200을 성공으로 처리하므로, 절반이 실패한 배치가 성공한 것으로 기록됩니다.

각 행은 다섯 상태 중 하나를 답니다.

| 상태 | 집계 | 뜻 | 할 일 |
|---|---|---|---|
| `created` | 성공 | 프로젝트나 스캔이 생성되었습니다. | 없습니다. |
| `already_exists` | 성공 | 이미 있었습니다. 스캔은 이미 대기 중이거나 실행 중입니다. | 없습니다. 재실행의 정상 결과입니다. |
| `forbidden` | 실패 | 호출자가 대상 팀의 구성원이 아닙니다. | 권한을 받은 뒤 그 행을 다시 보내세요. |
| `invalid` | 실패 | 다른 이유로 거부되었습니다. `detail`에 이유가 있습니다. | 행을 고치세요. |
| `rate_limited` | 실패 | 팀의 동시 스캔 상한에 도달했습니다. `retry_after_seconds`가 대기 시간을 추정합니다. | 남은 행을 나중에 다시 보내세요. |

`already_exists`를 성공으로 세는 것은 의도적입니다. 중단된 온보딩을 마치는 방법이
배치 재실행이고, 두 번째 실행에서는 대부분의 행이 이미 존재합니다. 그것을 실패로 세면
모든 재실행이 실패로 보고됩니다. 아무것도 바뀌지 않은 재실행은 `created: 0`과 `total`과
같은 `already_existed`를 반환하며, 호출자는 그것으로 앞선 실행이 끝났음을 압니다. 기존
행도 `project_id`를 담으므로 첫 응답을 잃은 호출자가 id를 회수할 수 있습니다.

### 스캔과 동시 실행 상한

`POST /v1/scans:batch`는 팀별 동시 스캔 상한을 우회하지 않으며 그럴 의도도 없습니다.
상한은 공용 워커 풀을 보호하기 위한 것입니다. 상한은 행마다 팀의 실시간 활성 스캔 수를
다시 세어 적용하므로, 배치는 상한만큼 스캔을 시작하고 나머지를 `rate_limited`로
보고합니다. 상한을 넘겨 대기열에 넣으면 부하가 사라지는 것이 아니라 시점만 미뤄지므로
나머지는 보류하지 않고 거절합니다. 앞선 스캔이 끝난 뒤 다시 보내세요.
