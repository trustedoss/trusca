...
id: glossary
title: 용어집
description: TRUSCA 도메인 용어 — SCA, SBOM, VEX, 라이선스 단계, RBAC 역할, CycloneDX / SPDX 매핑.
sidebar_label: 용어집
sidebar_position: 4
---
...
- **SCA — Software Composition Analysis.** 소프트웨어 프로젝트에서
  ...
  취약점을 식별하는 분야. TRUSCA는 SCA 도구입니다.
- **SBOM — Software Bill of Materials.** 소프트웨어에 포함된 모든
  컴포넌트(버전·라이선스·공급자 포함)의 기계 가독 명세. TRUSCA는
  CycloneDX(JSON·XML)와 SPDX(JSON·Tag-Value) 형식으로
  SBOM을 내보냅니다.
  [CISA SBOM 자료](https://www.cisa.gov/sbom) 참고.
- **CycloneDX.** OWASP가 관리하는 SBOM 명세. TRUSCA는 1.6
  버전(JSON·XML)을 사용합니다.
  [cyclonedx.org/specification](https://cyclonedx.org/specification/)
  참고.
- **SPDX — Software Package Data Exchange.** Linux Foundation이
  관리하는 SBOM 명세. TRUSCA는 2.3 버전(JSON·Tag-Value)을
...
- **EPSS — Exploit Prediction Scoring System.** CVE가 향후 30일 내
  **실제 악용**될 0–1 확률. EPSS는 CVSS를 보완합니다 — CVSS는
  심각도, EPSS는 가능성. TRUSCA는 score를 백분율로, percentile을
...
- **VEX — Vulnerability Exploitability eXchange.** 알려진 취약점이
  ...
  `analysis.state`와 SPDX VEX 두 가지가 주된 인코딩입니다. TRUSCA는
...
- **scancode — scancode-toolkit.** 프로젝트의 **first-party** 소스
  ...
  태깅합니다. TRUSCA는 소스 스캔의 두 번째 단계로 scancode를
  실행합니다(v0.10.0에서 OSS Review Toolkit, ORT를 대체). 서드파티
...
- **cosign.** Sigstore의 서명 CLI. TRUSCA는 모든 소스 스캔의
  ...
  [SBOM 서명 검증](../ci-integration/sbom-signature-verification.md)과
...
- **Attestation / provenance (in-toto, SLSA).** 산출물이 *어떻게*
  생성되었는지에 대한 서명된 명세. TRUSCA는 SBOM 서명과 함께
  [SLSA](https://slsa.dev/) provenance(빌더 신원 + 빌드 컨텍스트)를 담은
  [in-toto](https://in-toto.io/) Statement를 생성합니다.
  [SBOM 서명 검증](../ci-integration/sbom-signature-verification.md#provenance-attestation-확인) 참고.
