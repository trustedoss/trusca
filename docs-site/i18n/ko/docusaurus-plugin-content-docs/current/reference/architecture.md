...
| `backend` | `trustedoss/backend:<tag>` | FastAPI + uvicorn(4 workers). Traefik이 `/api`와 `/health`로 라우팅. (`/metrics`는 [관측성](#관측성) 참고.) |
...
스캔은 Celery 태스크 체인입니다. 아래 단계명은 `scan.<id>.progress` WebSocket 채널에 스트리밍되는 `step` 값 그대로이며 — `tasks/scan_source.py::_STAGE_PROGRESS`와 `tasks/scan_container.py::_STAGE_PROGRESS`에서 옵니다.

소스 스캔 단계 (`tasks/scan_source.py`):

```
1. bootstrap     (0 %)   workspace 셋업, 프로젝트별 lock 획득, 스캔 행을 running으로 표시
2. fetch         (10 %)  git clone / fetch / checkout
3. prep          (18 %)  lockfile 사전 패스(cdxgen 이전 일회성 의존성 해석)
4. cdxgen        (25 %)  cdxgen → CycloneDX SBOM
5. ort           (50 %)  ORT가 SBOM을 소비, 라이선스 발견 + 의무사항 emit
6. dt_upload     (70 %)  Dependency-Track으로 SBOM 업로드
7. dt_findings   (90 %)  DT 취약점 발견 가져오기 (breaker OPEN 시 캐시 폴백)
8. finalize      (100 %) 단일 트랜잭션으로 PostgreSQL 기록, 스캔 완료 표시
```

컨테이너 스캔 단계 (`tasks/scan_container.py`):

```
1. bootstrap     (0 %)   workspace 셋업 + 이미지 가져오기(skopeo pull 또는 worker 캐시 적중)
2. trivy         (60 %)  OS 패키지 CVE 탐지
3. persist       (90 %)  발견을 PostgreSQL에 삽입
4. finalize      (100 %) 트랜잭션 커밋, 스캔 완료 표시
```
...
- **Metrics** — 기본 서비스-헬스 메트릭은 Traefik 액세스 로그를 통해 제공됩니다. 백엔드 `/metrics` Prometheus exporter는 **아직 구현되지 않았으며**, `docker-compose.yml`이 placeholder로 Traefik `PathPrefix(/metrics)` 라우트만 예약해 두었지만 백엔드에 핸들러가 바인딩되어 있지 않습니다. 이 엔드포인트는 GA 이후 로드맵에 도착합니다.

OpenTelemetry tracing exporter와 번들 Jaeger 오버레이는 GA 이후 로드맵 항목이며 — v2.0.0에는 `docker-compose.tracing.yml` 파일이 없습니다.
