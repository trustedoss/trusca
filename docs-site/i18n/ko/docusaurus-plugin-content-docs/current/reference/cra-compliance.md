...
| §2 — 취약점을 지체 없이 조치한다 | 각 발견 항목은 수정 버전 정보를 담고, 조치 시뮬레이션과 풀 리퀘스트 생성이 업그레이드 경로를 제안한다. 심각도별 조치 SLA 추적이 각 발견 항목의 경과시간을 최초 탐지 시각부터 기한 대비로 재고, 기한 초과 필터와 일일 초과 알림을 제공한다. CI 빌드 게이트는 Critical CVE·금지 라이선스에서 빌드를 실패시켜(`exit 1`) 회귀가 출하되지 않게 한다. | [SLA 추적](../user-guide/vulnerabilities.md#sla), [조치 PR](./remediation-pull-request.md), [시뮬레이션](./remediation-dry-run.md) |
...
- SLA 기간은 조직의 정책이지 규제 자체의 기준이 아니다. TRUSCA는 이제 각
  발견 항목의 경과시간을 최초 탐지 시각부터 심각도별 조치 기간 대비로
  추적하고, 기한 초과 필터와 초과 알림을 제공한다 —
  [조치 SLA와 경과시간 추적](../user-guide/vulnerabilities.md#sla) 참고. 다만
  이 기간은 운영자가 조정하는 기본값이며, 어떤 지연이 CRA가 말하는 "지체"에
  해당하는지는 포털이 대신 판단할 수 없다.
...
6. 심각도별 SLA 기간(`VULN_SLA_DAYS_*`)을 조직의 조치 정책에 맞게 설정하고,
   기한 초과 대기열(`?sla=overdue`)을 처리하며, 초과 알림을 켜 둔다 — "지체
   없는" 조치의 경과시간 증적이 된다.
