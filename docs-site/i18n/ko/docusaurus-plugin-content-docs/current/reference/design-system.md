...
`<html>`의 `.dark` 클래스로 전환합니다. 사용자 설정은 번들이 로드되기 전에 `index.html`의 인라인 스크립트가 해석합니다. React 안에서 해석하면 첫 프레임이 라이트로 그려져 새로고침마다 화면이 하얗게 번쩍이기 때문입니다. 상태는 셋(라이트 · 다크 · 시스템 따라가기)이고 글로벌 바에서 순환합니다.
