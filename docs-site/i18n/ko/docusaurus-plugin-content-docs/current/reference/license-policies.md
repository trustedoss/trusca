...
정책은 다음 두 종류의 범위 중 정확히 하나로 존재합니다.

| 범위 | `team_id` | 적용 대상 | 작성 권한 |
| --- | --- | --- | --- |
| **그룹** | 설정됨 | 해당 그룹(팀) 하나 | 그룹의 `team_admin` 또는 `super_admin` |
| **조직 기본** | `null` | 적용할 그룹 정책이 없는 조직 내 모든 그룹 | `super_admin` 전용 |

조직마다 조직 기본 정책은 최대 1개, 그룹마다 정책은 최대 1개입니다. 같은 범위에 다시
`PUT` 하면 기존 행을 업데이트합니다(멱등 upsert).

### 유효 정책 해석

그룹은 계층 깊이에 제한 없이 중첩됩니다. 한 팀이 어느 사업부 아래에 있고, 그 사업부가
또 다른 사업부 아래에 있는 식으로 조직 루트까지 이어질 수 있습니다. 한 팀의 **조상
사슬**은 그 팀 자신과 그 위의 모든 그룹을 가장 가까운 것부터 루트까지 나열한 목록입니다.
팀의 유효 정책은 이 사슬을 따라가며 다음 순서로 결정됩니다.

1. 팀 자체 정책 — 존재하고 enabled 이면 적용, 아니면
2. 가장 가까운 조상 그룹의 정책 — 존재하고 enabled 이면 적용, 아니면
3. 그 위 조상, 그 위 조상, ... 루트까지 반복, 그래도 없으면
4. 조직 기본 정책 — 존재하고 enabled 이면 적용, 아니면
5. 없음(팀은 내장 정적 카탈로그로 폴백합니다).

사슬에서 팀에 가장 가까운 **enabled** 정책이 필드 전체를 통째로 적용합니다. 두 범위의
필드를 섞어 합치지 않습니다. 어떤 단계의 정책이 비활성이거나 없으면 그 단계만 건너뛰고
다음 조상으로 넘어갈 뿐, 탐색을 멈추거나 곧바로 조직 기본값으로 건너뛰지 않습니다.
그룹을 조직 루트 아래로 중첩하지 않는 배포라면 이 순서는 예전의 2단계("팀, 그다음
조직") 동작과 결과가 같습니다. 그런 팀의 조상 사슬은 조직 기본값 앞에 항목이 하나뿐이기
때문입니다.

`enabled: false`는 정책을 삭제하지 않고 비활성화하므로, 그룹은 정책을 다시 작성하지
않고도 동적 정책을 껐다 켤 수 있습니다. 그러면 사슬 탐색은 다음 조상(또는 조직
기본값)으로 그대로 이어집니다.
...
아닙니다. 조직 엔드포인트는 super-admin 전용이며 존재를 숨깁니다(비-super-admin은
`404`).
