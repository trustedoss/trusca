...
:::note 상태
정책 **데이터 모델 + CRUD API**와 **동적 빌드 게이트 평가**(하드닝된 복합 SPDX
해석 포함)가 이제 연결되었습니다. 유효하고 enabled 인 정책은 해당 팀의 게이트
금지-라이선스 판정을 바꿉니다. 아래 [동적 게이트 평가](#동적-게이트-평가)를
참고하세요. 정책 적용 시 라이선스의 유효 카테고리를 미리 보는 UI는 후속 릴리스에서
다룹니다.
:::
