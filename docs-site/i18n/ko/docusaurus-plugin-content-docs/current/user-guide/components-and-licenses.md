...
분류는 룰 기반입니다. `ort/rules.kts`를 수정하고 워커를 재시작 후 재스캔. 룰 자체가 맞는데 concluded 라이선스가 잘못이면, concluded 라이선스의 수동 오버라이드는 v2.2 로드맵 항목입니다. 현재 표시되는 결론은 ORT/cdxgen 출력 그대로입니다.
