...
### 패키지 관리자가 이미 찾은 라이브러리 {#vendored-declared-merge}

같은 라이브러리가 두 번 보고될 수 있습니다. 패키지 관리자는 레지스트리 이름으로, 핑거프린트 매치는 소스 저장소 이름으로 적기 때문입니다. 둘이 같은 라이브러리를 가리키면 TRUSCA는 패키지 관리자가 찾은 행 하나만 남기고, 핑거프린트 매치를 그 행의 `raw_data.fingerprint_match`에 기록합니다. 기록에는 매치가 보고한 식별자, 버전, 뒷받침한 파일 수, 확인한 라이선스가 들어 있습니다. 버려지는 정보는 없고, 추가 근거가 해당 행에 남아 직접 확인할 수 있습니다.

두 조건이 모두 맞아야 행을 합칩니다. 핑거프린트 서비스는 한 라이브러리에 여러 식별자(저장소, 배포판 패키지, Go 모듈)를 알려 주는데, 그중 하나가 패키지 관리자가 기록한 식별자와 같아야 합니다. 버전도 같아야 하며 앞의 `v`는 무시합니다. 이름이 같다는 사실만으로는 합치지 않습니다. 식별자가 같아도 버전이 다르면 합치지 않습니다. 소스 트리에 그 라이브러리의 사본이 하나 더 있다는 뜻이라, 별도 항목으로 보이는 편이 낫기 때문입니다.
...
### 의무 이행 기록하기 {#fulfilment}

라이선스가 무엇을 요구하는지 아는 것은 절반입니다. 표에서 의무사항 칩을
클릭하면 해당 의무가 열리고, 프로젝트가 실제로 무엇을 했는지 기록할 수
있습니다.

- **상태** — 시작 전, 진행 중, 완료, 해당 없음 중 하나입니다. 아무도 손대지
  않은 의무는 **기록 없음**으로 표시되며, 이는 시작 전과 다릅니다. 하나는
  아무도 확인하지 않았다는 뜻이고, 다른 하나는 확인했지만 아직 시작하지
  않았다는 뜻입니다.
- **기한** — 언제까지 해야 하는지 적습니다.
- **수행한 내용** — 실제로 처리한 사람이 자기 표현으로 적습니다.
- **확인할 위치** — 릴리스 페이지, 저장소의 파일, 티켓 주소를 적습니다. 파일을
  올리지 않고 링크만 두는 이유는, 근거가 어긋날 수 있는 장소를 하나 더 만들지
  않기 위해서입니다.
- **내가 담당하기** — 담당을 가져옵니다. 팀의 다른 사람이 이미 담당인 기록은
  편집한다고 해서 담당이 바뀌지 않습니다.
...
API에서는 `GET /v1/projects/{id}/obligation-fulfilments`가 프로젝트에 기록된
내용을 모두 반환하고, `/v1/projects/{id}/obligations/{obligation_id}/fulfilment`
의 `PUT`과 `DELETE`가 기록 하나를 쓰거나 지웁니다. `PUT`은 읽어 온 버전을 담은
`If-Match` 헤더를 받으며, 그 사이에 다른 사람이 기록을 바꿨으면 412를
반환합니다.

완료로 표시하면 누가 언제 했는지 함께 기록됩니다. 다시 진행 중으로 되돌리면 그
기록이 지워지므로, "완료되지 않았는데 화요일에 끝냈다"는 상태로 남지 않습니다.

팀 관리자뿐 아니라 프로젝트 팀의 구성원이면 누구나 기록할 수 있습니다. 릴리스를
내보내는 사람이 저작자 표시를 추가한 사람인 경우가 많기 때문입니다. 뷰어는
기록을 읽을 수 있지만 쓸 수는 없습니다.

:::note 이행 기록은 NOTICE를 바꾸지 않습니다
생성되는 NOTICE는 라이선스가 요구하는 내용을 담습니다. 이 화면에서 기록한
내용은 NOTICE를 편집하지 않습니다. 이행 기록이 규정 준수 문서를 바꿀 수 있다면,
그 문서가 누군가 바라는 대로 말하게 만드는 통로가 됩니다.
:::
