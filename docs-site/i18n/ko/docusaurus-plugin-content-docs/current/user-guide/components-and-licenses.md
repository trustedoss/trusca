...
| **Concluded** | 다중 생태계 레지스트리 fetcher(Maven Central / PyPI / crates.io / pkg.go.dev / RubyGems / NuGet). `cdxgen`이 의존성의 SPDX id를 전혀 만들지 못했을 때**만** 폴백으로 사용. | 메타데이터가 침묵한 의존성에 대해 레지스트리에서 도출한 라이선스. declared와 detected를 화해한 결과가 *아닙니다* — v0.10.0은 자동 화해(reconciliation)를 수행하지 않습니다. |
