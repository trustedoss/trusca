...
툴바 검색창은 프로젝트의 **이름**, **슬러그**, **git URL**을 함께 찾습니다. 화면에 보이는 목록만이 아니라 포트폴리오 전체를 서버에서 검색합니다. 세 값 중 어느 것이든 일부만 입력하면 되고, 입력은 디바운스를 거쳐 URL(`?search=`)에 반영되므로 걸러진 목록을 새로고침하거나 링크로 공유해도 그대로 유지됩니다.
