...
- **문서 메타데이터** — 스캔이 만드는 SBOM(서명해서 내려받는 그 문서)과 이 페이지가
  내보내는 SBOM 모두 2026 SBOM 최소 요소가 요구하는
  자기 기술을 기록합니다. 생성 시점(`metadata.lifecycles` — 소스 스캔은
  `pre-build`, 컨테이너 스캔은 `post-build`. 인제스트한 공급자 문서를 다시
  내보낼 때는 단계를 적지 않습니다. 남의 문서를 변환한다고 우리가 작성자가
  되지는 않기 때문입니다), 생성 도구와 그 버전,
  [`SBOM_AUTHOR`](../reference/env-variables.md)로 선언한 작성자, 그리고 비어
  있는 필드는 숨긴 것이 아니라 이번 스캔이 확인하지 못한 것이라는 문서 단위
...
보통 특정 릴리스 시점의 SBOM을 요청합니다(예: "2026-01-15에
출고된 것은 무엇인가?"). 과거 스캔 고정()이 적용되기 전까지는
각 릴리스 경계에서 SBOM 산출물을 캡처하여 릴리스 아카이브에
보관하세요. 포털은 *현재* SBOM으로 다루되 *과거* SBOM의 출처로
보지 마세요.
:::

## 정책 프로파일 {#policy-profiles}

기본 내보내기는 가공하지 않은 컴포넌트 목록입니다. 선택적 **정책 프로파일**은
팀의 [라이선스 정책](./components-and-licenses.md#라이선스-분류)을 내보내기에
적용해, 배포용 SBOM이 조직의 기준을 반영하게 합니다.

- **정책 주석(policy annotated)** — 위반 컴포넌트를 제거하지 않고 **그 자리에**
  표시합니다. CycloneDX 컴포넌트에는 `trusca:policy:category` /
  `trusca:policy:spdx` `properties`가, SPDX 패키지에는 `REVIEW` annotation이
  추가됩니다. 금지뿐 아니라 조건부 컴포넌트도 함께 표시되므로, 검토자가 SBOM
  안에서 각 컴포넌트가 왜 문제인지 바로 볼 수 있습니다.
- **정책 필터(policy filtered)** — 금지 컴포넌트를 내보내기에서 **제외**하고
  (그 컴포넌트를 참조하던 취약점 항목도 함께 제거), 제외한 개수를 문서에
  기록합니다(`trusca:policy:excluded_count` 또는 문서 annotation).

팀·조직 정책이 설정돼 있지 않으면 내장 분류 카탈로그로 대체 판정하므로, 금지
GPL은 그래도 표시됩니다.

SBOM 탭에서 프로파일을 선택하거나 API에 `profile`을 전달합니다.
...
- 검출된 라이선스별 섹션 하나씩 — 해당 라이선스의 컴포넌트(`name @ version`) 목록과 컴포넌트별 저작권 라인 포함. SBOM이 코드가 아니라 데이터로 기록한 컴포넌트에는 버전 뒤에 `[dataset]` 표시가 붙습니다. AI SBOM은 학습 데이터와 라이브러리를 한 목록에 함께 싣는데, 데이터셋의 라이선스는 데이터를 규율하므로 따져야 할 의무가 다릅니다.
...
각 컴포넌트 항목에는 스캔 SBOM에 기록된 저작권 문구가 표시됩니다(`cdxgen`이 패키지
...
### 저작권 라인에 저작권자 대신 레지스트리 링크가 표시됨

스캔 SBOM에 해당 컴포넌트의 저작권자가 기록되지 않은 경우입니다 — `cdxgen`이
패키지 메타데이터에서 저작권 문구를 찾지 못했습니다. NOTICE는 라인을 공란으로
두지 않고 저작권자 미수록 안내와 함께 컴포넌트의 레지스트리 URL을 표시하므로,
상위 소스에서 저작권 문구를 직접 확인해 이행하세요. 컴포넌트 드로어의 수동
오버라이드는 [로드맵](#roadmap)에 있습니다. SPDX 내보내기는 별도 표면이며 여전히
`copyrightText: NOASSERTION`을 출력합니다.
...
|------------|----------------|------|
| "릴리스 X 시점의 SBOM을 보여달라" | 수동 아카이브; 포털은 최신본만 보존 | 과거 스캔 고정은 로드맵 |
| "지난 분기에 누가 SBOM / NOTICE를 다운로드했나?" | `structlog`(Loki / journald) — `audit_logs` 아님 | 감사 행 승격은 로드맵 |
| "프로젝트 X에서 GPL이 처음 탐지된 시점은?" | `scans.create`의 `audit_logs` + 스캔별 `vulnerability_findings.create` | 가능 — 전체 증거 체인 보유 |
| "2026 Q1의 모든 승인 결정을 보여달라" | `component_approvals.update`의 `audit_logs` + `decision_note` | 가능 — 전체 증거 체인 보유 |
...
- **취약점 보고서**는 _이미 구현_되어 있습니다 — **PDF**로, v0.13.0부터는 **Excel(`.xlsx`)** 로도 제공됩니다. [취약점 → 보고서 다운로드 (PDF 또는 Excel)](./vulnerabilities.md#보고서-다운로드-pdf-또는-excel)(`GET /v1/projects/{id}/vulnerability-report.pdf` / `.xlsx`) 참고. 아직 **구현되지 않은** 것은 독립형 **컴포넌트 Excel** 내보내기와 **컴플라이언스 PDF**이며 향후 릴리스에서 제공됩니다. 컴포넌트 표가 즉시 필요한 이해관계자는 SBOM(CycloneDX JSON)을 선호 도구로 소비하십시오.
...
- [SBOM 서명 검증 (cosign)](../ci-integration/sbom-signature-verification.md) — SBOM이 온전하며 이 배포에서 서명되었음을 증명
