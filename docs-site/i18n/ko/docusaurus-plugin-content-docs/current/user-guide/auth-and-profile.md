...
2. **설정**을 누릅니다. QR 코드와 그 아래에 같은 값을 글자로도 보여 줍니다.
3. 인증 앱으로 QR 코드를 스캔합니다. 데스크톱 앱을 쓰거나, 카메라 초점이 맞지
   않거나, 화면 낭독기를 쓰는 경우에는 글자로 된 값을 직접 입력합니다.
4. 앱이 보여 주는 여섯 자리를 입력하고 제출합니다.
...
다시 들어온 뒤 `/profile`에서 새 묶음을 발급받습니다. 발급하면 이전 묶음의
코드가 모두 무효가 되므로, 예전에 인쇄해 둔 목록은 누가 주워도 쓸모가 없습니다.
