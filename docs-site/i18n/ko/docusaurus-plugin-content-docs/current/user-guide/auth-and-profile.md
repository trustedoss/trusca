...
성공 시 `/login`으로 리다이렉트됩니다. 새 비밀번호는 bcrypt 해시되고 리셋 토큰은 소비됩니다. 다른 모든 세션은 즉시 끝납니다. refresh token이 폐기되고, 변경 이전에 발급된 access token도 남은 유효 기간을 채우지 못하고 거부되며, 리셋과 같은 시점에 진행되던 로그인이나 토큰 갱신도 뒤에 세션을 남기지 못합니다. 자격 증명이 유출됐다고 판단해 비밀번호를 바꾼 사람이 훔쳐간 세션이 만료되기를 기다릴 필요가 없습니다.
