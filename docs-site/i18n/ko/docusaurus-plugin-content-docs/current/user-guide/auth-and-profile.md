...
- 실패는 **이메일 주소 단위로도** 셉니다. 그래서 여러 출처에 흩어진 추측도 함께 느려집니다. 몇 번 실패하면 그 주소가 잠시 거절되고, 거절이 이어질수록 기간이 길어집니다. 응답은 얼마나 기다려야 하는지 알려 주며, 그 주소에 계정이 있든 없든 내용이 같습니다.

*"Invalid email or password"* 메시지가 보이면 이메일이 정확한지 확인 후 한 번 더 시도하세요. 메시지는 의도적으로 일반화되어 있어 공격자가 계정을 열거할 수 없습니다.

### 기다리라는 안내를 받았다면

비밀번호를 여러 번 틀리면 그 주소가 거절됩니다. 이 횟수는 본인의 시도만 세는 것이 아니어서, 이메일을 아는 사람이라면 누구나 실패를 만들어 넣을 수 있습니다. 돌아오는 방법은 셋입니다.

- **기다리기.** 거절은 반드시 스스로 끝납니다. 다시 거절되려면 시도 한 번이 아니라 처음부터 다시 실패가 쌓여야 하므로, 계속 막아 두려면 상대가 추측을 이어 가야 합니다.
- **비밀번호 재설정.** 횟수가 바로 지워집니다. 비밀번호가 아니라 받은 편지함이 필요하므로, 추측하는 쪽이 멈추지 않아도 쓸 수 있는 경로입니다. 가장 최근에 받은 재설정 메일을 쓰세요. 새로 요청하면 이전 링크가 무효가 되므로 편지함에 남아 있는 예전 메일은 동작하지 않을 수 있습니다.
- **관리자에게 요청하기.** 슈퍼관리자가 대신 횟수를 지울 수 있습니다. 받은 편지함까지 쓸 수 없게 된 경우의 답입니다.
...
비밀번호는 추측당하거나, 다른 곳에서 재사용되거나, 피싱으로 넘어갈 수 있습니다.
2단계 인증을 켜면 로그인에 휴대폰 앱이 보여 주는 여섯 자리 숫자가 함께 필요합니다.
이 숫자는 30초마다 바뀌고 이메일로 오가지 않습니다.

켜기 전에는 적용되지 않으며, 켜는 주체는 본인입니다.

### 켜기

1. `/profile`을 열어 **2단계 로그인** 항목을 찾습니다.
2. **설정**을 누른 뒤 현재 비밀번호를 입력합니다. 이미 로그인한 상태인데도 다시
   묻는 이유는, 여기서 받는 것이 비밀번호를 바꾼 뒤에도 계속 쓰이기 때문입니다.
   열려 있는 브라우저 탭만으로는 근거가 부족합니다.
3. QR 코드와 그 아래에 같은 값을 글자로도 보여 줍니다.
4. 인증 앱으로 QR 코드를 스캔합니다. 데스크톱 앱을 쓰거나, 카메라 초점이 맞지
   않거나, 화면 낭독기를 쓰는 경우에는 글자로 된 값을 직접 입력합니다.
5. 앱이 보여 주는 여섯 자리를 입력하고 제출합니다.

값을 보여 준 다음 코드를 한 번 더 묻는 것은 형식적인 절차가 아닙니다. 앱이 그
값을 실제로 받았다는 것을 코드로 확인하기 전에는 아무것도 켜지지 않으므로,
중간에 탭을 닫아도 이전과 똑같은 상태로 남습니다.

켜는 것은 이후의 로그인을 보호합니다. 이미 열려 있는 세션을 끊지는 않으므로,
다른 사람이 본인 계정으로 로그인해 있다고 생각된다면 이것 말고 비밀번호 재설정을
쓰세요. 재설정은 다른 세션을 즉시 모두 끊습니다.

성공하면 포털이 **복구 코드 10개**를 보여 줍니다. 읽을 수 있는 것은 이때뿐입니다.
해시로 저장하므로 관리자를 포함해 누구도 다시 보여 줄 수 없습니다. 인증 앱이
들어 있는 휴대폰이 아닌 다른 곳에 보관하세요.

### 이후 로그인

1. 이메일과 비밀번호를 이전처럼 입력합니다.
2. 그다음 화면이 코드를 묻습니다. 여섯 자리를 입력합니다.

두 단계 사이에 주어지는 시간은 5분입니다. 더 걸렸다면 비밀번호부터 다시
시작하면 됩니다. 아직 로그인된 것이 없으므로 잃는 것도 없습니다.

코드는 한 번만 통합니다. 이미 쓴 코드는 화면에 아직 같은 숫자가 떠 있는 30초
안에도 거절됩니다. 옆에서 입력을 지켜본 사람이 뒤따라 들어오는 것을 막는 부분이
바로 이것입니다.

### 인증 앱을 잃어버렸다면

여섯 자리를 묻는 자리에 복구 코드를 입력합니다. 하나는 한 번 쓰면 사라지므로
10개는 시도 10번이 아니라 로그인 10번입니다.

다시 들어온 뒤 `/profile`에서 새 묶음을 발급받습니다. 설정할 때와 같은 이유로
비밀번호나 인증 앱의 코드를 먼저 묻습니다. 발급하면 이전 묶음의 코드가 모두
무효가 되므로, 예전에 인쇄해 둔 목록은 누가 주워도 쓸모가 없습니다.

계정에 2단계 인증이 설정되거나 새 복구 코드가 발급되면 포털 알림으로
알려 드립니다. 본인이 하지 않은 알림이 오면 비밀번호를 바꾸고 관리자에게
알리세요. 다른 사람이 본인 계정으로 로그인해 있다는 뜻입니다.

앱도 복구 코드도 없다면 슈퍼관리자가 계정의 2단계 인증을 해제할 수 있고, 해제
사실은 본인에게 알림으로 전달됩니다.

### 적용되지 않는 곳

- **API Key는 그대로 동작합니다.** Key는 이미 아는 것이 아니라 가진 것이고,
  휴대폰 코드를 기다리느라 멈추는 빌드는 끝나지 않는 빌드입니다. Key는 비밀번호와
  같은 무게로 다루고, 비밀번호를 바꾸듯 폐기하세요.
- **OAuth도 코드를 묻습니다.** GitHub나 Google로 로그인하면 공급자 계정은
  증명되지만 2단계 인증은 증명되지 않습니다. 공급자가 포털로 돌려보낸 뒤에
  코드를 묻습니다.
...
2단계 로그인을 켜 두었다면 공급자가 돌려보낸 뒤 코드를 묻고, 그전에는 아무것도 로그인되지 않습니다.
