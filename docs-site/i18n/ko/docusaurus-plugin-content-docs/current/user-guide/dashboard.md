...
그 위에는 다 끝낼 때까지 **Getting started (시작하기)**가 놓입니다. 새로 설치한 곳을 위한 네 단계이고 각 단계가 그 일을 하는 화면으로 이동합니다. 프로젝트 등록, 첫 스캔 실행, 라이선스 정책 설정, CI 키 발급입니다. 각 단계의 체크는 브라우저에 기억해 둔 값이 아니라 서버에서 읽으므로, 동료가 한 일도 내 화면에 반영됩니다. 네 단계를 모두 마치면 카드가 사라지고, 필요 없으면 닫기 버튼으로 영구히 숨길 수 있습니다. 체크의 뜻은 정확히 봐야 합니다. 스캔 단계는 성공한 스캔만 세고, 정책 단계는 직접 정한 정책이 적용된다는 뜻이지 누가 읽었다는 뜻이 아닙니다. 정하기 전까지는 [기본 라이선스 분류](../reference/license-policies.md)가 적용됩니다.
