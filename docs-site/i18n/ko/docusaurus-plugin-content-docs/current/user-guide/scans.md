...
3. **컨테이너 이미지** 참조를 `name:tag` 형식으로 입력합니다. 예: `alpine:3.19` 또는 `ghcr.io/org/app:1.2.3`. 워커가 풀할 수 있는 이미지여야 합니다. 공개 레지스트리는 그대로 동작하지만, 사설 레지스트리는 워커에 아직 자격증명이 없어 풀에 실패합니다. 운영자가 허용 레지스트리를 제한해 둘 수도 있으며(아래 [허용 레지스트리](#허용-레지스트리) 참고), 그 경우 목록에 없는 이미지는 스캔을 시작할 때 거부됩니다.
