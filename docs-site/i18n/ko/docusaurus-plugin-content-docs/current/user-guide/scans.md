...
v2.0.0 UI에는 종류 선택 다이얼로그나 브랜치 오버라이드 필드가 없습니다 — 해당 컨트롤은 v2.1로 이연되었습니다([로드맵](#로드맵-v2x) 참고). 프로젝트 목록 페이지에서 우측 슬라이드 드로어가 열리며 WebSocket 기반의 실시간 진행 뷰가 표시됩니다. 탭을 닫아도 스캔은 워커에서 계속됩니다. 프로젝트를 다시 열면 언제든 재연결됩니다. 스캔이 `queued` 또는 `running` 인 동안 드로어에는 **Cancel scan**(스캔 취소) 동작이 함께 표시됩니다 — [스캔 취소](#스캔-취소) 참고.
