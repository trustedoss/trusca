...
- **자기 레지스트리로만 나갑니다.** 워커는 지금 스캔하는 이미지가 온 레지스트리
  하나만을 호스트 키로 하는 Docker `config.json`을 씁니다. 한 레지스트리의
  자격증명이 다른 레지스트리로 제시되지 않고, 이번 스캔과 무관한 자격증명은 파일에
  아예 쓰이지 않습니다.
- **스캐너의 환경변수에는 들어가지 않습니다.** Trivy에는 환경변수가 아니라 파일로
  전달되므로, 스캐너가 죽으면서 남기는 리포트나 오류 경로에 빠져나갈 자격증명이
  없습니다. 파일은 워커 사용자만 읽을 수 있게 만들고, 백업이 담지 않도록 스캔
  워크스페이스 밖에 두며, 스캔이 끝나면 실패한 경우를 포함해 지웁니다.
