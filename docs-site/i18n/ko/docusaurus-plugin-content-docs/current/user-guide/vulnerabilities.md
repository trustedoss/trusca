...
담당 필터에는 "전체", "내 것", "미배정"이 있습니다. 선택은 URL에 남으므로(`?assignee=me`,
`?assignee=unassigned`) 걸러 놓은 목록을 그대로 공유하거나 북마크할 수 있습니다. CSV
내려받기에도 그대로 적용되며, 이때 "내 것"은 링크를 만든 사람이 아니라 파일을 내려받는
사람을 기준으로 판정합니다.

한 가지 주의할 점이 있습니다. "미배정"은 항목이 실제로 비어 있는 것만 뜻합니다. 담당자가
처리할 수 없는 finding에도 담당자는 있으므로 여기에 걸리지 않습니다. 팀에서 누가 빠진 뒤에는
그런 finding을 따로 훑어보는 편이 좋습니다.
