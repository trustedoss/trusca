...
### 다른 사람의 동의가 필요한 상태 {#transition-approvals}

기본값에서는 권한을 가진 분석자가 혼자 항목을 닫습니다. 조직에 따라 어떤
결과는 한 사람이 정할 일이 아니라고 볼 수 있습니다. 항목을 `Suppressed`나
`Not affected`로 닫으면 취약점을 고치지 않은 채 의무가 끝나기 때문에, 이
결정만큼은 다른 사람이 동의한 뒤에 적용되기를 바라는 팀이 있습니다.

관리자가 [정책] - [빌드 게이트] - [다른 사람의 동의가 필요한 상태]에서 상태별로
켭니다. 기본값은 아무것도 선택되지 않은 상태이고, 선택이 없으면 모든 전환은
지금처럼 한 번의 동작으로 끝납니다.

상태를 선택하고 나면 이렇게 동작합니다.

1. 평소의 전환은 `409`와 사유로 거절되고 항목은 움직이지 않습니다. 대량 전환과 VEX
   문서 가져오기에도 똑같이 적용되므로, 여러 건을 한 번에 바꾸거나 문서를 올리는
   방법으로 우회할 수 없습니다. 가져오기는 해당 구문만 `approval_required` 사유로
   건너뛰고 나머지는 평소대로 반영합니다.
2. 사유를 적고 [동의 요청 보내기]를 누릅니다. 요청만 기록되고 항목은 그대로입니다.
3. 요청은 그 팀을 관리하는 사람들의 [승인] 화면에 나타납니다. 요청한 본인에게는
   버튼 없이 대기 중으로 보입니다.
4. 다른 팀 관리자가 동의하거나 거절합니다. 동의하면 항목이 옮겨지고 직접 전환과
   똑같이 감사 기록이 남습니다. 거절하면 항목은 그대로 두고 거절한 사실이 기록으로
   남습니다.

켜기 전에 알아 둘 제약이 두 가지 있습니다.

- 요청한 사람은 승인할 수 없습니다. 역할이 아니라 사람 단위로 막기 때문에, 관리자가
  한 명뿐인 팀에서는 어떤 요청도 끝낼 수 없습니다. 먼저 관리자를 한 명 더 두세요.
- 한 항목에 열려 있는 요청은 하나뿐입니다. 대기 중에 들어온 두 번째 요청은
  거절되므로, 승인하는 사람이 서로 다른 두 제안 중 무엇을 정하는지 헷갈릴 일이
  없습니다.

요청이 대기하는 동안 항목이 다른 상태로 옮겨졌다면(예를 들어 누군가 다시 열었다면)
그 요청에 동의하는 시도는 `422`로 실패하고 요청은 열린 채로 남습니다. 일어나지
않은 일에 동의한 기록을 남기지 않기 위해서입니다.
...
- **이 매칭의 근거 (Why this match)** — 이 패키지를 영향 대상으로 보고한 권고 피드입니다. 언어 생태계는 GitHub Security Advisory, OS 패키지는 배포판 자체 트래커(Alpine Secdb, Debian Security Tracker, Rocky Linux updateinfo)입니다. 누가 그렇게 주장하는지를 알려 주는 것이고, 개발자가 오탐이라고 할 때 반박의 대상이 되는 것이 이것입니다. 링크는 그 피드의 권고 목록을 열며 이 CVE로 가지 않습니다. 건별 링크는 위의 참고 자료에 있습니다. 피드를 아예 보고하지 않는 스캔도 있고, 그때는 비워 두지 않고 그렇다고 적습니다.
- **영향 (Affected)** — 본 프로젝트의 컴포넌트와 그 버전, 그리고 **수정 버전(fixed version)** — *이 컴포넌트*에 대해 *이 CVE*를 해소하는 버전 — 을 스캔 파이프라인이 판별할 수 있었던 경우 표시합니다. [수정 버전 — CVE를 해소하는 버전](#수정-버전--cve를-해소하는-버전) 참고. 영향 컴포넌트는 **의존성 깊이**()도 함께 표시합니다: 직접 선언한 **직접(direct)** 의존성(깊이 `1`)인지, 다른 패키지가 끌어온 **전이(transitive)** 의존성(깊이 `2+`)인지. 직접 의존성의 CVE는 대개 선언 버전을 올려 본인이 고치고, 전이 의존성의 CVE는 그것을 요구하는 직접 부모를 업그레이드해 고칩니다 — [직접 vs. 전이 (의존성 깊이)](./components-and-licenses.md#dependency-depth) 참고.
...
### 담당자와 기한과 티켓 {#assignment}

finding 에는 누가 고치는지, 언제까지인지, 어디서 추적하는지를 담을 수 있습니다.

- 담당자: 그 프로젝트 팀의 활성 사용자만 지정할 수 있습니다. 팀 밖 사람을 지정하는 것은
  거부합니다. 처리할 수 없는 사람의 이름이 붙으면 아무도 부탁받지 않았는데 주인이 있는
  것처럼 보이기 때문입니다.
- 기한(`due_on`)은 사람이 직접 정하는 달력 날짜입니다.
- 티켓: 외부 추적 시스템의 링크와 식별자입니다. `http://`와 `https://`만 받습니다.

#### 사람이 정한 기한과 정책이 다를 때 {#due-date-precedence}

두 기한이 동시에 있을 수 있고 둘 중 이른 쪽이 이깁니다. 정책보다 이르게 약속할 수는
있지만 정책을 뒤로 미룰 수는 없습니다.

- 정책보다 이른 날짜를 적으면 그것이 기한이 됩니다. 그 날짜에 초과로 넘어갑니다.
- 정책보다 늦은 날짜를 적으면 정책 기한이 그대로 적용됩니다. 적은 날짜는 저장되고
  화면에 남지만, 저장하는 시점에 그 날짜가 적용되지 않는다고 알려 줍니다. 기한을 미뤘다고
  잘못 믿게 두지 않기 위해서입니다.
- SLA 창이 없는 심각도(`info`, `unknown`)에 날짜를 적으면 그것이 해당 finding의 유일한
  기한이 됩니다. 이전에는 그런 finding은 무엇을 적어도 초과가 되지 않았습니다.

기한을 실제로 늦춰야 한다면 그것은 정책의 문제입니다. 심각도별 창을 조정하거나 상태
변경으로 결정을 남기십시오. 기한 항목은 계획을 적는 자리이지 면제를 주는 자리가 아닙니다.

초과·임박·정상 상태와 `sla` 필터와 `sort=sla_due`는 모두 실제로 적용되는 기한을 기준으로
판정합니다. 그래서 `sla_status`가 비는 것은 두 종류의 기한이 모두 없을 때뿐이며, 이전보다
좁은 뜻이 됐습니다.

:::note 기한은 UTC 기준입니다
`due_on`은 날짜이고 그 날의 끝(UTC)에 만료합니다. UTC 서쪽이라면 "7일" 기한인
finding은 현지 7일 오후에 초과로 바뀌고, 동쪽이라면 그만큼 시간을 더 씁니다. 아직 조직별
타임존 설정이 없으므로 날짜를 정할 때 감안하십시오.
:::

#### 담당자가 처리할 수 없게 되면

자격 검사는 지정하는 시점에만 하고 그 뒤에는 다시 하지 않습니다. 나중에 그 사람이
비활성화되거나 팀을 떠나도 배정은 그대로 남습니다. 조용히 지우면 그 일이 보이지 않게 되기
때문입니다. 대신 담당자가 아직 활성 상태인지를 함께 알려 주므로, 처리할 수 없는 사람에게
배정된 상태가 정상 진행처럼 읽히지 않습니다.

사용자를 삭제하는 경우만 이 항목이 비워집니다. 삭제를 막는 대신 그 사람의 finding은
미배정으로 돌아갑니다.

#### 화면에 보이는 상태와 인계 방법 {#assignment-on-screen}

목록의 담당자 열과 드로어의 담당 항목은 세 가지 상태 중 하나를 보여 줍니다. 이 중
무엇이 문제 상황인지 구분해 두면 좋습니다.

| 화면 표시 | 뜻 | 지금 처리할 수 있나 |
|---|---|---|
| 미배정 | 아직 아무에게도 부탁하지 않았습니다. | 누군가 가져가면 가능합니다. |
| 배정됨(자기 것이면 "내가 담당") | 팀의 활성 사용자에게 배정돼 있습니다. | 그 사람이 처리합니다. |
| 담당자가 처리할 수 없음 | 배정은 돼 있지만 그 사람이 비활성화됐거나 팀을 떠났습니다. | 그 사람은 못 합니다. 누군가 인계해야 합니다. |

세 번째 상태는 포털의 오류로 오해하기 쉽습니다. 이것은 오류가 아니라 상태이고,
생기는 경위도 평범합니다. 자격 검사는 배정하는 시점에만 하고 그 뒤에는 다시 하지 않으므로,
3월에 배정한 finding은 담당자가 6월에 퇴사해도 그 이름을 그대로 달고 있습니다. 배정하던
시점에는 잘못된 것이 없었습니다. 오히려 잘못되는 쪽은 이름을 조용히 지우는 것입니다. 그러면
멈춰 있는 일이 손대지 않은 일처럼 보이고, 그 일이 놓였다는 사실을 아무도 모르게 됩니다.

그래서 포털은 이름을 남기고 표시만 붙입니다. 그 표시가 인계하라는 신호입니다.

- 주인이 없는 finding에는 팀 누구에게나 "나에게 배정"이 보입니다.
- 담당자가 처리할 수 없는 finding에는 "비활성 담당자에게서 인계받기"가 보입니다. 같은
  동작이지만 문구를 일부러 다르게 뒀습니다. 하나는 주인 없는 일을 집는 것이고, 다른 하나는
  이미 적혀 있는 이름을 밀어내는 것이라서 같은 행동이 아닙니다.
- 활성 담당자가 있는 finding에는 아무 조작도 제공하지 않습니다. 그 일이 다른 사람 몫이라면
  담당자에게 놓아 달라고 요청하십시오. 남의 일을 말없이 가져가는 것은 아예 제공하지 않는
  것보다 나쁩니다.
- 자기가 담당인 finding은 미배정으로 되돌리거나 "다른 사람에게 넘기기" 목록으로 팀원에게 넘길 수 있습니다. 그 목록에는 그 프로젝트 팀의 활성 사용자가 모두 들어 있고, 이름이 있으면 이름을 없으면 id를 보여 줍니다. 가입할 때 이름은 선택이라 이름이 없는 사람이 있고 그런 사람도 배정 대상이기 때문입니다. 이메일 주소는 보여 주지 않습니다. 동료를 지목하는 목록이 주소를 읽어 가는 자리가 되지 않게 하기 위해서입니다.

  같은 목록이 미배정 finding과 담당자가 처리할 수 없는 finding에도 나옵니다. 그래서 각자 집어 가는 것만이 아니라 일을 나눠 줄 수 있습니다. 활성 담당자가 있는 동안에는 나오지 않습니다. 그 사람에게 놓아 달라고 요청하십시오.

인계받으면 그 자리에서 자기 이름이 올라갑니다. 이 조작은 언제나 자기 자신에게만 배정하므로
남을 대신 세워 둘 수는 없습니다.

#### 내가 맡은 것 찾기 {#assignment-filter}

담당 필터에는 "전체", "내 것", "미배정", "담당자 비활성"이 있습니다. 선택은 URL에
남으므로(`?assignee=me`, `?assignee=unassigned`, `?assignee=inactive`) 걸러 놓은 목록을
그대로 공유하거나 북마크할 수 있습니다. CSV 내려받기에도 그대로 적용되며, 이때 "내 것"은
링크를 만든 사람이 아니라 파일을 내려받는 사람을 기준으로 판정합니다.

"미배정"은 항목이 실제로 비어 있는 것만 뜻하고, 이것은 생각보다 좁습니다. 담당자가 처리할
수 없는 finding에도 담당자는 있으므로 미배정이 아니고 거기에 걸리지 않습니다.

"담당자 비활성"이 그 나머지입니다. 계정이 비활성화된 사람에게 배정된 일입니다. 계정이 닫혀도
배정은 지우지 않습니다. 지우면 누가 그 일을 맡았다는 사실까지 사라지기 때문입니다. 대신
주인이 있어 보이는데 움직이지 않는 행이 남고, 이 필터가 그것을 찾는 방법입니다. 팀에서 누가
...
클러스터는 기본적으로 프로젝트의 **가장 최근 성공** 스캔을 기준으로 계산됩니다. `?scan_id=`를 주면 특정 성공 스캔에 고정할 수 있습니다 — 결과 목록과 같은 스냅샷 의미론이라, 이 프로젝트의 성공 스캔이 아닌 id는 `404`를 반환합니다. 성공 스캔이 없는(또는 미해결 결과가 없는) 프로젝트는 빈 `clusters` 목록과 `total_findings: 0`으로 `200`을 반환합니다. 보기 조회에는 결과 목록과 같은 기준인 `developer` 이상이 필요합니다.
...
Trivy DB가 갱신되어 새 CVE가 도착하면 **자동 재매칭** Celery beat 태스크가 모든 프로젝트의 최신 SBOM을 순회하며 재상관합니다. 새 결과는 자동으로 등장하므로 재스캔이 필요하지 않습니다.
...
- **해당 없음** — 취약 코드 경로가 실행되지 않음을 확신할 때 사용합니다. 분석가가 파일이나 모듈을 짚을 수 있을 때만 쓰고, 신중하게 판단하세요.
- **억제됨** — 다른 상태에 맞지 않는 사유로 명시적으로 침묵할 때 씁니다(예: "내부 보상 통제 적용"). 더 신중하게 쓰고, 사유에 만료일을 명시하는 것을 권장합니다.
- **수정됨** — 컴포넌트를 업그레이드하거나 패치를 적용했을 때 씁니다. 다음 스캔에서 (아마도) 확인되며, 결과를 더 이상 보고하지 않으면 포털이 `Fixed`를 자동으로 closed로 승격합니다.
