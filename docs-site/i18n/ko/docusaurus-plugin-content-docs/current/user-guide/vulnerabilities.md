...
| 사람 이름 | 팀의 활성 사용자에게 배정돼 있습니다. | 그 사람이 처리합니다. |
| 이름과 함께 "처리 불가" | 배정은 돼 있지만 그 사람이 비활성화됐거나 팀을 떠났습니다. | 그 사람은 못 합니다. 누군가 인계해야 합니다. |

세 번째 상태를 포털의 오류로 오해하는 경우가 많습니다. 이것은 오류가 아니라 상태이고,
...
- 주인이 없는 finding에는 팀 누구에게나 "가져오기"가 보입니다.
- 담당자가 처리할 수 없는 finding에는 "인계받기"가 보입니다. 같은 동작이지만 문구를
  일부러 다르게 뒀습니다. 하나는 주인 없는 일을 집는 것이고, 다른 하나는 이미 적혀 있는
  이름을 밀어내는 것이라서 같은 행동이 아닙니다.
...
담당자 필터에는 "내 담당"과 "미배정"이 있습니다. 선택은 URL에 남으므로 걸러 놓은 목록을
그대로 공유하거나 북마크할 수 있습니다. CSV 내려받기에도 그대로 적용되며, 이때 "내 담당"은
