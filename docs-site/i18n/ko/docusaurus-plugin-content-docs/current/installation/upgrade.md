...
각 중간 버전이 forward-only 마이그레이션 경로를 가지는 한, 한 번의 `upgrade.sh` 실행으로 여러 버전 점프가 가능하며 한 번에 2.0.0 → 2.0.5는 지원됩니다. CI는 빈 데이터베이스뿐 아니라, 최신 배포 릴리스에서 개발 최신본으로 올릴 때 API로 데이터를 채운 데이터베이스에서도 마이그레이션 체인을 검증합니다. 옮길 행이 있을 때만 어긋나는 데이터 마이그레이션을 릴리스 전에 잡기 위해서입니다.

래퍼는 다음 두 경우를 아무것도 바꾸기 전에 거부합니다.

- **다운그레이드.** 마이그레이션은 forward-only이므로 `IMAGE_TAG`를 이전 릴리스로 되돌려도 되돌아가지 않습니다. 더 새로운 스키마 위에서 더 오래된 코드가 뜰 뿐입니다. 되돌리려면 [백업을 복원](#롤백)하세요.
- **메이저 건너뛰기.** 예를 들어 1.x에서 3.x로 바로 가는 경우입니다. 각 메이저의 수동 절차는 그 메이저의 릴리스 노트에 있고, 래퍼 자체의 마이그레이션 사전 작업도 메이저 한 단계씩을 전제합니다.

두 검사 모두 실행 중인 백엔드 컨테이너의 이미지 태그와 옮겨 갈 `IMAGE_TAG`를 비교합니다. 둘 중 하나를 `X.Y.Z`로 읽을 수 없으면(사용자 지정 태그이거나 스택이 내려가 있는 경우) 무엇을 봤는지 알리고 업그레이드를 막지 않고 진행합니다. 의도적으로 거부를 넘기려면 `UPGRADE_ALLOW_VERSION_SKIP=true`를 설정하세요.

메이저 점프는 래퍼 호출 전에 릴리스 노트의 "Migration steps" 섹션을 따르세요.

## 컨테이너 스캔 식별자 교정 이후

지금까지 컨테이너 스캔은 이미지에 무엇이 들어 있든 모든 패키지를 Alpine 패키지로 기록했습니다. `pkg:apk/{이름}@{버전}`에 패키지 타입 `apk` 였습니다. Rocky 이미지의 rpm도, Debian 이미지의 deb도, Python 이미지 안의 pip도 모두 잘못된 생태계로 목록에 올랐습니다. 취약점을 놓치지는 않았기 때문에 개수는 맞고 값만 틀린 상태였습니다.

이제는 스캐너가 알려 주는 생태계를 그대로 기록합니다. rpm이나 deb 기반 이미지를 스캔한 적이 있는 배포에 한해 두 가지가 달라집니다.

- 기존에 저장된 행은 다시 쓰지 않습니다. 지난 스캔은 그때 저장한 식별자를 그대로 유지합니다. 그 스캔이 실제로 본 값이기 때문입니다. 교정된 목록을 보려면 이미지를 다시 스캔하세요.
- 다시 스캔하면 그 프로젝트의 취약점 판정이 초기화됩니다. 최초 발견 시각과 분석가가 내린 판정(수용, 오탐 등)은 컴포넌트를 기준으로 이어지는데, 교정된 패키지는 다른 컴포넌트이기 때문입니다. rpm·deb 이미지를 다시 스캔하면 취약점이 새로 발견된 것으로 나오고 판정을 다시 입력해야 합니다.

Alpine 이미지만 스캔해 온 배포는 영향이 없습니다. 그 값은 원래 맞았습니다.
