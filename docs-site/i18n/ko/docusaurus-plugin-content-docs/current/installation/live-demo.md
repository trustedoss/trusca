...
# 권장 — 공개 데모 자격 증명이 매일 밤 바뀌지 않도록 데모 슈퍼 관리자 비밀번호를
# 고정합니다. Secret Manager 에 저장되며 리셋 Job 은 자격 증명을 생성·기록하지
# 않습니다. 미설정 시 Job 이 매일 밤 무작위 비밀번호를 생성하지만 demo 환경에서는
# 평문을 로그에 출력하지 않으므로(마스킹된 안내만) 새 자격 증명을 알 수 없습니다.
# 공개할 수 있는 알려진 값으로 설정하세요.
