...
Helm 차트(`charts/trustedoss`, 차트 버전 **0.2.0**)는 포털 전체를 배포합니다.
...
:::info Dependency-Track은 번들되지 않습니다
이 차트는 Dependency-Track을 배포하지 **않습니다**. `env.dt.url`로 외부 인스턴스를
가리키십시오. 설정하지 않으면 circuit breaker가 OPEN 상태로 유지되며 포털은 캐시된
취약점 데이터를 제공합니다 — [아키텍처 레퍼런스](../reference/architecture.md)를
참고하십시오.
:::
...
| 키 | 기본값 | 용도 |
|---|---|---|
| `image.tag` | `2.0.0` | backend / worker / frontend 이미지 태그(절대 `:latest` 금지). |
...
| `env.dt.url` | `""` | 외부 Dependency-Track URL. 미설정 시 breaker OPEN 유지. |
...
- [아키텍처](../reference/architecture.md) — 서비스, DT circuit breaker, 마이그레이션 모델
