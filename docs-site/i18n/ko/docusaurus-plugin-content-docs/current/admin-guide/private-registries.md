...
- **망 분리 환경에서도 이 문서가 필요합니다.** [오프라인 설치
  번들](./offline-install.md)은 네트워크에 전혀 접근할 수 없는 호스트를 위해
  워커 이미지와 Trivy 취약점 데이터베이스를 패키징하는 문제를 다루며, 이는
  도달은 가능하지만 자격증명이 필요한 사내 미러에 인증하는 문제와는 다릅니다.
  네트워크 구성에 따라 둘 다 필요할 수도, 하나만 필요할 수도 있습니다.
