...
**/admin/disk**는 포털이 신경 쓰는 파일시스템마다 카드 하나를 렌더합니다. 현재 릴리스의 실제 카드는 **workspace**, **trivy_db**, **postgres**, **redis** 입니다(API가 `items: AdminDiskItem[]`으로 반환하고 페이지가 항목당 카드 하나를 렌더). 이전 **dt_volume** 카드는 Dependency-Track 폐기와 함께 제거되었습니다.
