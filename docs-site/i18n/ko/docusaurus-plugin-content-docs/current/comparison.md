...
sidebar_label: 비교
description: TrustedOSS Portal과 상용 SCA(Black Duck, Snyk), Dependency-Track, SW360을 정직하게 비교합니다 — 강점과 현재 한계.
---
...
— v2.4.0부터 Trivy가 단일 취약점 엔진)를 팀·역할·승인·CI 게이트가 포함된 하나의
자체 호스팅 UI로 묶는 것입니다. 아래 비교는 그 아이디어를 세 가지 대표 대안과
견줍니다. 모든 내용은 **v2.4**까지 출시된 기능을 설명하며(진행 중인 v2.x 항목은
별도 표시), 벤치마크가 아니고 다른 프로젝트를 비방하지 않습니다 — 그중 여럿은
TrustedOSS Portal이 직접 활용하는 도구입니다.
...
| | TrustedOSS Portal | 상용 SCA (Black Duck / Snyk) | Dependency-Track | Eclipse SW360 |
|---|---|---|---|---|
...
| 자동 리메디에이션 / PR | 진행 중 (v2.2) | 지원 | 미지원 | 미지원 |
| EPSS 우선순위화 | **출시됨 (v2.1)** | 지원 | 부분 | 미지원 |
| VEX 소비 | **출시됨 (v2.1)** | 지원 | 부분 | 미지원 |
...
- **큐레이션된 취약점·라이선스 인텔리전스.** 상용 벤더는 독점 데이터베이스와 전담
  연구 조직을 운영합니다. TrustedOSS Portal은 Trivy DB로 전달되는 공개
  피드(NVD + OSV + GHSA + EPSS + KEV)에 의존합니다.
- **자동 리메디에이션.** Snyk 등은 수정 PR을 자동으로 생성합니다. TrustedOSS
  Portal에서는 진행 중(v2.2)입니다. finding별 `fixed_version`과 의존성 그래프
  depth는 출시되었고 추천 업그레이드를 구축 중입니다. 현재는 탐지·게이트만 하고
  업그레이드 PR을 아직 생성하지 않습니다.
- **우선순위화 신호.** 1급 EPSS 우선순위화는 출시되었습니다(v2.1) — 컬럼·정렬·
  필터·정책 게이트 임계값. Reachability 분석은 아직 계획 단계입니다.
...
취약점 인텔리전스 플랫폼. TrustedOSS Portal은 v2.3까지 DT를 취약점 엔진으로
사용했고 v2.4.0에서 Trivy로 교체했습니다([ADR-0001](https://github.com/trustedoss/trustedoss-portal/blob/main/docs/decisions/0001-replace-dt-with-trivy.md) 참조).
관건은 어떤 형태의 플랫폼이 팀에 맞는가입니다.

**TrustedOSS Portal이 DT를 직접 운영하는 것과 다른 점:**
...
- **자동 리메디에이션 진행 중.** 탐지·게이트를 하고 finding별 `fixed_version`과
  의존성 그래프 depth(v2.2)를 노출하지만 업그레이드 PR은 아직 생성하지 않습니다 —
  추천 업그레이드를 구축 중입니다(v2.2).
- **취약점 데이터가 Trivy DB에 의존.** 신호는 NVD + OSV + GHSA + EPSS + KEV
  노출분에 더해 1급 EPSS 우선순위화(v2.1 출시), KEV 실환경 배지, 임포트된 VEX로
  보강됩니다.
