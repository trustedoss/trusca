...
**v2.1**까지 출시된 기능을 설명하며(진행 중인 v2.2 항목은 별도 표시), 벤치마크가
아니고 다른 프로젝트를 비방하지 않습니다 — 그중 여럿은 TrustedOSS Portal이 직접
활용하는 도구입니다.
...
- **트리아지 신호.** EPSS는 포털에서 1급 신호입니다(컬럼·정렬·필터·정책 게이트
  임계값). 외부 VEX(OpenVEX / CycloneDX VEX)를 임포트해 finding을 자동 억제할 수
  있습니다.

**Dependency-Track을 직접 쓰는 경우:** 이미 생성한 SBOM에서 취약점 추적만
필요하거나, Dependency-Track의 네이티브 기능 전체를 즉시 쓰고 싶거나, 포털이 아직
노출하지 않은 Dependency-Track 네이티브 기능이 필요할 때입니다. Dependency-Track
직접 사용 대비 동등성 회복은 명시적 로드맵 목표입니다.
...
- **취약점 데이터가 Dependency-Track에 의존.** 신호는 Dependency-Track 노출분에
  더해 1급 EPSS 우선순위화(v2.1 출시)와 임포트된 VEX로 보강됩니다.
