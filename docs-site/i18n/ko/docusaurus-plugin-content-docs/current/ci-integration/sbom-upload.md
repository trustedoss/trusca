...
중첩 컴포넌트도 셉니다. CycloneDX 컴포넌트는 자기 `components` 배열을 가질 수 있습니다. 번들된 의존성이나 이미지 레이어의 패키지가 그렇습니다. 그 안의 항목도 최상위 항목과 똑같이 적재하고 채점하고 매칭합니다. 그래야 합니다. Trivy는 업로드한 파일을 직접 읽어 중첩 컴포넌트에도 취약점을 보고할 수 있는데, 적재하지 않은 컴포넌트에는 그 결과를 붙일 행이 없기 때문입니다.
...
대응표는 문서화 준비를 돕는 자료입니다. 기본 배포판에서 적합성 응답에는 아래 면책 문구가 그대로 실립니다.

TRUSCA는 EU 인공지능법, AI 기본법, EU 사이버복원력법을 비롯한 어떤 규제의 준수 여부도 인증하거나 판정하지 않습니다. 이 대응표는 SBOM이 담을 수 있는 문서화 항목만 다룹니다. 공정성·편향 평가, 위험관리, 인간 감독처럼 SBOM으로 표현할 수 없는 의무는 이 대응표 범위 밖이며, 별도 문서로 이행해야 합니다. 이 표는 문서로 남기지 못한 부분을 눈에 보이게 해 준비를 돕기 위한 것이며, 특정 제품의 법적 의무에 비추어 해석하는 일은 사람이 해야 합니다.
:::
...
취약점 스캐너는 운영체제 패키지(rpm, deb, apk)를 해당 배포판의 어드바이저리 데이터베이스와 대조합니다. 어느 데이터베이스를 쓸지는 패키지 식별자가 아니라 문서 안의 `operating-system` 컴포넌트를 보고 정합니다. 그래서 이미지에 설치된 rpm을 하나도 빠짐없이 담고 PURL도 모두 올바른 SBOM이, 배포판 이름을 적지 않았다는 이유만으로 취약점 0건을 보고할 수 있습니다.

업로드된 SBOM에 배포판 패키지는 있는데 이 컴포넌트가 없으면, TRUSCA가 패키지들에서 배포판을 추론해 그 컴포넌트를 넣은 사본으로 스캔합니다. 무엇을 넣었고 근거가 얼마나 되는지는 스캔 로그에 남습니다.
...
- 업로드한 문서는 고치지 않습니다. 적합성 판정의 대상도, 서명 번들이 제공하는 것도, SBOM 다운로드가 돌려주는 것도 사용자가 보낸 그 문서입니다. 보강한 사본은 스캔 동안에만 존재합니다.
- 배포판은 추론하되 추측하지 않습니다. rpm은 `.elN` 접미사나 `distro=` 한정자에 주 버전을 적고, deb와 apk는 `distro=`에만 적습니다. 아무것도 적지 않은 패키지나 TRUSCA에 매핑이 없는 배포판은 판단 근거로 세지 않고, 그런 문서는 받은 그대로 스캔합니다. 배포판을 잘못 고르면 영향을 받은 적 없는 패키지에 취약점이 붙는데, 이는 아무것도 못 찾는 것보다 나쁩니다.
- 이미 `operating-system` 컴포넌트가 있는 문서는 건드리지 않습니다.

SPDX 업로드도 같은 방식으로 동작하며, 한 가지만 다릅니다. Tag-Value 문서는 다시 쓰지 않으므로 운영체제가 빠진 Tag-Value 업로드는 그 상태로 남습니다. 생성 도구가 운영체제를 적지 않는다면 SPDX-JSON이나 CycloneDX를 쓰세요.
