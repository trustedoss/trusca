...
상황이 해소된 뒤 Git 호스트에서 그 이벤트를 재전송하면 정상적으로 스캔됩니다. 이 두 스킵은 전송을 기록하지 않기 때문에 식별자가 쓰이지 않은 채로 남고, 재전송이 중복이 아니라 새 이벤트로 처리됩니다.

용량 검사가 중복 검사보다 먼저 돌기 때문에, 이미 처리된 전송을 팀이 상한에 걸린 동안 재전송하면 그것도 용량 스킵으로 답합니다. 그 전송은 처음 도착했을 때 이미 스캔된 것입니다. status는 그 커밋이 스캔됐는지가 아니라 이번 요청에서 무슨 일이 있었는지를 말합니다.
...
- 팀이 동시 스캔 상한에 도달했거나 작업 볼륨이 가득 찼습니다(`skipped_team_at_capacity` / `skipped_disk_full`). 이 스킵은 전송 식별자를 쓰지 않으므로, 운영자가 상황을 해소한 뒤 재전송하면 정상적으로 스캔됩니다.
