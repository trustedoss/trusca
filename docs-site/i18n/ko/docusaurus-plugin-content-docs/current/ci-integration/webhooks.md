...
이 둘은 손으로 재전송하지 않아도 됩니다. `WEBHOOK_CAPACITY_RETRY_ENABLED`가 켜져 있으면(기본값) 용량 스킵이 일어나는 그 순간 재시도가 예약되고, 이후로도 간격을 점점 늘려 가며(대략 1분, 2분, 4분 이렇게 늘다가 최대 30분 간격까지) 최대 6회 더 시도합니다. 전체로 보면 한 시간에서 세 시간 정도를 커버하는 셈입니다. 재시도 중 용량이 비는 것을 찾으면 그 자리에서 스캔을 큐에 넣고 전송을 `enqueued`로 기록합니다. 즉, 처음부터 성공한 것과 같은 결과입니다. 모든 시도에서 계속 같은 상황이면 전송은 `capacity_retry_exhausted`로 기록되고, Slack이나 Teams가 설정돼 있다면 어느 전송·어느 프로젝트인지 알려주는 알림이 갑니다.

이 타임라인 어느 시점에서든 Git 호스트에서 재전송하는 방법은 그대로 씁니다. 다음 예약된 시도를 기다리지 않고 바로 반영됩니다. 전송은 사유와 함께 기록되고, 재전송은 그 행을 덮어쓰며 중복으로 읽히지 않습니다. 식별자는 한 번 쓰면 없어지는 번호가 아니라 그 전송의 현재 상태를 가리킵니다. 자체 재전송 도구를 운영 중이라 둘이 겹치는 것을 원하지 않는다면 `WEBHOOK_CAPACITY_RETRY_ENABLED=false`로 끄세요. 어느 쪽이든 수동 재전송에는 영향이 없습니다.
...
- 팀이 동시 스캔 상한에 도달했거나 작업 볼륨이 가득 찼습니다(`skipped_team_at_capacity` / `skipped_disk_full`). 스캔이 시작된 적 없는 전송이므로, 운영자가 상황을 해결한 뒤 재전송하면 정상적으로 스캔됩니다.
