...
:::note 액션 출처
모노레포의 `actions/scan/action.yml` 컴포지트 액션을 `uses: trustedoss/trusca/actions/scan@v0.22.6`로 직접 참조하세요. 독립된 Marketplace 게시는 로드맵에 있습니다.
:::
  ...
  sca:
    ...
    steps:
      ...
      - name: TRUSCA SCA scan
        uses: trustedoss/trusca/actions/scan@v0.22.6
        with:
...
셋 있고, 각각 포털에서 따로 켜는 축입니다(`.env` 수정 후 백엔드 재시작).
...
| `GATE_INCOMPLETE_SCAN_ENABLED=true` | 불완전한 것으로 확인된 스캔입니다. 빌드 준비나 CocoaPods 보완이 열화됐거나, 소스가 의존성 매니페스트를 선언했는데 아무것도 찾지 못한 경우입니다. 이런 스캔은 다른 개수도 같은 이유로 작아지므로 다른 축을 모두 통과합니다. 판정은 [SBOM 내보내기가 밝히는 것](../user-guide/sbom.md#completeness)과 같습니다. |

셋 다 기본은 꺼져 있습니다. 켜면 실패하는 빌드가 달라지므로 운영자가 정할 일입니다.

불완전 스캔 축은 게이트 결과에 `incomplete_scan_outcome`(`complete`, `incomplete`, `unknown`)과 `incomplete_scan_basis`를 담습니다. `incomplete`는 축이 켜져 있으면 항상 빌드를 실패시킵니다. `unknown`(단계 기록보다 먼저 만든 스캔, 업로드한 SBOM, 매니페스트가 없는 소스)은 기본적으로 통과하며, `GATE_INCOMPLETE_SCAN_ON_UNKNOWN=block`을 설정하면 실패시킵니다. SBOM을 모두 직접 생성하는 배포에 맞는 설정입니다.
...
```yaml
- uses: trustedoss/trusca/actions/scan@176bc3f0632bf0cf209c443da308e3d863dfde44  # v0.22.6
```
