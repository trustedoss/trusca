...
재현성을 위해 릴리스 태그 대신 특정 커밋 SHA에 핀:
...
PR 코멘트는 워크플로가 아니라 **포털이 서버 사이드에서** 게시합니다. action이 SCA 결과를 업로드한 뒤 포털이 정책 게이트를 평가하고 — 코멘트 게시가 활성화되어 있다면 — 포털 환경(`GITHUB_TOKEN` 또는 `TRUSTEDOSS_GITHUB_TOKEN`)에 저장된 GitHub PAT로 `https://api.github.com`을 직접 호출합니다. 워크플로의 `secrets.GITHUB_TOKEN`은 포털로 전달되지 않습니다.

포털에 저장된 installation 토큰을 가진 정식 GitHub App은 로드맵에 있습니다.
...
API Key의 `scope`(`org`, `team`, 또는 `project`)가 스캔 대상 프로젝트를 포괄하지 않습니다. 올바른 scope로 재발급. [API keys](../admin-guide/api-keys.md) 참고.
...
- 포털의 `GITHUB_TOKEN` / `TRUSTEDOSS_GITHUB_TOKEN` 환경변수가 미설정·만료되었거나 대상 저장소에 대해 `pull-requests: write` 권한이 없음. 운영자가 포털 `.env`에서 PAT를 회전·연장하고 백엔드를 재기동.
