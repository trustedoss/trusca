...
1. `docs-site/docs/release-notes/X.Y.Z.md`에 릴리스 노트를 두고, `.env.example`의
   `IMAGE_TAG`를 `X.Y.Z`로 올립니다.
2. 태그를 push합니다. `git tag vX.Y.Z && git push origin vX.Y.Z`.
3. `release-gate` 잡을 지켜봅니다. 초록색이 되면 Release가 자동으로 공개되고
