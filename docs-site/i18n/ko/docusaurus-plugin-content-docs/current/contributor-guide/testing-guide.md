...
스키마가 필요한 테스트는 `tests._db_required`를 부릅니다. 이 모듈이 `TRUSCA_TESTS_REQUIRE_DB`를 읽어 한 가지를 정합니다. 데이터베이스가 없는 것이 건너뛸 이유인가, 실패인가.

로컬에서는 이 값이 없으므로 건너뜁니다. 그래서 Postgres 없이도 스위트를 돌릴 수 있습니다. CI에서는 이 값을 켭니다. 거기서는 데이터베이스가 있다고 약속된 환경이므로 없는 것이 잘못입니다.
