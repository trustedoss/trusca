...
- **도구가 아무것도 찾지 못한 경우.** 결과만으로는 찾을 것이 없었는지 그곳을 보지 않았는지 구분되지 않습니다. 저장소 린트를 엉뚱한 트리에서 돌리면 열어 보지도 않은 파일에 대해 0건을 보고합니다. 구조가 있는 출력을 파싱하지 않고 정규식으로 훑을 때 특히 그렇습니다. `gh`도 `EXPLAIN`도 JSON을 돌려주는데, `lint (backend)` 같은 이름이나 `SubPlan` 아래 중첩된 계획 노드는 손으로 만든 패턴이 기대하는 모양과 다릅니다. 구조를 파싱하거나 정확한 문자열로 비교하고, 값 안의 괄호·대괄호·점이 정규식에서는 다른 뜻이라는 것을 기억하십시오.
  ...
  소스에서 무언가를 셀 때도 같습니다. 헬퍼의 반환 타입을 고치느라 테스트 파일에서 5요소 튜플을 전부 찾아야 했는데, `grep`은 셋을 찾았고 실행은 여전히 실패했으며 `ast`로 훑으니 다섯이었습니다. 놓친 둘은 패턴이 예상하지 못한 모양으로 쓰여 있었고 그중 하나는 두 번째 헬퍼 함수 안에 있었습니다. 패턴은 내가 상상한 표기를 찾고, 파싱은 실제로 거기 있는 것을 찾습니다. 이 저장소는 이미 여러 곳에서 AST로 단언하고 있으니 새로 꺼내는 도구도 아닙니다.
