...
상용 제품과 달리 사용자 수에 따른 라이선스 비용 없이 운영할 수 있습니다.

## 시작 지점

TRUSCA가 처음이라면 권장 경로를 따르십시오. 각 단계는 앞 단계를 바탕으로 이어집니다.

1. **5분 안에 체험**: [Quickstart](./quickstart.md)에서 몇 개의 명령으로 데모 데이터셋과 함께 실행합니다.
2. **실제 프로젝트 첫 스캔**: [Quickstart 5단계](./quickstart.md#first-real-scan)에서 데모 스택 그대로 프로젝트를 등록하고 자기 코드를 스캔합니다.
3. **프로덕션으로 이행**: [Docker Compose](./installation/docker-compose.md) 또는 [Helm 차트](./installation/helm.md)를 사용합니다.
...
| 다국어 | 영어·한국어. UI·오류 메시지·본 문서 모두 지원합니다. |

## 제공하지 않는 기능

- **SAST 스캐너 아님.** 자체 작성한 코드의 정적 분석은 다루지 않습니다. 본 포털은
  제3자 컴포넌트에 집중합니다.
- **취약점 데이터베이스 아님.** Trivy DB를 통해 NVD, OSV, GHSA, EPSS, KEV를
  소비할 뿐, 직접 큐레이션하지 않습니다.
