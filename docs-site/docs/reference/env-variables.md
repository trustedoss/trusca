...
| `REQUIRE_DB_ROLE_SEPARATION` | `false` | `core/db_role.py` | When `true`, refuses to start (rather than warning) if the connected role still holds DDL privileges. See [Hardening](../admin-guide/hardening.md) for the full L1 role-separation setup. |
...
| `SCAN_SCOPE_FILTER_NON_DEPLOYABLE_ENABLED` | `true` | `config.py` | Path half of the scope filter: drops a component whose every manifest sits under a `test`/`tests`/`examples`/`demo`/`benchmark`-style directory (full list in the user guide). Disable when a repository ships code from such a directory. Part of the scan-reuse fingerprint, so flipping it never reuses an earlier scan. |
...
| `LICENSE_FETCH_SCAN_BUDGET_SECONDS` | `300` | `config.py` | Seconds of registry lookups one scan may spend on license enrichment (cache hits are free and do not count). When it is spent the remaining components stay license-unknown, the scan still succeeds, and `scan_metadata.license_enrichment` records how many lookups were not made. Those components are not cached as a miss, so the next scan of them continues the lookups. The check runs between lookups, so a scan can overrun by one lookup. A non-numeric or non-positive value falls back to the default. |
| `LICENSE_FETCH_CONSECUTIVE_FAILURE_LIMIT` | `30` | `config.py` | After this many consecutive lookups that got no answer from the registry (connection error, timeout, 429 or 5xx after every retry), a scan stops looking up licenses. A registry replying "not found" (404) is answering and resets the count. The skipped lookups are recorded in `scan_metadata.license_enrichment` and are not cached. A non-integer or non-positive value falls back to the default. |
| `LICENSE_FETCH_MAVEN_PARENT_MAX_DEPTH` | `3` | `config.py` | A Maven component whose own POM declares no `<licenses>` takes them from its `<parent>` POM, and that one from its own parent. This is how many parent links a lookup follows. Reaching the limit leaves the component license-unknown; it is not an error. `0` turns inheritance off and reads only the component's own POM. A parent shared by several components is looked up once and cached. The license row records the ancestor it came from in `raw_data.inherited_from`. A non-integer or negative value falls back to the default. |
...
| `QUEUE_BACKLOG_ALERT_ENABLED` | `false` | `config.py` | Whether the S6 beat sweep that turns a sustained Celery-queue backlog into a Slack/Teams alert runs. A Compose deployment has no autoscaler layer, so this is the signal the product gives instead - see [Docker Compose - Scan capacity](../installation/docker-compose.md#scan-capacity-sizing-and-scaling) for the formula it pairs with. Hard dependency on `QUEUE_BACKLOG_METRICS_ENABLED`: turning this on while that is off does not error, it degrades to a per-tick skip logged at WARNING. |
...
- [Hardening](../admin-guide/hardening.md) - a curated, task-oriented tour of the security-relevant keys above.
- [Postgres sizing and connection tuning](../admin-guide/postgres-tuning.md) - the connection-budget formula behind `DB_POOL_SIZE` / `DB_MAX_OVERFLOW` / `DB_SYNC_*`.
