...
| `LOGIN_THROTTLE_FAILURES` | `5` | `config.py` | Consecutive failures for one address before it is refused. Five rather than three because the per-IP budget is already 5/minute, so one person mistyping meets that first. |
