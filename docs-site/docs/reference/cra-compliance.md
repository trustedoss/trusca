...
| **§2 — Address and remediate vulnerabilities without undue delay** | Findings carry fixed-version data; remediation dry-run and pull-request generation propose the upgrade. Per-severity **remediation-SLA tracking** ages every finding from its first detection against a due date, with an overdue filter and daily breach alerts. The CI build gate fails the build (`exit 1`) on Critical CVEs / forbidden licenses so regressions do not ship. | [SLA tracking](../user-guide/vulnerabilities.md#sla), [Remediation PR](./remediation-pull-request.md), [dry-run](./remediation-dry-run.md) |
...
- **The SLA windows are your policy, not the regulation's.** TRUSCA now tracks
  each finding's age from first detection against per-severity remediation
  windows, with an overdue filter and breach alerts — see
  [Remediation SLA and aging](../user-guide/vulnerabilities.md#sla). But the
  windows are operator-tuned defaults, and whether a given delay is "undue"
  under the CRA is a judgement the portal cannot make for you.
...
6. Set the **per-severity SLA windows** (`VULN_SLA_DAYS_*`) to your internal
   remediation policy, work the overdue queue (`?sla=overdue`), and keep the
   breach alerts on — the finding-age evidence for "without undue delay".
