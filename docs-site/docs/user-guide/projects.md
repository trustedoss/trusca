...
The **Projects** sidebar entry lands on the project list — every project your team memberships reach, with status badges, severity counts, an inline **Scan** action, and a compact per-project meta row showing `n scans · m releases · last scan <relative time>`:
...
The toolbar search box matches a project's **name**, **slug**, or **git URL**, and runs on the server against your whole portfolio — not just the projects currently on screen. Type at least part of any of the three; the term is debounced and mirrored into the URL (`?search=`), so a filtered list is reload-safe and link-shareable.

The page renders one server page at a time. If more projects match than fit on it, a notice above the list says how many matched and how many are shown — narrow the search to reach the rest.
...
The default branch (`main`), visibility (`team`), and owning team (the team selected in the global bar) are set server-side and can be reviewed from **Project Settings**.
...
## The governance band {#governance-band}

Between the project header and the tabs sits a five-tile strip that summarises where the project stands, so reading it costs no navigation while you work inside any tab:

| Tile | What it says |
|---|---|
| **Risk score** | The Overview tab's overall score, from the same computation — the worse of the security and licence axes. |
| **Build gate** | What CI would decide right now: **Pass**, **Blocked** (with the blocking counts, linking to the Vulnerabilities tab), or **Never scanned**. |
| **KEV deadlines** | Known-exploited findings against their CISA remediation date, split into overdue and due within a week. |
| **Approvals waiting** | Component approvals still pending or under review for this project. |
| **Critical trend** | The critical count across the last few succeeded scans, with the change in words. Fewer than two scans draws no line — one point is not a trend. |

Every number is produced by the service that owns it elsewhere in the product, so the band cannot disagree with the tab below it. Two states are deliberately distinct: a project with **no successful scan** shows "Never scanned" rather than a passing gate, and a band that fails to load says so instead of rendering zeros — an absent answer, not a reassuring one.
