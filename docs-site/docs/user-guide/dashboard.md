...
Above them, until it is finished with, sits **Getting started**: four steps for a new deployment, each linking to the screen that does it. Register a project, run the first scan, set a licence policy, issue a CI key. Each step's tick is read from the server, not remembered in your browser, so a colleague who did the work is credited on your screen too. The card disappears once all four are done, and a close button hides it for good if the list is not for you. Note what the ticks mean: the scan step counts only a scan that *succeeded*, and the policy step means a policy of your own applies, not that anyone has read it. Until you set one, the [built-in licence categories](../reference/license-policies.md) apply.
