...
- **Malicious** — a red **MALICIOUS** badge when the component is a known-malicious package; blank otherwise. Blank does not mean safe — see [Known-malicious packages](#known-malicious-packages).
...
- **Flagged only** — a toggle that narrows the table to known-malicious packages (`?malicious=true`). See [Known-malicious packages](#known-malicious-packages).
...
A CVE says an honest package has a flaw you can patch. A **malicious package**
was published to attack whoever installs it — a typosquat of a popular name, a
hijacked maintainer account, an install-time payload. The response is
different, so the signal is kept separate from the severity table: you remove
the package and rotate every credential the build could reach. Upgrading to a
later version does not help, because there is no honest version to upgrade to.

Every scanned component is matched against a snapshot of the `MAL-` advisories
published through [OSV](https://osv.dev) by the OpenSSF Package Analysis
project. The snapshot ships **with the release**, so scans make no network
calls and this works air-gapped.

Matching is by package identifier (purl) only, never by name. That is
deliberate: malicious packages are named to resemble real ones, so a name
match is exactly the wrong tool.

How to read the surfaces:

- **Malicious column / badge** — the red **MALICIOUS** badge appears only for
  a flagged component. The drawer adds the advisory id and the snapshot the
  verdict came from.
- **Overview chip** — a flagged component puts a chip at the top of the
  Overview tab, ahead of the Critical CVE count. A CVE is a defect to schedule
  a fix for; this is an attack already running inside the build.
- **Filter** — the **Flagged only** toggle narrows the table
  (`?malicious=true`).
- **This is not a vulnerability** — no finding is created and the count never
  enters severity totals or the risk score.

**Blank does not mean safe.** A blank cell means one of two things: the
snapshot was consulted and did not list the package, or the package was never
assessed (no snapshot, or `MALICIOUS_ENABLED=false`). Every verdict carries
its snapshot date for the same reason — the claim is "not in this snapshot",
never "safe today".

**If a flag is wrong**, the correction belongs upstream: challenge the
advisory at [ossf/malicious-packages](https://github.com/ossf/malicious-packages).
...
| `conditional` | **Conditional** | Triggers the [approval workflow](./approvals.md). Build proceeds — including after a **Rejected** verdict; see the [approvals caveat](./approvals.md#rejected-verdict). | LGPL-2.x, LGPL-3.x, MPL-2.0, EPL-1.x, EPL-2.0, CDDL-1.0, CC-BY-SA-4.0, CC-BY-ND-4.0 |
| `forbidden` | **Forbidden** | Build gate exits 1 in CI. | AGPL-3.0, GPL-2.0, GPL-3.0, SSPL-1.0, BUSL-1.1, CC-BY-NC-4.0 and its ShareAlike / NoDerivatives variants |
...
| **Concluded** | The multi-ecosystem registry fetcher (Maven Central / PyPI / crates.io / pkg.go.dev / RubyGems / NuGet), used when `cdxgen` produced no SPDX id for a dependency, and to re-check one that would block the build (a package offering a choice of licences is often recorded with only one of them). | A registry-derived license for a dependency whose own metadata was silent. It is *not* the result of reconciling declared and detected — v0.10.0 does not perform automatic reconciliation. |
