...
This page is about **exporting** an SBOM that TRUSCA generated from a scan. To go the other way — **upload** an SBOM your own tooling already produced (CycloneDX or SPDX) so TRUSCA matches CVEs and scores its conformance — see [Upload an SBOM](../ci-integration/sbom-upload.md) and [Scans → SBOM upload](./scans.md#received-sboms-uploaded).
:::
...
- **Document metadata** — the SBOM a scan produces (the one that is signed and
  ...
  because converting someone else's document does not make us its author), the
  tool and its version, the author when [`SBOM_AUTHOR`](../reference/env-variables.md)
  declares one, and a document-level statement that any empty field is one this
  scan could not establish rather than one being withheld. A component whose
