...
Nested components count. A CycloneDX component may declare its own `components` array — a bundled dependency, an image layer's packages — and every entry in it is persisted, scored and matched exactly like a top-level one. It has to be: Trivy reads the uploaded file directly and can raise a finding against a nested component, and a component we never persisted has no row for that finding to attach to.

"Package components" means every component except `"type": "data"` and `"type": "file"`. Neither can answer a package question — a file on disk has no package version, and purl defines no type for one — so counting them would report a document as short on fields its entries cannot hold. File entries are asked instead for the identifier they can carry, by the recommended `file-hash` check. The coverage details name how many components were measured and how many sat outside, because a percentage alone cannot tell you whether it was measured over two components or fifty.

This verdict is a quality bar, not evidence that a document was not tampered with. It judges what the SBOM says about the components it lists; it cannot see a component that was left out, or one re-typed as a file to keep it out of the package checks. A document whose components are ALL of the excluded types does not pass on the empty denominator: it has identified no package, and the mandatory checks exist to answer questions that need one, so both are downgraded to a warning. `file-properties` has one extra behavior: when **no** component in the document carries the property trio, no producer in the chain inspected the delivered files, so the check reports `requires inspecting the delivered files (no automated source in this scan)` with `source: "na"` — a human-review item, not a coverage failure.
