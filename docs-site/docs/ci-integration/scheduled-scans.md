...
The contract is opt-in both ways: with no schedule configured anywhere, nothing scans on its own — a fresh install behaves exactly as it did before this feature existed. Configuration in this release is **API-only**; there is no settings-page UI yet.
