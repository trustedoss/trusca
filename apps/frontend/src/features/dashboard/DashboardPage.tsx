 ...
 * `Navigate to="/projects"` redirect: landing on a list makes the operator
 * assemble the portfolio view in their head before they can act on it.
 *
      ...
      {isAnyError ? (
        ...
        <>
          {/* The queue leads. A distribution is context; a queue is work, and
              reading order is the strongest statement a layout makes about
              which of the two the reader should deal with first. */}
          <SavedSearchesPanel />
        ...
        <section className="px-6 pt-4">
          ...
          {/* Then the direction of travel. The queue says what is waiting;
              this says whether the pile is growing — the question a report
              printed once cannot answer, because answering it needs history
              the tool has to have kept. */}
          <section className="px-6 pt-4">
            <TrendsPanel />
          </section>
          ...
          <section
            // `grid-cols-1` below `lg` for the reason spelled out in
            // OverviewTab: an implicit template sizes tracks to max-content,
            // so a card with a wide row inside stops shrinking and pushes the
            // page past the viewport (501 px on a 390 px phone).
            className="grid grid-cols-1 gap-4 px-6 pb-6 lg:grid-cols-3"
            ...
            >
              ...
              <CardContent className="p-0">
                {scansQuery.isLoading ? (
                  ...
                  // G0-5 — the five-column table needs 474 px of min-content
                  // (project name + four cells) and the card is 340 px wide on
                  // a 390 px phone, so it pushed the card, the grid section and
                  // the page shell past the viewport: all four of the
                  // dashboard's frozen narrow-viewport spills were this one
                  // table. A scroll container is the honest answer for a data
                  // table on a phone — the alternative is truncating scan
                  // status and duration into uselessness — and the gate treats
                  // a declared scroller as a decision rather than a defect.
