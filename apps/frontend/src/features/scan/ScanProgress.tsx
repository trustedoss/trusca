...
export function ScanProgress({
  ...
  return (
    <div className="flex flex-col gap-4" data-testid="scan-progress">
          ...
          return (
            <li
              ...
              className={cn(
                ...
                isFailedAtThisStep && "border-risk-critical/40 text-risk-critical-foreground",
              )}
              data-step={s}
              data-state={
                isCompleted
      ...
      {!terminal && gaveUp ? (
        <StreamStopped closeCode={closeCode} onReconnect={reconnect} />
...
/**
 * What the panel says once the stream has stopped for good (C4).
 *
 * Before this it went on saying "Reconnecting… Attempt 14" for as long as the
 * page stayed open, because the hook's give-up was invisible to it. Two
 * things a reader needs at that moment: why it stopped, and whether their
 * scan died with it. It did not, and that sentence is here every time.
 *
...
function StreamStopped({
  ...
  return (
    <div
      ...
      data-testid="scan-progress-stopped"
      ...
      {retryable ? (
        <div>
          <Button
            ...
            size="sm"
            onClick={onReconnect}
            data-testid="scan-progress-reconnect"
          >
