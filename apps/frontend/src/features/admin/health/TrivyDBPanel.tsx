...
/**
 * TrivyDBPanel — admin/health Trivy DB status panel.
 *
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        />
        <KpiTile
          // G0-5 — the label used to interpolate the same relative time the
          // tile renders below it, inside a sentence that assumed the future:
          // "Next refresh in 5 days ago", with "5 days ago" repeated
          // underneath. A scheduled refresh that has not happened is exactly
          // the case this panel exists to show (the stale badge next to it says
          // so), so the label states the field and the value states when —
          // which is also what the sibling feeds' tiles do ("Next sync",
          // "Next tick").
