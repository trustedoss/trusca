...
export function OverviewTab({
  ...
  return (
    <div
      ...
      // grid cells.
      // `grid-cols-1` is load-bearing below `md`, not a default being spelled
      // out. Without an explicit column template a grid sizes its tracks to
      // `auto`, which is max-content: a card holding a long purl or a wide
      // table stops shrinking and takes the page with it — 516 px of content
      // on a 390 px phone. `grid-cols-1` is `repeat(1, minmax(0, 1fr))`, and
      // the `0` is the part that matters.
      className="grid grid-cols-1 items-start gap-4 p-6 md:grid-cols-2"
    >
      ...
      {/* #26 — known-malicious KPI. First among the attention chips, ahead
          ...
          Destructive tone, and the body text says what to do rather than
          restating the count. Rendered only when the anchored scan carries
          flagged components — a zero here could equally mean "nothing was
          assessed", so it is never drawn as reassurance. */}
      {data.malicious_count > 0 ? (
        <Alert
          variant="destructive"
          className="md:col-span-2"
          data-testid="overview-malicious-chip"
          data-malicious-count={data.malicious_count}
        >
          <AlertTriangle className="h-4 w-4" aria-hidden />
          <AlertDescription className="flex flex-wrap items-baseline gap-2">
            <span className="font-semibold">
              {t("overview.malicious_chip.title", {
                count: data.malicious_count,
              })}
            </span>
            <span>{t("overview.malicious_chip.body")}</span>
            <button
              type="button"
              className="underline underline-offset-2 hover:no-underline"
              data-testid="overview-malicious-chip-link"
              onClick={() =>
                setSearchParams(
                  (prev) => {
                    const next = new URLSearchParams(prev);
                    next.set("tab", "components");
                    next.set("malicious", "true");
                    return next;
                  },
                  { replace: false },
                )
            ...
            >
              {t("overview.malicious_chip.link")}
            </button>
          </AlertDescription>
        </Alert>
      ) : null}
      ...
      {data.outdated_count > 0 ? (
        <Alert
          className="border-status-info-border bg-status-info-subtle text-status-info-foreground md:col-span-2"
      ...
      <Card className="md:col-span-2" data-testid="overview-recent-scans-card">
        <CardHeader>
          ...
          {/* G0-5 — the subtitle is a claim about content ("Last five scans
              for this project"), so it has to go when there is no content.
              On a project that has never been scanned the card read "Last five
              scans for this project." and then, immediately below, "No scans
              have been recorded yet." — the description promising five of a
              thing the body says there are none of. */}
