  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      >
        ...
        {/*
          * W2 #31 — commercial-SCA-style Type + Usage rows.
        ...
        {/*
          * #26 — known-malicious row. Rendered ONLY when flagged, unlike the
          * EOL/currency rows below: those say "—" for the common case because
          * their absence is unremarkable, while a row reading "Malicious: —"
          * on every healthy package would train the eye to skip exactly the
          * line that matters. The response text is an instruction, not a
          * label — an upgrade is the wrong action here.
          */}
        {detail.malicious_state === "flagged" ? (
          <div
            className="flex flex-col gap-1 rounded-md border border-destructive/40 bg-destructive/5 p-2 text-xs"
            data-testid="component-drawer-malicious"
          >
            <div className="flex items-center gap-2">
              <span className="uppercase tracking-wide text-muted-foreground">
                {t("drawer.malicious_label")}
              </span>
              <MaliciousBadge
                maliciousState={detail.malicious_state}
                maliciousId={detail.malicious_id}
                maliciousSource={detail.malicious_source}
                showAdvisory
              />
            </div>
            <p className="text-muted-foreground">
              {t("drawer.malicious_action")}
            </p>
            {detail.malicious_source ? (
              <p className="font-mono text-[10px] text-muted-foreground">
                {t("drawer.malicious_source", {
                  source: detail.malicious_source,
                })}
              </p>
            ) : null}
          </div>
