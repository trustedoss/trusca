...
export function LicenseDistributionChart({
  ...
  return (
    ...
    >
      {/* `img` while the bar is decoration, `group` once its segments are
       *  buttons. An element with role="img" that contains focusable children
       *  is what axe's nested-interactive rule flags: it promises a single
       *  graphic and then hands the reader controls inside it. `group` says
       *  what the element actually is — a set of related controls — and each
       *  segment already carries its own label. */}
      <div
              ...
              if (interactive) {
                return (
                  <button
                    ...
                    className={cn(
                      segmentClass,
                      // W11-F polish — matches SeverityDistributionChart so
                      // both charts share the W11-A motion token vocabulary.
