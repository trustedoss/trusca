...
function GraphCanvas({
  ...
  useEffect(() => {
    ...
    // `theme` is in here because `themeColors()` reads the custom properties
    // once, when the graph is built. Cytoscape draws to a canvas, so nothing
    // re-resolves a `var()` afterwards the way the SVG charts do — without
    // this dependency the graph keeps the previous theme's palette until the
    // next data change, which on a settled page is never (W18).
    //
    // Rebuilding rather than restyling costs a layout pass, but the dagre
    // layout is deterministic: the same graph lands in the same place, so a
    // theme switch does not shuffle what the user was looking at.
  }, [elements, nodeById, theme]);
