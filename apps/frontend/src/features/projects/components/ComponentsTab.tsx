...
function getComponentColumnsCatalog(
  ...
  return [
    ...
    { id: "malicious", label: t("components.col.malicious") },
...
export function ComponentsTab({ projectId, scanId }: ComponentsTabProps) {
  ...
  const [search, setSearch] = useState(() =>
    readTabSearchParam(searchParams, COMPONENTS_SEARCH_PARAM),
  );
  ...
  // W2 #31 — Direct/Transitive 3-state and commercial-SCA-style "Usage" multi-select.
  ...
  // #26 — known-malicious-only toggle. `?malicious=true` when on; "off" means
  // "no opinion" (both buckets), never `?malicious=false`. Same boolean shape
  // as the EOL/currency facets even though the underlying axis differs: this
  // one is an attack, not a maintenance signal.
  const [maliciousOnly, setMaliciousOnly] = useState<boolean>(
    () => searchParams.get("malicious") === "true",
  );
  ...
  useEffect(() => {
    setSearchParams(
      (prev) => {
        const next = new URLSearchParams(prev);
        writeTabSearchParam(next, COMPONENTS_SEARCH_PARAM, debouncedSearch);
        ...
        // #26 — `?malicious=true` only when the toggle is on.
        if (maliciousOnly) next.set("malicious", "true");
        else next.delete("malicious");
  ...
  const filters = useMemo(
    () => ({
      ...
      malicious: maliciousOnly ? true : null,
      sort,
      order,
      pageSize: PAGE_SIZE,
      scanId,
  ...
  return (
    <div data-testid="components-tab" className="flex flex-1 flex-col">
      ...
      <ComponentsToolbar
        ...
        onMaliciousOnlyChange={setMaliciousOnly}
        severity={severity}
      ...
      {!components.isLoading && !components.isError && items.length > 0 ? (
        // The row + header together need a pinned inner width to render every
        // column without squeezing the Name cell (flex-1) to zero — narrower
        ...
        // instead of column-hiding (the user explicitly wants all the
        ...
        <div className="flex flex-1 flex-col overflow-x-auto">
          <div
            ...
            role="table"
            aria-label={t("components.table_aria", {
  ...
  return (
    ...
    >
      ...
      <span
        ...
        role="columnheader"
        aria-sort={ariaSortFor("name", currentSort)}
      >
        <SortableColumnHeader
          column="name"
      ...
      {visibleColumns.has("type") ? (
        <span
          className="w-24"
          role="columnheader"
          data-testid="components-header-cell-type"
        >
          {t("components.col.type")}
        </span>
      ) : null}
      <span className="w-24 text-right" role="columnheader">
        {t("components.col.version")}
      </span>
      {visibleColumns.has("license") ? (
        <span
          className="w-28"
          role="columnheader"
          aria-sort={ariaSortFor("license", currentSort)}
          data-testid="components-header-cell-license"
        >
          <SortableColumnHeader
            column="license"
      ...
      {visibleColumns.has("policy") ? (
        <span
          className="w-24"
          role="columnheader"
          data-testid="components-header-cell-policy"
        >
      ...
      {visibleColumns.has("usage") ? (
        <span
          className="w-24"
          role="columnheader"
          data-testid="components-header-cell-usage"
        >
          {t("components.col.usage")}
        </span>
      ) : null}
      {visibleColumns.has("malicious") ? (
        <span
          className="w-20"
          role="columnheader"
          data-testid="components-header-cell-malicious"
        >
          {t("components.col.malicious")}
        </span>
      ) : null}
      {visibleColumns.has("eol") ? (
        <span
          className="w-16"
          role="columnheader"
          data-testid="components-header-cell-eol"
        >
      ...
      {visibleColumns.has("currency") ? (
        <span
          className="w-20"
          role="columnheader"
          data-testid="components-header-cell-currency"
        >
      ...
      {visibleColumns.has("severity") ? (
        <span
          ...
          data-testid="components-header-cell-severity"
        >
          <SortableColumnHeader
            column="severity"
...
function ComponentRow({
  ...
  return (
    <div
      ...
      // G0-5 backlog — a real row. `aria-rowindex` counts the header, so the
      // first data row is 2, and it indexes the whole result set rather than
      // the rendered window. The click moved here from the button that used to
      // wrap most of the cells: a `row` owns `cell`s, and that button was not
      // one. The keyboard path is the name cell's button now.
      role="row"
    ...
    >
      <span
        className="flex flex-1 min-w-[180px] items-center gap-2 truncate"
        role="cell"
      >
        {/* G0-5 — the keyboard path into the drawer, one control in the
            identifier cell instead of a button wrapping nine of them. */}
        <button
          type="button"
          ...
          className="truncate rounded-sm font-medium hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1"
        >
          {component.name}
        </button>
        {component.purl ? (
      ...
      {visibleColumns.has("type") ? (
        <span className="w-24" role="cell" data-testid="component-row-cell-type">
          <DependencyTypeBadge
      ...
      {visibleColumns.has("policy") ? (
        <span className="w-24" role="cell" data-testid="component-row-cell-policy">
          <LicenseCategoryBadge category={component.license_category} />
        </span>
      ) : null}
      {visibleColumns.has("usage") ? (
        <span className="w-24" role="cell" data-testid="component-row-cell-usage">
      ...
      {visibleColumns.has("malicious") ? (
        <span
          className="w-20"
          role="cell"
          data-testid="component-row-cell-malicious"
        >
          <MaliciousBadge
            maliciousState={component.malicious_state}
            maliciousId={component.malicious_id}
            maliciousSource={component.malicious_source}
          />
        </span>
      ) : null}
      {visibleColumns.has("eol") ? (
        <span className="w-16" role="cell" data-testid="component-row-cell-eol">
          <EolBadge
      ...
      {visibleColumns.has("currency") ? (
        <span className="w-20" role="cell" data-testid="component-row-cell-currency">
          <CurrencyBadge
      ...
      {visibleColumns.has("severity") ? (
        <span className="w-24" role="cell" data-testid="component-row-cell-severity">
