...
/**
 * SLA filter dropdown options (X1). `""` is the "any" / off state; the three
 * tokens map to the backend's single-valued `?sla=` parameter. Sourced from
 * the contract-tested vocabulary mirror so the select can never offer a
 * token the backend rejects.
 */
export const SLA_OPTIONS: SlaFilter[] = [...SLA_STATUS_VALUES];
...
const SORT_OPTION_LABEL_KEY: Record<VulnerabilitySortKey, string> = {
  ...
  sla_due: "vulnerabilities.column.sla_due",
...
export interface VulnerabilitiesToolbarProps {
  ...
  /**
   * SLA-status filter (X1): `"overdue"` / `"imminent"` / `"ok"` or `null`
   * for "any". Backs the single-valued `?sla=` query parameter.
   */
  sla: SlaFilter | null;
...
export function VulnerabilitiesToolbar({
  ...
  return (
    <div
      className={cn(
        // G0-5 — `lg:flex-wrap`. The row variant had nine controls and no wrap
        // point, so at 1440 px (a ~1160 px pane once the sidebar is out) the
        // line was over-full and flexbox squeezed the last controls instead:
        // "Add filter" rendered one character per line, the VEX checkbox label
        // broke into four, and the EPSS placeholder was cut mid-number. Korean
        // made it worse, since its labels are wider — "업그레이드별" arrived as
        // "업그". Wrapping keeps the filters inline, which is the design
        // system's rule for this surface (no modal filter dialogs); it just
        // spends a second line when the first one runs out.
        "flex flex-col gap-3 border-b bg-background px-4 py-3 lg:flex-row lg:flex-wrap lg:items-end lg:gap-4",
        className,
      )}
    ...
    >
      ...
      {/* X1 — SLA status filter. Same inline single-select pattern as the
          reachability filter above; the option labels reuse the badge's
          state vocabulary (`vulnerabilities.sla.state.*`) so the filter and
          the column chip always speak the same words. */}
      <div className="flex flex-col">
        <label
          htmlFor="vulnerabilities-sla-filter"
          className="text-xs font-medium text-muted-foreground"
          title={t("vulnerabilities.sla.tooltip_filter")}
        >
          {t("vulnerabilities.toolbar.sla_label")}
        </label>
        <select
          id="vulnerabilities-sla-filter"
          value={sla ?? ""}
          onChange={(event) =>
            onSlaChange(
              event.target.value === ""
                ? null
                : (event.target.value as SlaFilter),
            )
          }
          className="mt-1 h-9 w-36 rounded-md border border-input bg-background px-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
          data-testid="vulnerabilities-sla-filter"
        >
          <option value="">{t("vulnerabilities.toolbar.sla_any")}</option>
          {SLA_OPTIONS.map((opt) => (
            <option key={opt} value={opt}>
              {t(`vulnerabilities.sla.state.${opt}`)}
            </option>
          ))}
        </select>
      </div>
