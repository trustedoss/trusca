 ...
 * membership-based `group_admin` of this project's team must see `group_admin`
 ...
 */
export type TeamScopedRole = "super_admin" | "group_admin" | "developer" | "viewer";
...
export interface ProjectOverviewResponse {
  ...
  /** #26 — distinct flagged components in the anchored scan. */
...
export interface ComponentSummary {
  ...
  direct: boolean;
  /**
   * commercial-SCA-style "Usage" facet (W2 #31). ``required`` / ``optional`` is the raw
  ...
  /**
   * #26 — known-malicious verdict. `flagged` = the vendored OSV MAL- snapshot
   * lists this package as published-to-attack; `clear` = consulted, not
   * listed; `null` = never assessed. Not a vulnerability: it creates no
   * finding and never enters severity counts, because the response is
   * removal plus credential rotation rather than an upgrade.
   */
  malicious_state: MaliciousState | null;
  /** OSV advisory id behind a flagged verdict ("MAL-2025-47141"). */
  malicious_id: string | null;
  /** Snapshot the verdict came from, `osv.dev@YYYY-MM-DD`. */
  malicious_source: string | null;
  /**
...
export interface ComponentDetailResponse {
  ...
  direct: boolean;
  /**
   * commercial-SCA-style "Usage" of the chosen (shallowest) path. ``null`` when the
   * scan didn't encode a scope on the edge.
   */
  dependency_scope: "required" | "optional" | null;
  /** Phase M — endoflife.date verdict (see ComponentSummary.eol_state). */
  /** #26 — known-malicious verdict (see ComponentSummary.malicious_state). */
  malicious_state: MaliciousState | null;
  malicious_id: string | null;
  malicious_source: string | null;
  eol_state: EolState | null;
...
/**
 * The closed malicious_state vocabulary (#26). Mirrors
 * ``services.malicious.malicious_catalog.MALICIOUS_STATES`` on the backend;
 * contract-tested in tests/unit/contracts/catalogMirrors.test.ts against
 * apps/backend/tests/unit/test_catalog_contracts.py.
 *
 * Note the pair is a DENY-list verdict, unlike EOL_STATES: `clear` means the
 * snapshot was consulted and did not list the package, while `null` (absent
 * from this union) means it was never assessed. Surfaces must not collapse
 * the two — an unevaluated catalog would otherwise read as a clean one.
 */
export const MALICIOUS_STATES = ["flagged", "clear"] as const;
export type MaliciousState = (typeof MALICIOUS_STATES)[number];

export type ComponentSortKey = "name" | "severity" | "license";
export type SortOrder = "asc" | "desc";

/**
 * commercial-SCA-style "Usage" filter bucket (W2 #31). ``unspecified`` maps to the NULL
...
export interface ListComponentsParams {
  ...
  /**
   * Known-malicious facet (#26). ``true`` keeps only components the vendored
   * OSV MAL- snapshot lists as published-to-attack; ``false`` keeps
   * everything else INCLUDING never-assessed rows. Omit / ``null`` to include
   * both.
   */
...
function listComponentsQuery(
  ...
  // Known-malicious facet — same emit-only-on-opinion contract as `eol`.
  if (params.malicious === true || params.malicious === false) {
    out.malicious = params.malicious;
  }
