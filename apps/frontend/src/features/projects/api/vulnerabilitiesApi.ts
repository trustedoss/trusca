...
export type VulnerabilitySortKey =
  ...
  | "priority"
  /**
   * Remediation-SLA due date (X1 SLA/aging). `asc` = most urgent first
   * (soonest / most-overdue due date on top); rows without an SLA
   * (info / unknown severity → `sla_due_date IS NULL`) sort LAST in both
   * directions (backend NULLS LAST).
   */
  | "sla_due";
export type SortOrder = "asc" | "desc";

/**
 * SLA status vocabulary (X1 SLA/aging) — FE mirror of the backend's
 * `services.vulnerability_service.SLA_STATUS_VALUES` tuple / `SlaStatus`
 * wire Literal. Contract-tested in `tests/unit/contracts/catalogMirrors.test.ts`
 * (set equality against the backend source) so the two never drift.
 *
 * `null` on the wire means "this severity carries no SLA" (info / unknown) —
 * the absence of an SLA, not a state of one, which is why it is neither in
 * this list nor a legal `?sla=` filter token.
 */
export const SLA_STATUS_VALUES = ["overdue", "imminent", "ok"] as const;

export type SlaStatus = (typeof SLA_STATUS_VALUES)[number];

/**
 * `?sla=` filter token (X1). Single-valued; accepts exactly the three
 * {@link SlaStatus} tokens. Omit the param to disable the filter. Anything
 * else is a 422 on the wire, so the UI only ever sends these three.
 */
export type SlaFilter = SlaStatus;
...
export interface VulnerabilityListItem {
  ...
  /**
   * When this (component_version × CVE) pair was FIRST detected in the
   * project (X1 SLA/aging). Carried forward across re-scans / re-matches —
   * unlike `discovered_at`, which is this scan's finding-row insert time.
   * The SLA clock starts here.
   */
  first_detected_at: string;
  /**
   * Remediation SLA due date: `first_detected_at` + the per-severity SLA
   * window (defaults: critical 7d / high 30d / medium 90d / low 180d).
   * `null` when the severity carries no SLA (info / unknown).
   */
  sla_due_date: string | null;
  /**
   * Server-computed SLA state (imminent = due within 7 days). The FE renders
   * this verbatim and NEVER recomputes it client-side — a locally derived
   * state could disagree with the server's `?sla=` filter (clock skew /
...
export interface VulnerabilityDetail {
  ...
  /**
   * Project-level first-detection instant (X1 SLA/aging) — same semantics as
   * the list item's field; the SLA clock starts here.
   */
  first_detected_at: string;
  /** SLA due date (`first_detected_at` + per-severity window); `null` = no SLA. */
  sla_due_date: string | null;
  /**
   * Server-computed SLA state — same semantics as the list rows so the drawer
...
export interface ListVulnerabilitiesParams {
  ...
  /**
   * SLA-status filter (X1). Single-valued: keep only findings whose
   * server-computed `sla_status` equals the token. Omit to disable. Rows
...
function listVulnerabilitiesQuery(
  ...
  // X1 — only forward one of the three legal SLA tokens (same defensive
  // narrowing as `reachable` so a hand-edited URL can't trip the 422).
  if (
    params.sla === "overdue" ||
    params.sla === "imminent" ||
    params.sla === "ok"
