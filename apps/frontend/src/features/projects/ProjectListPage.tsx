...
export function ProjectListPage() {
  ...
  // S1-3 — search runs on the SERVER. It used to be a client-side filter over
  // whatever the single size=100 page happened to contain, so any tenant with
  // more than 100 projects silently could not find the 101st: the box looked
  // like it worked and simply returned nothing. The debounced term is part of
  ...
  // What the server says matched, vs. what this one page can show. The two
  // differ once a term matches more than PROJECT_PAGE_SIZE projects; the
  // banner below says so rather than letting the list read as complete.
  ...
  return (
    ...
    >
      ...
      <main className="flex flex-1 flex-col" data-testid="project-list-main">
        ...
        {/* The page fetches one server page. When more projects match than fit
            on it, say so — a list that quietly stops at 100 reads as "that is
            everything", which is how the old client-side search hid its own
            truncation. Narrowing the search is the way out. */}
        {isTruncated ? (
          <div className="px-6 pt-4">
            <Alert data-testid="project-list-truncated">
              <AlertDescription>
                {t("list.truncated", {
  ...
  return (
    ...
    >
      <div className="flex flex-1 items-center gap-3 truncate">
        ...
        <span
          // G0-5 — also `lg`-gated. Once the row fitted, the repository URL
          // was taking 121 px of the 169 px available and the project name was
          // left with 37 — four characters and an ellipsis. Between a
          // truncated URL and a readable name, the name is the one the row
          // exists to show; the URL returns with the rest of the dense
          // columns above `lg`.
...
function SeveritySummary({
  ...
  const buckets: SeverityBucket[] = [
    {
      ...
      colorClass: "text-risk-critical-foreground",
      abbrevKey: "severity.abbrev.critical",
    },
    {
      key: "high",
      count: summary.high,
      colorClass: "text-risk-high-foreground",
      abbrevKey: "severity.abbrev.high",
    },
    {
      key: "medium",
      count: summary.medium,
      colorClass: "text-risk-medium-foreground",
      abbrevKey: "severity.abbrev.medium",
    },
    {
      key: "low",
      count: summary.low,
      colorClass: "text-risk-low-foreground",
  ...
  return (
    <div
      // G0-5 — hidden below `lg`. The row's fixed-width right-hand clusters
      // (this one, the scan metadata, the status pill, the Scan button) came to
      // 386 px of non-shrinking content, and the flex-1 name cell — the row's
      // whole identity — collapsed to exactly 0 px on a 390 px phone while the
      // row still overflowed. Same breakpoint and same mechanism as the
      // `created_by` cell below, and the same one at which the shell switches
      // to its drawer, so the row is dense above `lg` and legible below it.
...
function ScanMetadataSummary({ project }: { project: ProjectPublic }) {
  ...
  return (
    <div
      // G0-5 — hidden below `lg`, for the reason spelled out on
      // `project-row-severity`: these clusters together left the project name
      // no width at all on a phone.
      className="hidden items-center gap-2 font-mono text-xs text-muted-foreground lg:flex"
