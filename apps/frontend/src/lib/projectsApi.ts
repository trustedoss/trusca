...
/**
 * Ingest an external CycloneDX / SPDX SBOM (model 3) — `POST
 * /v1/projects/{project_id}/sbom-ingest`. Multipart form (`sbom` file plus
 * optional `ref` / `release`). The backend queues a Celery matching task and
 * returns the persisted `202` scan row, which the caller wires into the same
 * live `ScanProgress` drawer a source scan uses. Size / format problems surface
 * as RFC 7807 (413 / 415 / 422); the caller maps them to i18n via the shared
 * problem helpers.
 */
export async function ingestSbom(
  projectId: string,
  file: File,
  options: IngestSbomOptions = {},
): Promise<ScanPublic> {
  const form = new FormData();
  form.append("sbom", file, file.name);
  if (options.ref && options.ref.trim().length > 0) {
    form.append("ref", options.ref.trim());
  }
  if (options.release && options.release.trim().length > 0) {
    form.append("release", options.release.trim());
  }
  const { data } = await api.post<ScanPublic>(
    `/v1/projects/${projectId}/sbom-ingest`,
    form,
  );
  return data;
}
