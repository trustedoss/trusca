...
>(({ className, indicatorClassName, value, ...props }, ref) => (
  ...
  >
    <ProgressPrimitive.Indicator
      ...
      className={cn(
        // W14 — progress is where the product is visibly working, so it
        // carries the brand rather than neutral ink. Callers that need a
        // different meaning (disk usage nearing a threshold, for example)
        // still override via `indicatorClassName`.
        "h-full w-full flex-1 bg-brand transition-all duration-base ease-out-soft",
        indicatorClassName,
      )}
