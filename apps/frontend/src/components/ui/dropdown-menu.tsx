...
/**
 * A menu item that is one of a set, exactly one of which is current.
 *
 * Use this — not `DropdownMenuItem` plus a check glyph — whenever the menu
 * expresses a choice. Radix emits `role="menuitemradio"` with `aria-checked`,
 * so the current option is announced; a decorative check is invisible to a
 * screen reader, and axe cannot flag the difference because a plain named
 * menuitem is structurally valid either way.
 */
...
>(({ className, children, ...props }, ref) => (
  <DropdownMenuPrimitive.RadioItem
    ref={ref}
    className={cn(
      "relative flex cursor-pointer select-none items-center gap-2 rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none transition-colors duration-fast ease-out-soft",
      "focus:bg-accent focus:text-accent-foreground data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground",
      "data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className,
    )}
