...
test.describe.serial("@screenshots user-guide/dashboard", () => {
  ...
  test("user-dashboard — post-login dashboard landing page", async ({ page }) => {
    // The dashboard is the portal's root (`/`) after sign-in.
    await page.goto("/");
    await page
    ...
    await page
      ...
      .waitFor({ state: "visible", timeout: 10_000 });
    // The two containers above exist while their contents are still
    // skeletons, so waiting on them did not do what the comment here used to
    // claim — a capture could and did ship a page of grey blocks into the
    // user guide. These two testids belong to the loaded state only (the
    // pending state renders `-loading` instead), so waiting on them means
    // waiting on data.
    await page
      .getByTestId("action-queue")
...
test.describe.serial("@screenshots user-guide/notifications", () => {
  ...
  test("user-notifications-bell — header bell with unread badge", async ({
    ...
    await captureSection(page, "user-notifications-bell", "app-topbar");
  });
