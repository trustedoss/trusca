 ...
 *
 * The active language lives under `LANGUAGE_STORAGE_KEY`, imported from
 * `@/lib/languageStorage` rather than restated here. G0-5: this helper used to
 * hardcode `"i18nextLng"` — i18next's own default, not the key
 * `apps/frontend/src/lib/i18n.ts` configures — and that module's
 * `detection.order` is `["localStorage"]` with its one key, so the write
 * landed somewhere nothing reads and the page stayed English. Setting the key
 * before any navigation avoids an in-page click on the LanguageToggle button,
 * which would trigger a re-render mid-capture and risk a half-translated
 * frame.
 *
...
export async function setUiLanguage(
  ...
  await page.addInitScript(
    ...
    { key: LANGUAGE_STORAGE_KEY, language: lang },
  );
}
...
export async function ensureSeededPendingApproval(): Promise<void> {
  ...
  // CLAUDE.md pins `docker-compose` (V1, hyphen) because the developer
  // environment has no V2. GitHub's runners have the opposite: V2 only, and
  // a missing binary comes back as `status === null` rather than a non-zero
  // exit — which read as "psql failed (status=null): null" in the first CI
  // capture run and looked like a database problem.
  //
  // Try the pinned one, fall back to the plugin form. Neither environment
  // has to know about the other.
  let result = spawnSync("docker-compose", psqlArgs, {
    cwd: REPO_ROOT,
    encoding: "utf8",
  });
  if (result.status === null) {
    result = spawnSync("docker", ["compose", ...psqlArgs], {
      cwd: REPO_ROOT,
      encoding: "utf8",
    });
  }
