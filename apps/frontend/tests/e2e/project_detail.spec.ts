...
test.describe("@project-detail project detail page", () => {
  ...
  test("4) search narrows the components list and mirrors to URL", async ({
    ...
    // S1-5 — the Components tab mirrors into its own key, not the `?search=`
    // all four detail tabs used to share.
    expect(new URL(page.url()).searchParams.get("components_search")).toBe(
      "react-00007",
    );
