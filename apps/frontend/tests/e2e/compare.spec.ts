 ...
 * dev DB collide. (The delta components the diff actually asserts on are
 * already suffix-unique in the seed.)
 *
 * The prefix used to be `testInfo.testId` truncated to twelve characters,
 * which defeated its own purpose: Playwright's testId leads with a hash of the
 * spec file, so twelve characters of it are IDENTICAL for every test in this
 * file. S2 and S3 have therefore been seeding into S1's purls, tripping the
 * unique constraint, and skipping silently on CI ever since — confirmed in the
 * 2026-07-30 nightly, where both show as skipped while S1 passes. A per-run
 * token plus a monotonic counter cannot collide that way.
 */
...
function uniquePrefix(testInfo: import("@playwright/test").TestInfo): string {
  seedSequence += 1;
  return `cmp${RUN_TOKEN}s${seedSequence}r${testInfo.retry}`;
}
