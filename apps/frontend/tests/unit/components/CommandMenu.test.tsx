/**
 * CommandMenu — unit tests for the global ⌘K palette.
 *
 * Coverage:
 *   - ⌘K opens; Esc closes.
...
describe("CommandMenu", () => {
  ...
  it("renders component hits under the Components group and deep-links into the components tab", async () => {
    ...
    await waitFor(() => {
      ...
      // S1-5 — the Components tab owns `?components_search=`; the palette must
      // mint that key, not the `?search=` all four tabs used to share.
      expect(hit?.search).toBe("?tab=components&components_search=lodash");
    });
  });
  ...
  it("renders CVE hits under the CVEs group and deep-links into the vulnerabilities tab", async () => {
    ...
    await waitFor(() => {
      ...
      expect(hit?.search).toBe(
        "?tab=vulnerabilities&vulnerabilities_search=CVE-2021-23337",
      );
    });
  });
  ...
  // The search page is absent from the sidebar on purpose, so the palette is
  // the only standing way in. Both halves matter: it has to be reachable with
  // an empty input (otherwise a user who has never typed cannot discover the
  // page at all), and it has to survive typing (cmdk filters rows against the
  // input, so a row whose `value` omitted the term would vanish exactly when
  // it became useful).
  it("offers the search page with nothing typed", async () => {
    const user = userEvent.setup();
    const locations: Array<{ pathname: string; search: string }> = [];
    render(
      <ControlledHarness
    ...
    await user.click(await screen.findByTestId("command-menu-open-search"));

    await waitFor(() => {
      const hit = locations.find((l) => l.pathname === "/search");
      expect(hit).toBeDefined();
      // No term typed, so no `q` to carry — the page opens on its own empty
      // state rather than searching for "".
      expect(hit?.search).toBe("");
    });
  });

  it("carries the typed term through to the search page", async () => {
    const user = userEvent.setup();
    mockedGlobalSearch.mockResolvedValue(searchResults());
    const locations: Array<{ pathname: string; search: string }> = [];
    render(
    ...
    await waitFor(() => {
      const hit = locations.find((l) => l.pathname === "/search");
      expect(hit).toBeDefined();
      // No `kind` — the page's own default (components) applies, and the user
