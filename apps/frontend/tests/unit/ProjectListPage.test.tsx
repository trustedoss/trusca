...
describe("ProjectListPage", () => {
  ...
  it("sends the debounced search term to the server and renders its answer", async () => {
    // S1-3 — search runs server-side. It used to filter the single fetched
    // page in the browser, so a tenant with more projects than fit on one
    // page could not find the ones past it: the box worked and simply
    // returned nothing. The contract is now "the term reaches the API and the
    // API's rows are what render".
    ...
    // The first load asks for no term.
    expect(mockedListProjects).toHaveBeenCalledWith(
      expect.objectContaining({ q: undefined }),
    );

    // The server is what narrows: it answers the next call with one row.
    mockedListProjects.mockResolvedValue(listResponse([project("Alpha")]));
  ...
  it("says so when more projects match than the page can show", async () => {
    // "No silent caps" — a list that quietly stops at the page ceiling reads
    // as "that is everything", which is exactly how the old client-side
    // search hid its own truncation.
    mockedListProjects.mockResolvedValue({
      items: [project("Alpha")],
      total: 250,
      page: 1,
    ...
    renderPage();
    await waitFor(() => {
      expect(screen.getByTestId("project-list-truncated")).toBeInTheDocument();
    });
    expect(screen.getByTestId("project-list-truncated")).toHaveTextContent("250");
  });

  it("stays quiet when the page holds every match", async () => {
    mockedListProjects.mockResolvedValue(
      listResponse([project("Alpha"), project("Bravo")]),
    );
    renderPage();
    ...
    expect(screen.queryByTestId("project-list-truncated")).not.toBeInTheDocument();
  });
  ...
  it("renders a colored severity summary when severity_summary has non-zero buckets", async () => {
    ...
    // Risk design tokens, not hex literals — and the `-foreground` shade
    // rather than the severity hex, because these are counts drawn as text.
    // W18: the hex measured 2.86:1 (medium) to 4.83:1 on a white card and
    // 3.38:1 to 5.95:1 on a dark one, so a severity colour used as text was
    // under AA in one theme or the other. Icons keep the hex; letters do not.
    expect(
      summary.querySelector(".text-risk-critical-foreground"),
    ).toBeInTheDocument();
    expect(summary.querySelector(".text-risk-high-foreground")).toBeInTheDocument();
    expect(summary.querySelector(".text-risk-low-foreground")).toBeInTheDocument();
    // The omitted (zero) bucket renders no medium token.
    expect(
      summary.querySelector(".text-risk-medium-foreground"),
  ...
  it("toggles the severity chart filter off when the same segment is re-clicked", async () => {
  ...
  it("toggles the license chart filter off when the same segment is re-clicked", async () => {
    mockedListProjects.mockResolvedValueOnce(
      listResponse([
        project("Alpha", {
          latest_scan_status: "succeeded",
