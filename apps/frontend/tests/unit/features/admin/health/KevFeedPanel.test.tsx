 ...
 *   - Synced state renders 4 KPI tiles + success status badge + feed_host
...
describe("KevFeedPanel", () => {
  ...
  it("renders the synced state with KPI grid and success badge", async () => {
    ...
    expect(badge).toHaveAttribute("data-status", "synced");
        // W14 debt repayment moved these callouts onto the status tokens, so
    // the assertion follows the semantic name rather than the palette hue
