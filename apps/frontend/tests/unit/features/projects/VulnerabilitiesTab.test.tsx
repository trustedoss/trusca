...
vi.mock("@/features/projects/api/vulnerabilitiesApi", async () => {
  return {
    ...
    // X1 — the toolbar's SLA filter options + the tab's URL parser consume
    // the vocabulary mirror; the mock must re-export it (contract-tested
    // against the backend in tests/unit/contracts/catalogMirrors.test.ts).
    SLA_STATUS_VALUES: ["overdue", "imminent", "ok"],
  };
});
...
function vuln(
  ...
  return {
    ...
    sla_due_date: "2026-05-31T00:00:00Z",
    sla_status: "ok",
...
describe("VulnerabilitiesTab", () => {
  ...
  // ----- SLA / aging surface (X1) -----------------------------------------

  it("renders the SLA due cell: badge + date for SLA rows, em-dash for no-SLA rows", async () => {
    mockedList.mockResolvedValueOnce(
      listResponse([
        vuln("CVE-2024-1111", {
          severity: "critical",
          sla_status: "overdue",
          sla_due_date: "2026-05-08T00:00:00Z",
        }),
        vuln("CVE-2024-2222", {
          severity: "info",
          sla_status: null,
    ...
    const cells = screen.getAllByTestId("vulnerability-row-sla");
    expect(cells).toHaveLength(2);
    // SLA row: server state verbatim + the due DATE inline (the column IS
    // the deadline). Color is paired with the literal state label.
    expect(cells[0]).toHaveAttribute("data-sla-status", "overdue");
    const badge = within(cells[0]).getByTestId("sla-badge");
    expect(badge.textContent).toContain("Overdue");
    expect(
      within(cells[0]).getByTestId("sla-badge-due-date").textContent,
    ).toContain("2026-05-08");
    // No-SLA row (info severity): em-dash placeholder, no badge.
    expect(cells[1]).toHaveAttribute("data-sla-status", "none");
    expect(within(cells[1]).queryByTestId("sla-badge")).not.toBeInTheDocument();
    expect(cells[1].textContent).toContain("—");
  });

  it("selecting the SLA filter forwards ?sla=overdue at offset 0", async () => {
    mockedList.mockResolvedValue(listResponse([vuln("CVE-2024-1111")]));
    renderTab();
    await waitFor(() => {
      expect(screen.getAllByTestId("vulnerability-row")).toHaveLength(1);
    ...
    await userEvent.selectOptions(
      screen.getByTestId("vulnerabilities-sla-filter"),
      "overdue",
    );
    await waitFor(() => {
      expect(mockedList).toHaveBeenCalledWith(
        "proj-1",
        expect.objectContaining({ sla: "overdue", offset: 0 }),
      );
    });
  });

  it("hydrates the SLA filter from the URL on first render", async () => {
    mockedList.mockResolvedValueOnce(listResponse([vuln("CVE-2024-1111")]));
    renderTab(["/projects/proj-1?sla=imminent"]);
    await waitFor(() => {
      expect(mockedList).toHaveBeenCalledWith(
        "proj-1",
        expect.objectContaining({ sla: "imminent" }),
      );
    });
    expect(
      (
        screen.getByTestId(
          "vulnerabilities-sla-filter",
        ) as HTMLSelectElement
      ).value,
    ).toBe("imminent");
  });

  it("ignores an out-of-range sla URL value (no wire param)", async () => {
    mockedList.mockResolvedValueOnce(listResponse([vuln("CVE-2024-1111")]));
    renderTab(["/projects/proj-1?sla=bogus"]);
    await waitFor(() => {
      expect(mockedList).toHaveBeenCalledWith(
        "proj-1",
        expect.objectContaining({ sla: undefined }),
      );
    });
  });

  it("picking 'SLA due' in the sort select requests sort=sla_due ASC (urgent first)", async () => {
    mockedList.mockResolvedValue(listResponse([vuln("CVE-2024-1111")]));
    renderTab();
    await waitFor(() => {
      expect(screen.getAllByTestId("vulnerability-row")).toHaveLength(1);
    ...
    await waitFor(() => {
      // Unlike every other key (natural desc), sla_due's "most urgent first"
      // is ASC: the soonest / most-overdue due date is the smallest value.
      expect(mockedList).toHaveBeenCalledWith(
        "proj-1",
        expect.objectContaining({ sort: "sla_due", order: "asc", offset: 0 }),
      );
    });
  });

  it("clicking the SLA due column header requests sort=sla_due from the wire", async () => {
    mockedList.mockResolvedValue(listResponse([vuln("CVE-2024-1111")]));
    renderTab();
    await waitFor(() => {
      expect(screen.getAllByTestId("vulnerability-row")).toHaveLength(1);
    ...
    await userEvent.click(
      screen.getByTestId("vulnerabilities-sort-header-sla-due"),
    );
    await waitFor(() => {
      expect(mockedList).toHaveBeenCalledWith(
        "proj-1",
        expect.objectContaining({ sort: "sla_due" }),
      );
    });
  });
