...
describe("ProjectDetailPage", () => {
  ...
  it("renders only the not-found surface on a 404 — no tabs, no Scan (G0-5)", async () => {
    // The page used to draw its whole console around a project that does not
    // exist: eight clickable tabs, a Scan button, a release picker reading "No
    // version", the governance band reporting a second failure of its own, and
    // the backend's untranslated "Project Not Found / project <uuid> not found"
    // in the tab body. Each tab led to another error. What a 404 owes the user
    // is the reason and the way back.
    const notFound = () =>
      new ProblemError("not_found", {
        status: 404,
        title: "Project Not Found",
    ...
    mockedGetProject.mockRejectedValueOnce(notFound());
    mockedOverview.mockRejectedValueOnce(notFound());
    renderPage();

    await waitFor(() => {
      expect(
        screen.getByTestId("project-detail-unavailable"),
      ).toBeInTheDocument();
    });
    expect(screen.queryByTestId("project-detail-tabs")).not.toBeInTheDocument();
    expect(screen.queryByTestId("project-detail-scan")).not.toBeInTheDocument();
    expect(
      screen.getByTestId("project-detail-unavailable-back"),
    ).toHaveAttribute("href", "/projects");
    // The reason is localized, never the backend's English `title`.
    expect(screen.queryByText(/Project Not Found/)).not.toBeInTheDocument();
    expect(
      screen.getByTestId("project-detail-unavailable").textContent,
