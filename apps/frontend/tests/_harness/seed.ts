...
export interface SeedOptions {
  ...
  /**
   * Attach every seeded component to EVERY project's scan, not just the
   * first.
   *
   * The default fixture anchors all components on the first project, which is
   * what per-project scenarios want. The organization-wide inventory reports
   * "one package, N projects", and that shape cannot be seeded any other way —
   * so cross-project scenarios set this. Findings (licence, CVE) stay on the
   * first project either way, so per-project counts do not move.
   */
  shareComponents?: boolean;
  /**
...
export function seedE2eUser(opts: SeedOptions): SeedSummary {
  ...
  if (opts.shareComponents) {
    scriptArgs.push("--share-components");
  }
