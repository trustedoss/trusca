...
Security focus (adversarial parametrize on untrusted-input parsing):
...
class _FakeSession:
    ...
    async def execute(self, _stmt: Any) -> _FakeResult:
        ...
        return _FakeResult(SimpleNamespace(id=team_id, path=[]))
...
def test_safe_extract_rejects_compression_ratio_bomb(
    ...
    # The ratio guard only applies above the size floor; lower the
...
def test_safe_extract_ratio_floor_does_not_exempt_large_members(
    ...
    # UX: the rejection names the member and tells the operator the
