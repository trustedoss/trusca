...
def test_count_eligible_files_prunes_deeply_nested_excluded_dir(tmp_path: Path) -> None:
    """security review, medium severity — a 2-level-deep node_modules is excluded.
...
# ---------------------------------------------------------------------------
# security review, medium severity — result-size ceiling (OOM guard).
#
...
# ---------------------------------------------------------------------------
# security review, low severity — source_path truncation (unbounded telemetry).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# security review, low severity — symlink out of source_dir is never read.
#
...
# ---------------------------------------------------------------------------
# security review, high severity — adversarial SPDX tokens must not reach the DB.
#
...
def test_command_ignores_excluded_dirs_at_any_depth(tmp_path: Path) -> None:
    """security review, medium severity — a bare-name ignore matches at any depth.
