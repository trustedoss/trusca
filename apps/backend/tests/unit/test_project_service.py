...
async def test_list_projects_q_matches_git_url(
    db_session: AsyncSession,
) -> None:
    """S1-3 — the clone URL is a searchable axis on the server.

    The list page's search box has always said "name or git URL", but the
    server only matched name and slug; a client-side filter carried the URL
    axis, so it silently vanished for any tenant whose project list outgrew
    one page. Searching a fragment that appears ONLY in the clone URL is the
    regression that proves the axis now lives on the server.
    """
    from services.project_service import list_projects
    ...
    needle = unique_suffix()
    target = await make_project(
        db_session,
        team=team,
        name="unrelated name",
        git_url=f"https://github.com/example/{needle}.git",
    )
    decoy = await make_project(db_session, team=team, name="another project")

    rows, total = await list_projects(db_session, actor=actor, team_id=team.id, q=needle)
    ids = {row.id for row in rows}
    assert target.id in ids
    assert decoy.id not in ids
    assert total == 1
...
async def test_list_projects_q_treats_wildcards_literally(
    db_session: AsyncSession,
) -> None:
    """S1-2 — a '%' in the query matches a percent sign, not everything.

    Without ESCAPE handling the pattern ``%\\%%`` collapses into "match every
    row", so a user typing a stray percent sign would see the whole portfolio
    and believe it matched their term.
    """
    from services.project_service import list_projects
    ...
    literal = await make_project(db_session, team=team, name=f"100%-coverage-{suffix}")
    plain = await make_project(db_session, team=team, name=f"plain-{suffix}")

    rows, total = await list_projects(db_session, actor=actor, team_id=team.id, q="%")
    ids = {row.id for row in rows}
    assert literal.id in ids, "a project whose name contains '%' should match"
    assert plain.id not in ids, "'%' must not widen the search to every project"
    assert total == 1

    # And '_' — the single-character wildcard — is literal too.
    underscore = await make_project(db_session, team=team, name=f"a_b-{suffix}")
    rows, _total = await list_projects(db_session, actor=actor, team_id=team.id, q="a_b")
    ids = {row.id for row in rows}
