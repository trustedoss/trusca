...
않는 키와 틀린 키의 타이밍이 여전히 평탄하다", i.e. success/failure verdicts
are unchanged, and the offload must not introduce a new timing gap. The
...
def test_verify_password_async_does_not_block_the_event_loop():
    ...
    the loop, so the counter would not advance at all for the ~213ms the
