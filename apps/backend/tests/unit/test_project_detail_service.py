...
async def test_list_components_search_matches_purl(
    db_session: AsyncSession,
) -> None:
    """S1-4 — the second search axis is purl, and it actually matches.

    The box promises "search by name or purl". Before S1-4 the second axis
    was ``components.namespace``, a column the scan ingest never writes — so
    the OR branch could not match anything and pasting a coordinate found
    nothing. Searching a purl-only fragment (the ``pkg:npm/`` scheme prefix,
    which appears in no component name) proves the axis is live.
    """
    from services.project_detail_service import list_components_for_project
    ...
    needle = unique_suffix()
    _, target_cv = await _make_component_version(
        db_session, name=f"target-{needle}", package_type="npm"
    )
    await _attach_to_scan(db_session, scan_id=scan.id, cv_id=target_cv.id)

    # ``pkg:npm/target-<needle>`` — the scheme fragment lives only in the purl.
    items, total = await list_components_for_project(
        db_session, project_id=project.id, actor=actor, search=f"pkg:npm/target-{needle}"
    )
    assert total == 1
    assert items[0]["component_id"] == target_cv.component_id


async def test_list_components_search_treats_wildcards_literally(
    db_session: AsyncSession,
) -> None:
    """S1-2 — '%' in a component search is a percent sign, not "match all"."""
    from services.project_detail_service import list_components_for_project
    ...
    suffix = unique_suffix()
    _, literal_cv = await _make_component_version(db_session, name=f"pct%name-{suffix}")
    _, plain_cv = await _make_component_version(db_session, name=f"plain-{suffix}")
    await _attach_to_scan(db_session, scan_id=scan.id, cv_id=literal_cv.id)
    await _attach_to_scan(db_session, scan_id=scan.id, cv_id=plain_cv.id)

    items, total = await list_components_for_project(
        db_session, project_id=project.id, actor=actor, search="%"
    )
    assert total == 1, "'%' must not widen the search to every component"
    assert items[0]["component_id"] == literal_cv.component_id
...
async def test_component_detail_returns_drawer_payload_with_vulns(
    ...
    # uq_components_purl (cross-suite isolation; the neighbour drawer tests
    # already do this).
...
async def test_component_detail_vuln_ref_exposes_fixed_version(
    ...
    # collide on uq_components_purl (cross-suite isolation).
...
async def test_list_components_required_wins_over_optional_across_paths(
    ...
    path is reported as runtime-required (commercial-SCA-style 'Usage' semantics)."""
    from services.project_detail_service import list_components_for_project
