...
def test_probe_redis_outage(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    component = _probe_redis()
    assert component.status == "down"
    assert component.detail_code is None  # a probe exception is not translated


def test_probe_redis_ping_false_is_a_named_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A reachable Redis that answers ``False`` is a state we describe."""
    ...
    monkeypatch.setattr(
        "services.admin_health_service._redis.Redis.from_url",
        lambda _url, **_kw: _FalseClient(),
    )
    component = _probe_redis()
    assert component.status == "down"
    assert component.detail_code == "redis.ping_false"
...
def test_probe_celery_workers_present() -> None:
    ...
    assert component.detail_code == "celery.workers_responded"
    assert component.detail_params == {"count": 1}
...
def test_probe_celery_no_workers() -> None:
    ...
    assert component.detail_code == "celery.no_workers"
...
def test_probe_celery_control_failure() -> None:
    ...
    component = _probe_celery(celery_app_override=fake)
    assert component.status == "down"
    # A probe exception carries no code on purpose. The UI renders `detail`
    # verbatim for these, because the text the runtime produced is the
    # diagnostic — "ConnectionError: broker unreachable" translated into
    # Korean would tell an operator strictly less.
    assert component.detail_code is None
    assert component.detail is not None
    assert "ConnectionError" in component.detail
...
async def test_probe_active_scans_counts_queued_and_running(
    ...
    assert component.detail_code == "active_scans.count"
    assert component.detail_params == {"count": int(component.value)}
...
async def test_probe_last_24h_errors_counts_recent_failures(db_session: AsyncSession) -> None:
    ...
    assert component.value is not None
    assert int(component.value) >= 1
    assert component.detail_code == "last_24h_errors.count"
    assert component.detail_params == {"count": int(component.value)}
