...
``worker.autoscaling.mode`` (``cpu`` | ``queue``).
...
* ``mode`` defaults to ``cpu``, which is this chart's ONLY behavior before
  ...
  off) or with only ``worker.autoscaling.enabled=true`` (HPA on) must render
  identically to how it did before ``scaledobject-worker.yaml`` existed. The
  two manifest-set-equality tests below pin that by rendering the FULL chart
  and asserting no KEDA kind appears anywhere, whether or not autoscaling is
  on, as long as mode stays at its default.
...
def test_cpu_mode_enabled_renders_the_same_hpa_as_before_this_unit() -> None:
    ...
    manifest = _render_full("--set", "worker.autoscaling.enabled=true")
    ...
    hpa = yaml.safe_load(
        _render_show_only(
            "templates/hpa-worker.yaml", "--set", "worker.autoscaling.enabled=true"
        ).stdout
    )
...
def test_explicit_cpu_mode_is_equivalent_to_the_default() -> None:
    ...
    implicit = _render_full("--set", "worker.autoscaling.enabled=true")
    explicit = _render_full(
        "--set", "worker.autoscaling.enabled=true", "--set", "worker.autoscaling.mode=cpu"
    )
...
def test_queue_mode_renders_scaledobject_instead_of_hpa() -> None:
    ...
    manifest = _render_full(
        "--set", "worker.autoscaling.enabled=true", "--set", "worker.autoscaling.mode=queue"
    )
    assert "kind: HorizontalPodAutoscaler" not in manifest
...
def test_queue_mode_fails_the_render_when_redis_is_external_and_unaddressed() -> None:
    ...
    assert "worker.autoscaling.queue.redis.address is required" in result.stderr
...
def test_queue_mode_with_password_secret_renders_triggerauthentication() -> None:
    manifest = _render_full(
        ...
        "worker.autoscaling.mode=queue",
        "--set",
        "worker.autoscaling.queue.redis.passwordSecretName=external-redis-auth",
    )
    ...
    assert (
        ...
        == "trustedoss-golden-trustedoss-worker-queue-redis"
    )
...
def test_queue_mode_cooldown_defaults_to_the_worker_termination_grace_period() -> None:
    ...
    worker.autoscaling.queue.cooldownPeriod is the separate, complementary
    ...
    deployment = yaml.safe_load(_render_show_only("templates/deployment-worker.yaml").stdout)
