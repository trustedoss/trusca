    ...
    *,
    ...
    async with factory() as session:
        ...
        finding = VulnerabilityFinding(
            ...
            # X1 SLA — None simulates a pre-0041 legacy row (API falls back to
            # created_at); an explicit value backdates the SLA clock.
...
async def test_detail_response_carries_kev_fields(client) -> None:
    """Drawer KEV badge regression — the detail serializer used to drop
    ``kev`` / ``kev_due_date`` from the response model, so a KEV-listed CVE
    silently rendered the schema defaults (false / null) in the drawer."""
    _, team, user = await _seed_team_with_user(client)
    _, scan_id = await _seed_scanned_project(client, team_id=team.id)
    finding_id = await _seed_finding(
        client,
    ...
    assert body["kev_due_date"] == "2026-07-22"
...
# ---------------------------------------------------------------------------
# X1 SLA — first_detected_at / sla_due_date / sla_status response fields,
# ?sla= filter, sort=sla_due (nulls last).
# ---------------------------------------------------------------------------


async def test_list_response_carries_sla_fields_overdue(client) -> None:
    """critical (7d window) detected 30d ago → overdue, due == first + 7d."""
    _, team, user = await _seed_team_with_user(client)
    project_id, scan_id = await _seed_scanned_project(client, team_id=team.id)
    first = (datetime.now(tz=UTC) - timedelta(days=30)).replace(microsecond=0)
    await _seed_finding(
        client, scan_id=scan_id, severity="critical", first_detected_at=first
    )
    headers = _bearer_for(user)
    ...
    assert response.status_code == 200, response.text
    item = response.json()["items"][0]
    assert datetime.fromisoformat(item["first_detected_at"]) == first
    assert datetime.fromisoformat(item["sla_due_date"]) == first + timedelta(days=7)
    assert item["sla_status"] == "overdue"


async def test_list_first_detected_falls_back_to_created_at(client) -> None:
    """Legacy row (NULL first_detected_at) → COALESCE(created_at); a fresh
    high (30d window) is inside the window and beyond the 7d imminent edge."""
    _, team, user = await _seed_team_with_user(client)
    project_id, scan_id = await _seed_scanned_project(client, team_id=team.id)
    await _seed_finding(
        client, scan_id=scan_id, severity="high", first_detected_at=None
    )
    headers = _bearer_for(user)
    ...
    assert response.status_code == 200, response.text
    item = response.json()["items"][0]
    assert item["first_detected_at"] == item["discovered_at"]
    assert item["sla_due_date"] is not None
    assert item["sla_status"] == "ok"


async def test_list_info_severity_has_no_sla(client) -> None:
    _, team, user = await _seed_team_with_user(client)
    project_id, scan_id = await _seed_scanned_project(client, team_id=team.id)
    await _seed_finding(client, scan_id=scan_id, severity="info")
    headers = _bearer_for(user)
    ...
    assert response.status_code == 200, response.text
    item = response.json()["items"][0]
    assert item["sla_due_date"] is None
    assert item["sla_status"] is None
    assert item["first_detected_at"] is not None  # the clock itself always runs


async def test_list_sla_filter_overdue(client) -> None:
    """?sla=overdue keeps only past-due findings; ok / no-SLA rows drop."""
    _, team, user = await _seed_team_with_user(client)
    project_id, scan_id = await _seed_scanned_project(client, team_id=team.id)
    overdue_id = await _seed_finding(
        client,
        scan_id=scan_id,
        severity="critical",
        first_detected_at=datetime.now(tz=UTC) - timedelta(days=30),
    )
    await _seed_finding(client, scan_id=scan_id, severity="high")  # fresh → ok
    await _seed_finding(client, scan_id=scan_id, severity="info")  # no SLA
    headers = _bearer_for(user)
    ...
    body = response.json()
    assert body["total"] == 1
    assert body["items"][0]["id"] == str(overdue_id)
    assert body["items"][0]["sla_status"] == "overdue"


async def test_list_sla_filter_ok_excludes_overdue_and_no_sla(client) -> None:
    _, team, user = await _seed_team_with_user(client)
    project_id, scan_id = await _seed_scanned_project(client, team_id=team.id)
    await _seed_finding(
        client,
    ...
    ok_id = await _seed_finding(client, scan_id=scan_id, severity="high")
    await _seed_finding(client, scan_id=scan_id, severity="unknown")
    headers = _bearer_for(user)
    ...
    body = response.json()
    assert body["total"] == 1
    assert body["items"][0]["id"] == str(ok_id)


async def test_list_sla_bad_token_returns_422(client) -> None:
    _, team, user = await _seed_team_with_user(client)
    project_id, _ = await _seed_scanned_project(client, team_id=team.id)
    headers = _bearer_for(user)
...
async def test_list_sort_sla_due_asc_urgent_first_nulls_last(client) -> None:
    """sort=sla_due asc: earliest due first; no-SLA (info) rows LAST."""
    _, team, user = await _seed_team_with_user(client)
    project_id, scan_id = await _seed_scanned_project(client, team_id=team.id)
    now = datetime.now(tz=UTC)
    overdue_critical = await _seed_finding(
        client,
        scan_id=scan_id,
        severity="critical",
        first_detected_at=now - timedelta(days=30),  # due 23d ago
    )
    fresh_high = await _seed_finding(
        client, scan_id=scan_id, severity="high"  # due in ~30d
    )
    no_sla_info = await _seed_finding(client, scan_id=scan_id, severity="info")
    headers = _bearer_for(user)

    response = await client.get(
        f"/v1/projects/{project_id}/vulnerabilities",
        headers=headers,
        params={"sort": "sla_due", "order": "asc", "limit": 100},
    )
    assert response.status_code == 200, response.text
    order = [i["id"] for i in response.json()["items"]]
    assert order == [str(overdue_critical), str(fresh_high), str(no_sla_info)]


async def test_list_sort_sla_due_desc_keeps_nulls_last(client) -> None:
    _, team, user = await _seed_team_with_user(client)
    project_id, scan_id = await _seed_scanned_project(client, team_id=team.id)
    now = datetime.now(tz=UTC)
    overdue_critical = await _seed_finding(
    ...
    fresh_high = await _seed_finding(client, scan_id=scan_id, severity="high")
    no_sla_info = await _seed_finding(client, scan_id=scan_id, severity="info")
    headers = _bearer_for(user)
    ...
    response = await client.get(
        f"/v1/projects/{project_id}/vulnerabilities",
        headers=headers,
        params={"sort": "sla_due", "order": "desc", "limit": 100},
    )
    assert response.status_code == 200, response.text
    order = [i["id"] for i in response.json()["items"]]
    # desc: latest due first (fresh high), then overdue critical; NULLs still last.
    assert order == [str(fresh_high), str(overdue_critical), str(no_sla_info)]


async def test_detail_response_carries_sla_fields(client) -> None:
    _, team, user = await _seed_team_with_user(client)
    _, scan_id = await _seed_scanned_project(client, team_id=team.id)
    first = (datetime.now(tz=UTC) - timedelta(days=30)).replace(microsecond=0)
    finding_id = await _seed_finding(
    ...
    assert response.status_code == 200, response.text
    body = response.json()
    assert datetime.fromisoformat(body["first_detected_at"]) == first
    assert datetime.fromisoformat(body["sla_due_date"]) == first + timedelta(days=7)
    assert body["sla_status"] == "overdue"


async def test_detail_no_sla_severity_yields_nulls(client) -> None:
    _, team, user = await _seed_team_with_user(client)
    _, scan_id = await _seed_scanned_project(client, team_id=team.id)
    finding_id = await _seed_finding(client, scan_id=scan_id, severity="unknown")
    headers = _bearer_for(user)
    ...
    assert response.status_code == 200, response.text
    body = response.json()
    assert body["first_detected_at"] is not None  # falls back to created_at
    assert body["sla_due_date"] is None
    assert body["sla_status"] is None
