...
FIXTURES = os.getenv(
    "GOLDEN_FIXTURES",
    str(Path.home() / "projects" / "baseline-scan" / "tests" / "fixtures" / "projects"),
)
# Self-contained subset committed in-repo so the BUG-008 guard ("scan succeeded
# but 0 components") runs in nightly even without the external baseline-scan corpus
# (BASELINE_CORPUS_REPO_URL secret). Resolution prefers the external corpus and falls
