...
def normalize_license_name(raw: str | None) -> str | None:
    ...
    # S5-B — fall through to OSORI's curated alias list for spellings the rules
    # above were not written for ("Apache 2", "The MIT License (MIT)",
    # "Android-Apache-2.0"). Deliberately LAST: `_RULES` is order-dependent and
    # hand-tuned, so a curated lookup belongs after it rather than inside it,
    # where it could shadow a more specific rule.
    #
    # This is a lookup, not a heuristic. An alias two licences both claim is
    # dropped upstream rather than resolved, because a wrong SPDX id silently
    # reclassifies a component's obligations — worse than the `None` this
    # function has always returned for the unknown case.
    return _osori_alias(n)


@lru_cache(maxsize=1)
def _osori_canonical_aliases() -> dict[str, str]:
    """OSORI's aliases, re-keyed through :func:`_canon`.

    The snapshot stores them lowercased but otherwise verbatim, so an alias
    written ``"Android-Apache-2.0"`` sits under that exact string — while every
    name arriving here has already been through ``_canon``, which collapses
    ``. _ / -`` and whitespace and would look for ``"android apache 2 0"``.
    Re-keying makes both sides agree instead of matching by luck.

    Collapsing can make two distinct aliases identical. Those are dropped, on
    the same reasoning the snapshot loader drops aliases two licences claim: a
    wrong SPDX id silently reclassifies a component's obligations, which is
    worse than the ``None`` this module has always returned for the unknown
    case.
    """
    from services.license_osori import osori_alias_map

    claims: dict[str, set[str]] = {}
    for alias, spdx_id in osori_alias_map().items():
        key = _canon(alias)
        if key:
            claims.setdefault(key, set()).add(spdx_id)
    return {key: next(iter(owners)) for key, owners in claims.items() if len(owners) == 1}


def _osori_alias(canonical: str) -> str | None:
    """Look *canonical* up in the vendored OSORI alias list.

    Imported lazily so this module — on the hot ingest path — does not pull the
    snapshot into memory for callers whose names all match the rules above.
    """
    return _osori_canonical_aliases().get(canonical)
