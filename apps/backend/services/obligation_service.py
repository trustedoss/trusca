...
(security review, low severity) so a non-member cannot distinguish "exists but
...
read shapes for the latest-scan working set (schema verification, PR #13).
"""
...
# Defense-in-depth caps on the obligation drawer payload (security review
...
# G2 follow-up (security review, low severity/Info from PR #107) — the per-license
...
# security review finding — the "text not bundled" one-liners are cheap but were
...
async def list_project_obligations(
    ...
    *,
    ...
    - ``ProjectNotFound`` (404) if the project id doesn't exist OR the actor is
      not a team member (existence-hide, security review, low severity). We log
    ...
    assert_team_access(
        ...
        # Existence-hide cross-team reads as 404 (security review, low severity):
...
def _purl_source_url(purl: str | None) -> str | None:
    ...
    # security review finding (deliberate deviation from the BomLens jq, which
...
def _license_text_sections(
    ...
    texts clipped by :data:`_NOTICE_LICENSE_TEXT_CAP` plus (security review
...
def _display_spdx_id(spdx_id: str) -> str:
    ...
    security review finding: the plain-text branch interpolates the operand into
...
async def _load_notice_data(
    ...
    *,
    ...
    # security review finding: clamp the copyright INSIDE the aggregate. raw_data
