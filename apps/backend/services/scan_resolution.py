...
def latest_succeeded_scan_select(
    project_filter: ColumnElement[bool] | None = None,
) -> Select[tuple[uuid.UUID, uuid.UUID]]:
    """``SELECT project_id, scan_id`` — the current-state scan per project.

    The multi-project form of :func:`latest_succeeded_scan_id`, returned as a
    statement rather than executed so callers can either run it (the
    organization-wide inventory resolves the ids once and passes them on) or
    compose it into a larger query.

    Why it lives here rather than in the caller: the cross-project surfaces —
    the portfolio dashboard, the project list's risk badge, and the inventory —
    each need "the current-state scan for N projects at once", and each had
    started to grow its own copy. The rule has to be IDENTICAL to the
    single-project resolver above, or the inventory would list a component the
    owning project's Components tab does not show: same data, two answers.
    ``test_scan_resolution.py`` asserts the two agree for the same project.

    *project_filter* is applied to a joined ``Project``, so a caller can pass
    the team-scope predicate and get a set already narrowed to what the actor
    may read. Omit it to cover every project.

    ``superseded_at`` is deliberately NOT filtered — the single-project
    resolver does not filter it either, and matching the per-project detail
    surfaces is what keeps the cross-project ones honest.
    (``project_list_enrichment`` does add that filter for the risk badge; that
    divergence predates this function and is left alone rather than silently
    changed here.)
    """
    stmt = (
