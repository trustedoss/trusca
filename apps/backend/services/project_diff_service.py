...
* **vulnerabilities** — keyed by ``(vulnerability_id, component_version_id)``
  over the OPEN finding set: a finding whose status is ``not_affected`` /
  ``false_positive`` / ``suppressed`` / ``fixed`` (resolved/suppressed, incl.
  via VEX) counts as NOT open. introduced = open-in-target minus open-in-base;
  resolved = the converse. One query per side returns the open keys + severity
  + names; the diff is a set difference over the keys. The portfolio-wide
  version of this same question lives in ``services.dashboard_trends_service``,
  which imports the closed-status tuple from here and is held to these numbers
  by ``tests/integration/test_dashboard_trends_parity.py``.
...
async def _open_findings_by_key(
    ...
    *,
    ...
    """``{(vulnerability_id, component_version_id): finding info}`` for OPEN findings.

    "Open" = status NOT IN ``_CLOSED_FINDING_STATUSES`` (resolved / suppressed,
    incl. via VEX, are excluded). The key is the exposure's identity: this CVE
    against this package version. Two packages that happen to share a version
    string are two exposures, and the same CVE recorded twice against one
    package version is one — an earlier key of ``(cve external_id, version
    string)`` collapsed the first case into the second, silently dropping one
    package from both the introduced and the resolved list whenever a CVE hit
    two packages at the same version (a monorepo publishing its packages in
    lockstep is the ordinary way that happens).

    One indexed query per side; the set difference happens in Python over the
    two small key sets.
    """
    stmt = (
        select(
    ...
    result = await session.execute(stmt)
    findings: dict[tuple[uuid.UUID, uuid.UUID], dict[str, Any]] = {}
    for r in result.all():
        key = (r.vulnerability_id, r.component_version_id)
        # Dedupe on the exposure: two finding rows for the same CVE against the
        # same package version describe one thing to fix, so keep the first.
        if key in findings:
            continue
        findings[key] = {
            "cve_id": r.cve_id,
...
def _diff_vulnerabilities(
    base: dict[tuple[uuid.UUID, uuid.UUID], dict[str, Any]],
    target: dict[tuple[uuid.UUID, uuid.UUID], dict[str, Any]],
    ...
    purl) — see ``_order`` for why the purl is not optional.
    """
    base_keys = set(base.keys())
    target_keys = set(target.keys())

    # The purl is part of the sort key, not decoration. Two exposures can share
    # a CVE, a package name AND a version string — Maven artefacts differing
    # only by classifier do — and a sort with ties falls back to whatever order
    # Postgres returned, which is not stable across executions. A diff feeding
    # a PR comment would then produce a different comment body for identical
    # inputs.
    def _order(entry: dict[str, Any]) -> tuple[str, str, str, str]:
        return (
            entry["cve_id"],
    ...
    introduced_rows = sorted((target[k] for k in target_keys - base_keys), key=_order)
    resolved_rows = sorted((base[k] for k in base_keys - target_keys), key=_order)
