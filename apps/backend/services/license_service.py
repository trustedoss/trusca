...
# monorepo could materialize a many-thousand-row JSON list — security review
...
def _osori_reference(spdx_id: str | None) -> dict[str, object] | None:
    """OSORI's record for *spdx_id*, shaped for the wire.

    Imported lazily: the snapshot is ~140 KB of package data and only the
    license drawer ever asks for it.
    """
    if not spdx_id:
        return None
    from services.license_osori import osori_license, osori_source

    record = osori_license(spdx_id)
    if record is None:
        return None
    return {
        "name": record.name,
        ...
        "restrictions": [
            f"{name} (level {level})" if level is not None else name
            for name, level in record.restrictions
        ],
        "source": osori_source(),
    }
...
async def get_license_finding_detail(
    ...
    *,
    ...
    return {
        "id": finding.id,
        # S5-B — OSORI's reading of this license, when it has one. Its own
        # object, never folded into `category` / `summary`: those are this
        # deployment's classification (52 licenses, pinned by contract tests,
        # consumed by the build gate) and OSORI is an outside opinion covering
        # ~670. Most valuable exactly where `summary` is null.
        "osori": _osori_reference(lic.spdx_id),
        "license_id": lic.id,
        "spdx_id": lic.spdx_id,
        "name": lic.name,
        "summary": summary.en if summary else None,
