...
async def list_projects(
    ...
    *,
    ...
    if q and q.strip():
        ...
        # to find a project: display name, slug, and clone URL. git_url is
        # here because the list page's search box has always advertised it
        # ("search by name or git URL") while the server only matched name and
        # slug — the client-side filter was carrying it, which meant the axis
        # silently disappeared once the list outgrew one page (S1-3).
        #
        # Wildcards in the user's input are escaped so a literal '%' matches a
        # percent sign instead of collapsing the ILIKE into "match everything"
        # (S1-2). ix_projects_name_trgm serves the leading wildcard on name;
        # slug and git_url ride the team-scoped row set.
