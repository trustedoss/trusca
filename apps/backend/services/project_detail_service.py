...
# W2 #31 — commercial-SCA-style "Usage" rank for ``ScanComponent.dependency_scope``
...
async def distributions_for_scan(
    session: AsyncSession,
    *,
    scan_id: uuid.UUID,
) -> tuple[dict[str, int], dict[str, int], int]:
    """``(severity_distribution, license_distribution, component_count)`` for one scan.

    The population is ``scan_components`` — every component the scan saw — with
    findings LEFT JOINed onto it. That is the detail the numbers turn on: a
    component with no licence row at all is not absent from the licence
    distribution, it lands in ``unknown``, and ``unknown`` is a scored band. A
    version that starts from ``license_findings`` instead silently drops those
    components and reports a lower licence risk for a project whose licences
    are simply not known.

    Extracted so the Overview tab and the governance band cannot disagree.
    They previously computed this from different starting tables, and the band
    read zero on a project the tab scored — the exact drift the band exists to
    avoid. Anything else that needs "the Overview's numbers" should call this
    ...
    severity = dict.fromkeys(_ALL_SEVERITY_KEYS, 0)
    license_ = dict.fromkeys(_ALL_LICENSE_KEYS, 0)
    total = 0
    for row in (await session.execute(agg_stmt)).all():
        ...
        count = int(row.n)
        severity[sev_key] = severity.get(sev_key, 0) + count
        license_[lic_key] = license_.get(lic_key, 0) + count
        total += count
    return severity, license_, total
...
async def get_project_overview(
    ...
    *,
    ...
    if aggregate_scan_id is not None:
        ...
        # The per-component aggregation lives in `distributions_for_scan` so
        # the governance band can produce the same numbers from the same
        # query. It used to be inline here, and the copy that grew elsewhere
        # started from `license_findings` instead of `scan_components` —
        # silently dropping every component with no licence row, which is a
        # scored band, not an absence.
        ...
        # #26 — malicious rollup. Counts only ``flagged``; a not-yet-assessed
        # catalog yields 0 here, which is why the surface must distinguish
        # "0 flagged" from "nothing assessed" (the chip reads the snapshot
        # presence, not just this number).
        malicious_count_stmt = (
            select(func.count(func.distinct(ScanComponent.component_version_id)))
            .select_from(ScanComponent)
            .join(
                ComponentVersion,
        ...
        (
            ...
            malicious_count_result,
        ) = await asyncio.gather(
            distributions_for_scan(session, scan_id=aggregate_scan_id),
    ...
    *,
    ...
    base = (
        select(
            ...
            ComponentVersion.malicious_state.label("malicious_state"),
            ComponentVersion.malicious_id.label("malicious_id"),
            ComponentVersion.malicious_source.label("malicious_source"),
    ...
    # Search: ILIKE on component name OR purl, served by the GIN trigram
    # indexes added in migration 0043.
    #
    # The second axis used to be ``namespace``, which made the search box's
    # own placeholder ("search by name or purl") false twice over: purl was
    # never matched, and namespace is a column the scan ingest never writes,
    # so the OR branch could not match anything. Matching purl makes the
    # promise true and lets a user paste a coordinate straight in (S1-4).
    # Wildcards are escaped so a literal '%' stays a percent sign (S1-2).
    ...
    # W2 #31 — commercial-SCA-style "Usage" filter. The cv-level rank is compared to the
    ...
    # #26 — malicious filter. ``malicious=true`` keeps only flagged rows
    # (served by the partial index ix_component_versions_malicious).
    # ``malicious=false`` keeps everything else INCLUDING not-yet-assessed
    # (NULL) rows: the boolean asks "is this package known-malicious", and an
    # unassessed row is not. Callers who need "assessed and clean" read
    # ``malicious_state`` on the rows rather than this filter — collapsing
    # NULL into ``clear`` here would let an unevaluated catalog masquerade as
    # a clean one.
    if malicious is not None:
        if malicious:
            base = base.where(ComponentVersion.malicious_state == "flagged")
        else:
            base = base.where(
                or_(
                    ComponentVersion.malicious_state.is_(None),
        ...
        items.append(
            {
                ...
                # W2 #31 — commercial-SCA-style "Usage" mapped back from the per-cv rank.
                ...
                "malicious_state": r.malicious_state,
                "malicious_id": r.malicious_id,
                "malicious_source": r.malicious_source,
                "eol_date": r.eol_date,
    ...
    *,
    ...
    cv_stmt = (
        select(
            ...
            ComponentVersion.malicious_id,
            ComponentVersion.malicious_source,
    ...
    return {
        ...
        # W2 #31 — commercial-SCA-style "Usage" for the chosen path. Only ``required`` /
        ...
        "malicious_state": row.malicious_state,
        "malicious_id": row.malicious_id,
        "malicious_source": row.malicious_source,
