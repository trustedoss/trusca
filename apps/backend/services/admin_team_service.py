...
async def _lock_team_for_destructive_op(
    ...
    """
    Load the team WITH ``FOR UPDATE`` row lock (security review finding, CWE-367).
...
async def _lock_and_count_team_admins(session: AsyncSession, team_id: uuid.UUID) -> int:
    ...
    Reference: the same optimistic-concurrency pattern as
    ``vulnerability_service.py:309``.
    """
    locked = (
        (
...
async def list_teams(
    ...
    *,
    ...
    if search and search.strip():
        # Wildcards escaped so a literal '%' matches a percent sign rather
        # than collapsing the filter into "every team" (S1-2).
        like = f"%{escape_like(search.strip())}%"
        base = base.where(Team.name.ilike(like, escape="\\"))
        count_base = count_base.where(Team.name.ilike(like, escape="\\"))
...
async def delete_team(
    ...
    *,
    ...
    Concurrency (security review finding, CWE-367): the team row is locked
