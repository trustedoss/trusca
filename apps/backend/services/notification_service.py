...
# backslashes outright. ``#`` fragments stay rejected everywhere.
...
#
# Query strings (X1 step 2 amendment — REQUIRES security review sign-off):
# the original Medium follow-up rejected ``?`` outright to defeat
# open-redirect primitives (``?return_to=//evil``, ``?next=javascript:...``).
# X1's SLA alert needs a filter deep link (``?tab=vulnerabilities&sla=
# overdue``) and the approvals alert has ALWAYS built ``/approvals?id=…``
# (silently degraded to NULL under the blanket ban). Instead of the blanket
# ban we now allow a query component restricted to the charset
# ``[A-Za-z0-9_=&-]`` — no ``/`` ``:`` ``.`` ``%`` — which makes every known
# redirect/scheme payload INEXPRESSIBLE inside the QUERY: ``//evil`` needs
# slashes, ``javascript:`` needs a colon, ``evil.example.com`` needs dots,
# and ``%2F%2F`` percent-smuggling needs ``%``. Every pre-amendment
# adversarial case in tests/unit/services/test_notification_link_validator.py
# is still rejected verbatim; the suite adds query-shaped attack cases.
#
# Scope of that guarantee (security review, X1 step 2): the ``%``/``.``/
# ``:`` exclusions apply to the query component ONLY — the PATH component
# has always accepted them (``/p/%2F%2Fevil`` passes today, as before this
# amendment). That is acceptable under the CONSUMPTION CONTRACT this
# validator is allowed to assume: a notification ``link`` is consumed
# exclusively by the SPA's React Router ``navigate(item.link)``
# (NotificationsPage) — a same-origin ``history.pushState``, never an
# ``<a href>``, ``window.location`` sink or an outbound e-mail body. Anyone
# adding a new consumer of ``link`` MUST revisit this validator first; a
# unit test pins the path-side acceptance so a tightening is a conscious
# decision, not a silent behavior drift.
_LINK_SAFE_RE = re.compile(
    r"^/(?![/\\])[^\x00-\x1f\x7f\\?#]*(\?[A-Za-z0-9_=&-]*)?$"
)


def _validate_link(link: str | None) -> str | None:
    """Return ``link`` if same-origin safe (path + allowlisted query); else ``None``."""
    if link is None:
        return None
