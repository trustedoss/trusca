...
Model (stateful, so results stay readable in the UI):
...
Audit (security review Critical): hard delete leaves no row in
...
# ---------------------------------------------------------------------------
# Audit (security review Critical — explicit rows for the hard-delete path)
# ---------------------------------------------------------------------------
...
def scan_retention_task() -> dict[str, Any]:
    ...
    # before it runs (security review, low severity — config footgun visibility).
    log.info(
...
def _reclaim_aged(
    ...
    between the read and the delete is not destroyed (security review, high severity —
