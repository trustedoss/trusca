...
# Total wall-clock deadline for the download (security review MINOR-1).
...
# (security review MINOR-2). The refresh task feeds the parsed keys into a
...
def fetch_kev_catalog(*, http: httpx.Client | None = None) -> dict[str, KevEntry]:
    ...
    if url.strip().lower().startswith("http://"):
        # security review INFO — a plaintext mirror is an operator choice
    ...
    try:
        try:
            ...
            httpx.HTTPError,
            # security review INFO — httpx.InvalidURL is NOT an HTTPError
