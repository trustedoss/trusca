...
def _podfile_present(source_dir: Path) -> bool:
    ...
    an unbounded walk over an attacker-controlled tree is a denial-of-service
    surface, so the search stops at depth 3. ``Pods/`` copies are ignored —
    ``Pods/Manifest.lock`` trees ship their own Podfiles that do not make the
    project a CocoaPods root.
    """
...
def _build_cdxgen_env(
    *, source_dir: Path, output_dir: Path, verbose: bool = False
    ...
    Starts from the scrubbed cdxgen env (security review, medium severity v2,
