...
class ScancodeDisabled(ScancodeError):
    """scancode stage turned off via ``SCANCODE_ENABLED=false`` — detection skipped.

    feat/demo-sandbox-scan: the public sandbox worker sets ``SCANCODE_ENABLED=false``
    to shed the per-scan cost. A ``ScancodeError`` subclass so the scan pipeline's
    existing ``except ScancodeError`` best-effort handler treats it as a skip and
    continues on declared (cdxgen) licenses — no pipeline change required.
    """
...
def run_scancode(
    *,
    ...
    Raises:
        ...
        ScancodeDisabled: ``SCANCODE_ENABLED=false`` — the stage is skipped and
            the caller continues on declared (cdxgen) licenses only.
    """
    # feat/demo-sandbox-scan: honour the master switch BEFORE touching disk or a
    # subprocess. When off, skip the whole stage (the pipeline's best-effort
    # ScancodeError handler continues on declared cdxgen licenses only).
    if not scancode_enabled():
        raise ScancodeDisabled(
            "scancode stage disabled (SCANCODE_ENABLED=false); continuing with "
            "declared (cdxgen) licenses only.",
        )
...
def _build_command(
    *, source_dir: Path, result_path: Path, verbose: bool = False
    ...
    Ignore globs (security review, medium severity): scancode's ``*`` does NOT cross
    ...
    Symlinks (security review, low severity): scancode does NOT follow symlinks by
...
def _parse_detections(result_path: Path, *, cap: int) -> list[DetectedLicense]:
    ...
    # Result-size ceiling (security review, medium severity): the JSON is keyed off
...
def _split_spdx_expression(expression: str, *, source_path: str | None = None) -> list[str]:
    ...
    Security (security review, high severity): the expression is derived from
