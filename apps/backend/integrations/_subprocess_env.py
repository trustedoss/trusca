...
CLAUDE.md core rule #11 (no module-level env caching) + security review
Medium #1 + Medium #1 v2 — combined source of
truth.
...
_CREDENTIAL_DENY_SUBSTRINGS: tuple[str, ...] = (
    ...
    # for the same reason ``token`` is dropped — security review finding.
    "session",
    "bearer",
    "cookie",
)
...
def scrubbed_env_for_prep() -> dict[str, str]:
    """Env dict for the lockfile-resolver subprocesses."""
    return _build_env(
...
def scrubbed_env_for_cdxgen() -> dict[str, str]:
    """Env dict for the cdxgen invocation."""
    return _build_env(
...
# adapter. (security review finding on PR #196 — bring trivy to parity with
...
def scrubbed_env_for_trivy() -> dict[str, str]:
    """Env dict for ``trivy image`` / ``trivy sbom`` invocations.
