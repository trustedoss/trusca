...
# Defense-in-depth size cap on the on-disk Trivy report (security review finding
...
class TrivyFailed(TrivyError):
    ...
    ``problem+json`` ``detail`` field exposed to API callers (security review
...
# ---------------------------------------------------------------------------
# Internal: workspace boundary guard (security review finding on PR #196)
# ---------------------------------------------------------------------------
...
def run_trivy_sbom(
    ...
    *,
    ...
    cmd = [
        ...
        # paths against secret patterns. (security review finding on PR #196.)
        "--scanners",
        "vuln",
    ]
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    (security review finding on PR #196). Real Trivy output for even the largest
...
# The admin panel surfaces this so operators see the configured cadence next to
# the actual ``UpdatedAt`` from the on-disk metadata.
...
class TrivyDbStatus:
    ...
    panel. All fields are optional so the "not yet downloaded"
