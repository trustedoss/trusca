...
# ---------------------------------------------------------------------------
# feat/demo-sandbox-scan — PUBLIC-DEMO OPT-IN CARVE-OUT (security review target).
#
# When DEMO_READ_ONLY is on AND demo_allow_sandbox_scans() is on, the read-only
# demo additionally permits EXACTLY the two scan-ingest write path families so a
# visitor can drive a bounded (≤10 MiB) live scan / SBOM upload against the
# seeded "Demo Sandbox" project. This is a DELIBERATELY minimal widening:
#   * only these two path families,
#   * POST only (keyed on method, like the auth allow-list),
#   * and ONLY when the SECOND opt-in flag (demo_allow_sandbox_scans) is set.
# With that flag OFF (the default) the demo behaves 100% as before — fully
# locked — so an operator must consciously opt in to expose any write surface.
#
# Bypass hardening: the patterns are matched against the ALREADY-NORMALIZED path
# (via _normalize_path) so a traversal such as
# '/v1/projects/x/scans/../../admin/foo' collapses to '/v1/projects/admin/foo'
# BEFORE the regex runs and therefore does NOT match — it cannot smuggle a write
# onto a non-sandbox surface. Each pattern is anchored (^...$) and uses [^/]+ for
# the project-id segment so it matches exactly ONE path segment there and cannot
# be widened by extra trailing segments (e.g. '/v1/projects/x/scans/extra' fails).
# The endpoints keep their own auth (require_role("developer")): passing this
# middleware gate is NOT authentication — an anonymous caller still gets 401.
# ---------------------------------------------------------------------------
_DEMO_SANDBOX_WRITE_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("POST", re.compile(r"^/v1/projects/[^/]+/scans$")),
    ("POST", re.compile(r"^/v1/projects/[^/]+/sbom-ingest$")),
)
...
def _is_demo_write_allowed(method: str, path: str) -> bool:
    ...
    Order:
      ...
      2. the exact (METHOD, normalized-path) auth allow-list;
      3. the feat/demo-sandbox-scan carve-out — ONLY when
         ``demo_allow_sandbox_scans()`` is also on — permits the two
         scan-ingest POST path families. The regex is applied to the SAME
         normalized path so traversal cannot smuggle a write past the guard.
    ...
    if (upper, normalized) in _DEMO_WRITE_ALLOWLIST:
        return True
    if demo_allow_sandbox_scans():
        for pattern_method, pattern in _DEMO_SANDBOX_WRITE_PATTERNS:
            if upper == pattern_method and pattern.match(normalized):
                return True
    return False
