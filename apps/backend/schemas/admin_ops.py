...
replaced by Trivy. All DT-shaped schemas (DTStatusOut, DTOrphanItem,
...
class AdminDiskItem(BaseModel):
    ...
    Threshold cells are static configuration (80% warn, 90% critical);
...
class HealthComponent(BaseModel):
    ...
    detail: str | None = Field(
        default=None,
        description=(
            "Human-readable explanation of the status (1-line), in English. "
            "Prefer ``detail_code`` when it is present — this field stays as "
            "the fallback for the states that have no code (probe exceptions) "
            "and for non-UI consumers."
        ),
    )
    detail_code: str | None = Field(
        default=None,
        description=(
            "Stable identifier for a KNOWN detail, e.g. ``celery.no_workers``. "
            "Set only for the states this service produces itself, so a UI can "
            "translate them; ``null`` when ``detail`` carries a probe "
            "exception, which is a diagnostic and is not translated."
        ),
    )
    detail_params: dict[str, str | int | float] | None = Field(
        default=None,
        description=(
            "Interpolation values for ``detail_code`` (e.g. ``{\"count\": 3}``)."
        ),
    )
...
class TrivyDbStatusOut(BaseModel):
    """
    Response of ``GET /v1/admin/trivy/health``.
