...
TeamScopedRole = Literal["super_admin", "group_admin", "developer", "viewer"]
...
class ProjectOverviewResponse(BaseModel):
    ...
    malicious_count: int = Field(
        default=0,
        description=(
            "#26 — count of distinct components in the anchored scan that the "
            "vendored OSV MAL- snapshot lists as known-malicious. Counts only "
            "``flagged``; a catalog that was never assessed yields 0 here, so "
            "surfaces must read snapshot presence before rendering this as "
            "reassurance."
        ),
    )
...
class ComponentSummary(BaseModel):
    ...
    dependency_scope: Literal["required", "optional"] | None = Field(
        default=None,
        description=(
            "W2 #31 — commercial-SCA-style 'Usage' for the component, derived from the "
    ...
    malicious_state: Literal["flagged", "clear"] | None = Field(
        default=None,
        description=(
            "Known-malicious verdict for this component version (#26). "
            "``flagged`` = the package appears in the vendored OSV MAL- "
            "snapshot, meaning it was published to attack whoever installs "
            "it; ``clear`` = the snapshot was consulted and did not list it; "
            "``null`` = NOT ASSESSED (no snapshot loaded, or flagging "
            "disabled). "
            "``clear`` and ``null`` must not be conflated on screen — the "
            "index is a deny list, so 'absent from it' is a real answer while "
            "'never looked' is not. This is deliberately NOT a vulnerability: "
            "it creates no finding and never enters severity counts, because "
            "the response is removal plus credential rotation rather than an "
            "upgrade."
        ),
    )
    malicious_id: str | None = Field(
        default=None,
        description=(
            "OSV advisory id behind a ``flagged`` verdict (e.g. "
            "``MAL-2025-47141``). ``null`` unless flagged."
        ),
    )
    malicious_source: str | None = Field(
        default=None,
        description=(
            "Snapshot the verdict came from, ``osv.dev@YYYY-MM-DD``. Always "
            "shown beside the verdict: the claim is 'not in this snapshot', "
            "never 'safe today'."
        ),
    )
...
class ComponentDetailResponse(BaseModel):
    ...
    dependency_scope: Literal["required", "optional"] | None = Field(
        default=None,
        description=(
            "W2 #31 — commercial-SCA-style 'Usage' for the chosen (shallowest) path. "
    ...
    malicious_state: Literal["flagged", "clear"] | None = Field(
        default=None,
        description=(
            "#26 — known-malicious verdict (see ComponentSummary."
            "malicious_state for the full contract). ``clear`` means the "
            "snapshot was consulted and did not list it; ``null`` means it "
            "was never assessed."
        ),
    )
    malicious_id: str | None = Field(
        default=None, description="OSV advisory id behind a flagged verdict."
    )
    malicious_source: str | None = Field(
        default=None, description="Snapshot the verdict came from."
    )
    eol_state: Literal["eol", "supported", "unknown"] | None = Field(
        default=None,
        description=(
