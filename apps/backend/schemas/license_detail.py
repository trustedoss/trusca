...
class OsoriReference(BaseModel):
    """What OSORI says about a license — reference material, not our verdict.

    Deliberately a nested object rather than fields alongside ``summary`` and
    ``category``. Those are this product's own classification, pinned by
    contract tests and consumed by the build gate; OSORI is an outside opinion
    that reaches ~670 licenses instead of our 52. Its own envelope means a
    reader — and the UI — can never mistake one for the other.

    Useful mainly for licenses outside our catalog, where ``summary`` is null
    and there would otherwise be nothing to say at all.
    """

    model_config = ConfigDict(from_attributes=True)

    name: str | None = None
    #: Whether distribution requires a notification, in OSORI's terms.
    ...
    #: OSORI's caution items, each carrying a 1-5 level.
    restrictions: list[str] = Field(default_factory=list)
    #: Attribution the data's ODC-By 1.0 license requires wherever it appears.
    source: str = "OSORI (olis.or.kr), ODC-By 1.0"
...
class LicenseDetailResponse(BaseModel):
    ...
    osori: OsoriReference | None = Field(
        default=None,
        description=(
            "Reference metadata from the OSORI license database. Null when "
            "OSORI has no record for this SPDX id, or when the data is "
            "disabled. Never merged into `category` / `summary`, which stay "
            "this deployment's own classification."
        ),
    )
    review_flag: ReviewFlag | None = Field(
        default=None,
