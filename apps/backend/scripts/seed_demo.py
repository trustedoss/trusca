...
-------------
  ...
  * 6 users               — 1 super_admin + 3 team_admins + 1 developer
                            + 1 explorer (the address the login page prints)
...
# feat/demo-sandbox-scan — the sandbox project's name is owned by the write
# guard (services.scan_service) so the seed and the H-2 guard can never drift.
# Imported at module level (after the sys.path insertion above) and re-exported
# here for the shared-vocabulary contract test. This pulls only ORM class
# definitions — no DB connection — so ``seed_demo --help`` still works env-free.
from services.scan_service import (  # noqa: E402
    DEMO_SANDBOX_PROJECT_NAME as DEMO_SANDBOX_PROJECT_NAME,
)
...
# The address the public login page prints for a first-time visitor. Kept as a
# named constant because the frontend hard-codes the same string
# (apps/frontend/src/pages/auth/DemoCredentialsHint.tsx) and the two have to
# agree — a visitor who clicks "fill demo credentials" and cannot sign in has
# no way to tell which side is wrong.
_DEMO_EXPLORE_EMAIL = "explore@demo.trustedoss.dev"

# feat/demo-sandbox-scan — the single project a public-demo visitor scans /
# ingests into when the demo opts into sandbox writes (DEMO_READ_ONLY +
# DEMO_ALLOW_SANDBOX_SCANS). Lives in the Backend team so the seeded developer
# account (dev@demo.trustedoss.dev) can trigger scans (require_role developer).
_DEMO_SANDBOX_SLUG = "demo-sandbox"
    ...
    try:
        async with factory() as session:
            ...
            if existing_org is not None:
                ...
                # feat/demo-sandbox-scan: seed the sandbox project on the
                # already-seeded path too (idempotent) so an upgraded demo stack
            ...
            # The account the public login page offers to a first-time visitor.
            #
            # It exists because the team-named admins do not read as team names
            # on that screen. The login page shows one address and no other
            # context, so `frontend-admin@` was being read as "the account that
            # shows you the frontend" — as if the portal had a frontend view and
            # a backend view and you picked one at sign-in. It does not; the
            # account decides which team's data you see, and the teams happen to
            # be named after disciplines.
            #
            # `explore@` promises nothing about what you will see, which is the
            # point. It sits in Frontend because that team carries the richest
            # CVE and license data, so the demo looks like a portal in use.
            explorer = User(
                email=_DEMO_EXPLORE_EMAIL,
                hashed_password=hashed_password,
            ...
            # Same team and same role as frontend-admin: the visitor sees what
            # that account sees, under a name that does not describe a screen.
            session.add(
                Membership(
                    user_id=explorer.id,
            ...
            # ── feat/demo-sandbox-scan: the public-demo sandbox project ────
            sandbox_id = await _seed_demo_sandbox(session)
...
async def _seed_demo_sandbox(session: Any) -> str | None:
    """Idempotently seed the public-demo "Demo Sandbox" project.

    feat/demo-sandbox-scan: when the read-only demo opts into sandbox writes
    (``DEMO_READ_ONLY`` + ``DEMO_ALLOW_SANDBOX_SCANS``) a visitor drives a
    bounded (≤10 MiB) live scan / SBOM upload against THIS project via
    ``POST /v1/projects/{id}/scans`` and ``POST /v1/projects/{id}/sbom-ingest``.

    It lives in the Backend team so the seeded developer account
    (``dev@demo.trustedoss.dev``, role ``developer``) can trigger scans — those
    endpoints are ``require_role("developer")``. No ``git_url`` is pinned: the
    visitor supplies their own ≤10 MiB source URL or uploads an SBOM.

    Idempotent on the (team, slug) pair and runs on BOTH the fresh-seed and the
    already-seeded short-circuit paths, so an existing demo stack also gains the
    sandbox project on upgrade. Returns the project id (existing or newly
    created), or ``None`` when the carve-out is off or the demo org / Backend
    team is absent.

    Gated on ``demo_read_only() and demo_allow_sandbox_scans()`` (L-1 — aligned
    with the ``/health`` ``demo_sandbox_scans`` signal, which is likewise the
    AND of both flags): the sandbox project is only reachable when the read-only
    demo is on AND opts into sandbox writes. Materialising it on an env where
    ``DEMO_READ_ONLY`` is off (a normal deploy) would create a dead project, and
    on the default read-only demo (carve-out off) it would push the documented
    5-project list to 6 and break the quickstart / docs-uat visible-count
    contract. Gating on BOTH flags keeps every non-sandbox deploy byte-for-byte
    unchanged. Read at call time (CLAUDE.md core rule #11).
    """
    from core.config import demo_allow_sandbox_scans, demo_read_only

    if not (demo_read_only() and demo_allow_sandbox_scans()):
        return None

    from sqlalchemy import select

    from models import Organization, Project, Team, User
    ...
    backend_team = (
        await session.execute(
            select(Team).where(
                Team.organization_id == org.id, Team.slug == "backend"
            )
        )
    ).scalar_one_or_none()
    if backend_team is None:
        return None

    existing = (
        await session.execute(
            select(Project).where(
                Project.team_id == backend_team.id,
                Project.slug == _DEMO_SANDBOX_SLUG,
            )
        )
    ).scalar_one_or_none()
    ...
    sandbox = Project(
        ...
        name=DEMO_SANDBOX_PROJECT_NAME,
        slug=_DEMO_SANDBOX_SLUG,
        description=(
            "Public-demo sandbox — run a ≤10 MiB live scan or upload a "
            "CycloneDX SBOM here. Writes are accepted only when the demo opts "
            "into sandbox scans (DEMO_ALLOW_SANDBOX_SCANS)."
        ),
        default_branch="main",
        visibility="team",
        created_by_user_id=developer.id if developer else None,
    )
    session.add(sandbox)
    await session.flush()
...
async def _seed_verify_baseline(session: Any) -> dict[str, Any]:
    ...
    # A scan row parked in `queued` forever, for the specs that need one to
    # exist: scan-retention captures it via `GET /v1/scans?status=queued` to
    # prove an active scan cannot be deleted, and the dashboard specs count it.
    #
    # Not seeded on the public demo. Nothing will ever move it — no Celery task
    # was enqueued (see below) — so a visitor sees a scan sitting at 0% that
    # never finishes, which reads as a broken portal rather than as a fixture.
    # The specs run against a locally-booted stack with APP_ENV=dev
    # (.github/workflows/ci.yml), never against the demo host, so skipping it
    # here costs no coverage.
    #
    # Read at call time, not module import — CLAUDE.md core rule #11.
    seed_queued_scan = os.getenv("APP_ENV", "dev").lower() != "demo"
    ...
    if queued_host is not None and seed_queued_scan:
        queued = (
            await session.execute(
                select(Scan).where(
    ...
    summary["queued_scan"] = queued_host is not None and seed_queued_scan
