...
# ``--super-admin`` (security review finding / CWE-489 Active Debug Code).
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Attach every seeded component to EVERY project's scan, not just "
            "the first. Default: off — components anchor on the first project "
            "alone, which is what per-project scenarios want. Cross-project "
            "scenarios (the organization-wide inventory: 'one package, N "
            "projects') need the same package present in more than one "
            "project, and cannot express that otherwise."
        ),
    )
    parser.add_argument(
        "--with-obligations",
    ...
    try:
        async with factory() as session:
            ...
            if component_count > 0 and project_rows:
                ...
                # Which scans the components get attached to. Findings
                # (licence, CVE) stay anchored on the first project so the
                # per-project counts every existing scenario asserts do not
                # move; only the component membership fans out.
                component_scan_ids: list[uuid.UUID] = [anchor_scan_id]
                if share_components:
                    component_scan_ids = [
                        p.latest_scan_id
                        for p in project_rows
                        if p.latest_scan_id is not None
                    ]
                ...
                for i in range(component_count):
                    ...
                    if i == 2:
                        # #26 — deterministic malicious fixture: the THIRD
                        # seeded component is flagged. Columns written directly
                        # for the same reason as the two blocks above — the e2e
                        # must not depend on which packages the live OSV
                        # snapshot happens to list this week, and seeding a
                        # REAL malicious purl would make the fixture rot the
                        # moment that advisory is withdrawn.
                        cv.malicious_state = "flagged"
                        cv.malicious_id = "MAL-0000-SEED"
                        cv.malicious_source = "osv.dev@seed"
                        cv.malicious_evaluated_at = datetime.now(tz=UTC)
                    else:
                        # Every other row is assessed-and-clean, which is what
                        # a real scan produces. Leaving them NULL would make
                        # the "not assessed vs clear" distinction untestable.
                        cv.malicious_state = "clear"
                        cv.malicious_source = "osv.dev@seed"
                        cv.malicious_evaluated_at = datetime.now(tz=UTC)
                    session.add(cv)
                    await session.flush()
                    ...
                    # `share_components` puts the SAME package in every
                    # project, which is the only way to seed the shape the
                    # organization-wide inventory exists to report ("one
                    # package, N projects"). Off by default so per-project
                    # scenarios keep their single-project fixture.
                    for target_scan_id in component_scan_ids:
                        session.add(
                            ScanComponent(
    ...
    try:
        summary = asyncio.run(
            _seed(
                ...
                share_components=args.share_components,
