...
Why:
  ...
  - Stateful model, so results stay readable in the UI: scans are kept, but
    each (project, ref) target keeps only its latest succeeded snapshot.
    Older ones are stamped ``superseded_at`` and reclaimed after a grace
    period. Scans carrying an explicit ``metadata.release`` label are never
    superseded.
