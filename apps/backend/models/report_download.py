...
PII note:
  - ``client_ip`` (INET) and ``user_agent`` (VARCHAR 512) are operational
    PII. The masking + retention policy is owned by the the API layer
...
class ReportDownload(Base):
    ...
    Lifecycle:
      - INSERT only. The download endpoint (follow-up API PR) emits a
    ...
    # follow-up API PR's responsibility at emit time.
