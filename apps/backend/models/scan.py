...
class Component(Base):
    ...
    __table_args__ = (
        ...
        # S1-1 — every component search matches with a leading wildcard
        # (``ILIKE '%term%'``), which the b-tree above cannot serve. GIN
        # trigram indexes do, and they are also what makes ``similarity()``
        # ranking affordable later. Created in migration 0043.
        Index(
            "ix_components_name_trgm",
            "name",
            postgresql_using="gin",
            postgresql_ops={"name": "gin_trgm_ops"},
        ),
        Index(
            "ix_components_purl_trgm",
            "purl",
            postgresql_using="gin",
            postgresql_ops={"purl": "gin_trgm_ops"},
        ),
    )
...
class ComponentVersion(Base):
    ...
    # Malicious-package flag (#26, OSV MAL- advisories). Same catalog-fact
    # shape as EOL/KEV: a package published to attack its installers is that
    # for everyone observing the purl, not per project.
    #
    # NULL vs 'clear' carries meaning here, unlike EOL. The EOL map is a
    # WHITELIST, so "unmapped" is simply unknown and stays NULL. The malicious
    # index is a DENY list, so "absent from the index" is a real verdict — the
    # snapshot looked and did not find it. NULL is reserved for "never
    # evaluated" (no snapshot loaded, or a row predating this column), which is
    # why the UI must not draw a zero when nothing was assessed.
    #
    # malicious_state closed vocabulary ('flagged' | 'clear') enforced at the
    # application layer (services/malicious/malicious_catalog.py), VARCHAR not
    # native ENUM per the kev_sync_state.last_result convention.
    # Widths from the 2026-08-03 snapshot: advisory ids max 15 chars
    # ('MAL-2025-192894'), source is 'osv.dev@YYYY-MM-DD' (18).
    malicious_state: Mapped[str | None] = mapped_column(String(16), nullable=True)
    malicious_id: Mapped[str | None] = mapped_column(String(32), nullable=True)
    malicious_source: Mapped[str | None] = mapped_column(String(64), nullable=True)
    malicious_evaluated_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    __table_args__ = (
        ...
        # Partial: flagged rows are a vanishing minority of any real catalog
        # (the whole point — 232,747 known-malicious PURLs exist across eight
        # ecosystems, and a healthy project matches none of them), while
        # 'clear' would index nearly every row for no query. Mirrors the EOL
        # and currency partials.
        Index(
            "ix_component_versions_malicious",
            "malicious_state",
            postgresql_where=text("malicious_state = 'flagged'"),
        ),
    )
...
class Vulnerability(Base):
    ...
    __table_args__ = (
        ...
        # S1-1 — CVE id and summary are both searched with a leading wildcard.
        # The GIN above is for the ``references`` JSONB and does nothing for
        # text search; these two are the ones that serve it. Migration 0043.
        Index(
            "ix_vulnerabilities_external_id_trgm",
            "external_id",
            postgresql_using="gin",
            postgresql_ops={"external_id": "gin_trgm_ops"},
        ),
        Index(
            "ix_vulnerabilities_summary_trgm",
            "summary",
            postgresql_using="gin",
...
class VulnerabilityFinding(Base):
    ...
    # ``created_at`` resets on every scan, this does not. NULLable on purpose:
    # NULL = "row predates 0041"; readers COALESCE to ``created_at``.
