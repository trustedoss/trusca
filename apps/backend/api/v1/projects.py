...
from schemas.project_governance import ProjectGovernance
...
from services.project_governance_service import get_project_governance
...
def _problem_for_scan_error(request: Request, exc: ScanError) -> Response:
    ...
    # envelope.
    #
    # M1 (security review): we deliberately do NOT include the team's live
...
@router.get(
    ...
    summary="Aggregated risk / scan picture for the project (Overview tab)",
)
...
# ---------------------------------------------------------------------------
# GET /v1/projects/{project_id}/governance  (W17 — the band above the tabs)
# ---------------------------------------------------------------------------


@router.get(
    "/{project_id}/governance",
    response_model=ProjectGovernance,
    summary="Risk score, gate verdict, KEV deadlines, approvals and trend (W17 band)",
)
async def get_project_governance_endpoint(
    request: Request,
    project_id: uuid.UUID,
    session: AsyncSession = Depends(get_db),
    actor: CurrentUser = Depends(require_role("viewer")),
...
async def list_project_components_endpoint(
    ...
    direct: bool | None = Query(
        ...
        description=(
            ...
            "deps. Omit to include both. commercial-SCA equivalent of the 'Dependency "
            "type' facet."
        ),
    ),
    dependency_scope: list[str] | None = Query(
        default=None,
        description=(
            "W2 #31 — commercial-SCA-style 'Usage' facet. Repeatable; accepted values: "
    ...
    malicious: bool | None = Query(
        default=None,
        description=(
            "Known-malicious facet (#26). ``true`` keeps only components the "
            "vendored OSV MAL- snapshot lists as published-to-attack; "
            "``false`` keeps everything else, INCLUDING components that were "
            "never assessed. Omit to include both. Note the asymmetry with "
            "``eol``: this axis is not a vulnerability — a flagged package is "
            "removed and its reachable credentials rotated, not upgraded — so "
            "it creates no finding and never enters severity counts."
        ),
    ),
...
#
# L1 (security review): why `shared_limit` with a FIXED `scope`, not `@limit`.
