...
/**
 * The app's own Korean strings.
 *
 * These were outside the linter's reach until a reviewer found "트리아지하는
 * 그 스캔에서 …" on the login screen — English word order wearing Korean
 * particles, shipped to users, while the linter reported zero findings
 * because it had only ever been pointed at the documentation. UI copy is read
 * far more often than the guides are.
 *
 * The files are JSON, and no rule in the catalogue can match a key: keys are
 * ASCII identifiers and every pattern is Korean text. So the raw file is
 * linted as-is, which keeps line numbers honest for free.
 */
export const KO_LOCALE_ROOTS = [
  path.join(REPO_ROOT, "apps", "frontend", "src", "locales", "ko"),
];
...
function maskLine(line) {
  ...
  // Inline code spans: blank the CONTENT, keep the delimiters.
  //
  // This used to blank the backticks too, and that hid a whole class of error
  // from the catalogue. `latin-particle-space` fires on a Korean particle
  // written with a space after the word it belongs to, and by far the most
  // common form of it in these guides is after a code span — "`SBOM` 은",
  // 357 occurrences. With the delimiters gone the rule saw a space where the
  // span had been, so the character before the particle was not a character it
  // could recognise, and the linter reported a clean run over every one of
  // them. A gate that cannot see the most common shape of the error it exists
  // to catch is the failure this project keeps relearning.
  //
  // Keeping the delimiters is safe for the other rules: no pattern in the
  // catalogue matches a backtick, so nothing new can fire, and it removes a
  // false-positive shape instead — "에 `x` 의해" used to mask down to
  // "에       의해", which `passive-eui-hae` matched across a code span that
  // was never part of the phrase.
  out = out.replace(
    /`([^`]*)`/g,
    (_m, inner) => "`" + " ".repeat(inner.length) + "`",
  );
...
/** Extensions a target may carry: KO guides are markdown, UI strings are JSON. */
const LINTABLE = [".md", ".mdx", ".json"];

export function isLintable(rel) {
  return LINTABLE.some((ext) => rel.endsWith(ext));
}

/** Recursively collect lintable files under a root (paths relative to it). */
function walkFiles(root) {
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      for (const rel of walkFiles(abs)) out.push(path.join(entry.name, rel));
...
function resolveTargets(opts) {
  const roots = [KO_DOCS_ROOT, ...KO_LOCALE_ROOTS];
  if (opts.mode === "all") {
    return roots.flatMap((root) =>
      walkFiles(root).map((rel) => ({
        abs: path.join(root, rel),
        rel: path.join(path.relative(REPO_ROOT, root), rel),
      })),
    );
  }
  let paths;
  ...
  const prefixes = roots.map((root) => path.relative(REPO_ROOT, root));
  return paths
    .map((p) => (path.isAbsolute(p) ? path.relative(REPO_ROOT, p) : p))
    .filter((rel) => prefixes.some((prefix) => rel.startsWith(prefix)) && isLintable(rel))
