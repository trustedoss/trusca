...
TrustedOSS Portal 의 한국어 텍스트(문서 미러 + 앱 UI 문자열)가 영어/일본어
직역 티(번역투)를 내지 않도록 점검하는 무의존 Node 린터.
`im-not-ai`(이승현/Liam Lee)의 "10대분류 × 심각도" 접근을 산문 문서에 맞게
**결정적 정규식 규칙**으로 옮긴 것이다. LLM·외부 의존성·API 비용 없음.

## 무엇을 점검하나

- 대상: 한국어 문서 미러(`docs-site/i18n/ko/docusaurus-plugin-content-docs/current/**`)와
  앱 UI 문자열(`apps/frontend/src/locales/ko/*.json`). 영어 문서는 범위 밖.
- UI 문자열은 JSON 원문을 그대로 린트한다. 키는 전부 ASCII 식별자이고 규칙은 전부
  한국어 패턴이라 키에 오탐이 날 수 없고, 줄 번호가 파일과 그대로 맞는다.
- **범위가 넓어진 계기**: 로그인 화면에 "트리아지하는 그 스캔에서 …" 라는 문장이
  나갔다. 영어 어순에 한국어 조사만 붙인 문장인데, 린터는 바로 옆 문서를 검사하고
  "0건" 을 보고했다 — UI 문자열은 애초에 대상이 아니었기 때문이다. 사용자는 가이드보다
  화면을 훨씬 자주 읽는다.
- **한계를 분명히**: 범위를 넓혔다고 그 문장이 자동으로 잡히지는 않았다. 카탈로그에
  `relative-clause-geu` 규칙(S3)을 새로 넣어 그 형태를 지목하게 했지만, "번역처럼
  읽힌다" 는 정규식이 가리킬 수는 있어도 판정할 수는 없는 종류다. 최종 판단은 사람이다.
...
용어 일관성 규칙의 정규형은 `docs-site/docs/reference/glossary.md` 가 단일 진실이다.
