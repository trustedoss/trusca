...
export const ACCOUNTS = {
  ...
  // 로그인 화면이 첫 방문자에게 안내하는 계정. 권한은 team_admin_frontend와
  // 같고, 이름만 팀을 가리키지 않는다 — 화면에 주소 한 줄만 보이는 자리에서
  // `frontend-admin@`은 "프런트엔드 화면을 보여주는 계정"으로 읽혔다.
  explorer: { email: "explore@demo.trustedoss.dev", pw: DEFAULT_PW },
};
